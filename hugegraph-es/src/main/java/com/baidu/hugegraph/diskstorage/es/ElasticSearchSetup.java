...
public enum ElasticSearchSetup {
    ...
    NODE {
        ...
        public Connection connect(Configuration config) throws IOException {
            ...
            // Apply explicit HugeGraph properties file overrides (otherwise conf-file or ES defaults apply)
     ...
     *
     * @param config a HugeGraph configuration possibly containing Elasticsearch index settings
    ...
    private static ImmutableSettings.Builder settingsBuilder(Configuration config) throws IOException {
        ...
        // Set HugeGraph defaults
        settings.put("client.transport.ignore_cluster_name", true);

        // Apply overrides from ES conf file
        applySettingsFromFile(settings, config, INDEX_CONF_FILE);

        // Apply ext.* overrides from HugeGraph conf file
        applySettingsFromHugeGraphConf(settings, config, ElasticSearchIndex.ES_EXTRAS_NS);

        // Apply individual HugeGraph ConfigOptions that map to ES settings
