...
public abstract class AbstractCassandraStoreManager extends DistributedStoreManager
    ...
    public static final ConfigNamespace CASSANDRA_NS = new ConfigNamespace(GraphDatabaseConfiguration.STORAGE_NS,
            "cassandra", "Cassandra storage backend options");

    public static final ConfigOption<String> CASSANDRA_KEYSPACE = new ConfigOption<String>(CASSANDRA_NS, "keyspace",
            "The name of HugeGraph's keyspace.  It will be created if it does not exist.", ConfigOption.Type.LOCAL,
            "hugegraph");
    ...
    public static final ConfigOption<String> CASSANDRA_READ_CONSISTENCY =
            new ConfigOption<String>(CASSANDRA_NS, "read-consistency-level",
                    "The consistency level of read operations against Cassandra", ConfigOption.Type.MASKABLE, "QUORUM");

    public static final ConfigOption<String> CASSANDRA_WRITE_CONSISTENCY = new ConfigOption<String>(CASSANDRA_NS,
            "write-consistency-level", "The consistency level of write operations against Cassandra",
            ConfigOption.Type.MASKABLE, "QUORUM");

    public static final ConfigOption<Boolean> ATOMIC_BATCH_MUTATE = new ConfigOption<Boolean>(CASSANDRA_NS,
            "atomic-batch-mutate", "True to use Cassandra atomic batch mutation, false to use non-atomic batches",
    ...
    public static final ConfigOption<String> REPLICATION_STRATEGY = new ConfigOption<String>(CASSANDRA_NS,
            "replication-strategy-class", "The replication strategy to use for HugeGraph keyspace",
    ...
    public static final ConfigOption<String> COMPACTION_STRATEGY =
            new ConfigOption<String>(CASSANDRA_NS, "compaction-strategy-class",
                    "The compaction strategy to use for HugeGraph tables", ConfigOption.Type.FIXED, String.class);
    ...
    // Compression
    public static final ConfigOption<Boolean> CF_COMPRESSION = new ConfigOption<Boolean>(CASSANDRA_NS, "compression",
    ...
    public static final ConfigOption<String> CF_COMPRESSION_TYPE =
            new ConfigOption<String>(CASSANDRA_NS, "compression-type",
                    "The sstable_compression value HugeGraph uses when creating column families. "
                            + "This accepts any value allowed by Cassandra's sstable_compression option. "
                            + "Leave this unset to disable sstable_compression on HugeGraph-created CFs.",
                    ConfigOption.Type.MASKABLE, "LZ4Compressor");

    public static final ConfigOption<Integer> CF_COMPRESSION_BLOCK_SIZE = new ConfigOption<Integer>(CASSANDRA_NS,
            "compression-block-size", "The size of the compression blocks in kilobytes", ConfigOption.Type.FIXED, 64);
    ...
    public static final ConfigOption<Boolean> SSL_ENABLED = new ConfigOption<Boolean>(SSL_NS, "enabled",
            "Controls use of the SSL connection to Cassandra", ConfigOption.Type.LOCAL, false);

    public static final ConfigOption<String> SSL_TRUSTSTORE_LOCATION = new ConfigOption<String>(SSL_TRUSTSTORE_NS,
            "location", "Marks the location of the SSL Truststore.", ConfigOption.Type.LOCAL, "");

    public static final ConfigOption<String> SSL_TRUSTSTORE_PASSWORD = new ConfigOption<String>(SSL_TRUSTSTORE_NS,
            "password", "The password to access SSL Truststore.", ConfigOption.Type.LOCAL, "");

    // Thrift transport
    public static final ConfigOption<Integer> THRIFT_FRAME_SIZE_MB = new ConfigOption<>(CASSANDRA_NS, "frame-size-mb",
            "The thrift frame size in megabytes", ConfigOption.Type.MASKABLE, 15);

    /**
     * The default Thrift port used by Cassandra. Set {@link GraphDatabaseConfiguration#STORAGE_PORT} to override.
     * <p>
     * Value = {@value}
     */
    ...
    public AbstractCassandraStoreManager(Configuration config) {
        ...
        if (config.get(SSL_ENABLED) && config.get(SSL_TRUSTSTORE_LOCATION).isEmpty())
            throw new IllegalArgumentException(
                    SSL_TRUSTSTORE_LOCATION.getName() + " could not be empty when SSL is enabled.");
        ...
        if (config.has(REPLICATION_OPTIONS)) {
            ...
            if (options.length % 2 != 0)
                throw new IllegalArgumentException(
                        REPLICATION_OPTIONS.getName() + " should have even number of elements.");
            ...
            this.strategyOptions =
                    ImmutableMap.of("replication_factor", String.valueOf(config.get(REPLICATION_FACTOR)));
        }
        ...
        if (config.has(COMPACTION_OPTIONS)) {
            ...
            if (options.length % 2 != 0)
                throw new IllegalArgumentException(
                        COMPACTION_OPTIONS.getName() + " should have even number of elements.");
        ...
        if (partitioner == null) {
            try {
                ...
                throw new HugeGraphException(
                        "Could not connect to Cassandra to read partitioner information. Please check the connection",
                        e);
            }
        }
    ...
    public StoreFeatures getFeatures() {
        ...
        if (features == null) {
            ...
            Configuration global = GraphDatabaseConfiguration.buildGraphConfiguration()
                    .set(CASSANDRA_READ_CONSISTENCY, "QUORUM").set(CASSANDRA_WRITE_CONSISTENCY, "QUORUM")
            ...
            Configuration local = GraphDatabaseConfiguration.buildGraphConfiguration()
                    .set(CASSANDRA_READ_CONSISTENCY, "LOCAL_QUORUM").set(CASSANDRA_WRITE_CONSISTENCY, "LOCAL_QUORUM")
                    .set(METRICS_PREFIX, GraphDatabaseConfiguration.METRICS_SYSTEM_PREFIX_DEFAULT);
    ...
    /**
     * Returns a map of compression options for the column family {@code cf}. The contents of the returned map must be
     * identical to the contents of the map returned by
     * {@link org.apache.cassandra.thrift.CfDef#getCompression_options()}, even for implementations of this method that
     * don't use Thrift.
     *
     * @param cf the name of the column family for which to return compression options
     * @return map of compression option names to compression option values
     * @throws com.baidu.hugegraph.diskstorage.BackendException if reading from Cassandra fails
     */
