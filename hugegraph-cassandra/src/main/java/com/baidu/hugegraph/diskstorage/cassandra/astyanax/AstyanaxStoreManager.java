...
import static com.baidu.hugegraph.diskstorage.cassandra.CassandraTransaction.getTx;
...
public class AstyanaxStoreManager extends AbstractCassandraStoreManager {
     ...
     * <p/>
     */
    public static final ConfigOption<String> CLUSTER_NAME = new ConfigOption<String>(ASTYANAX_NS, "cluster-name",
            "Default name for the Cassandra cluster", ConfigOption.Type.MASKABLE, "HugeGraph Cluster");

    /**
     * Maximum pooled connections per host.
     * <p/>
     */
    public static final ConfigOption<Integer> MAX_CONNECTIONS_PER_HOST = new ConfigOption<Integer>(ASTYANAX_NS,
            "max-connections-per-host", "Maximum pooled connections per host", ConfigOption.Type.MASKABLE, 32);

    /**
     * Maximum open connections allowed in the pool (counting all hosts).
     * <p/>
     */
    public static final ConfigOption<Integer> MAX_CONNECTIONS = new ConfigOption<Integer>(ASTYANAX_NS,
            "max-connections", "Maximum open connections allowed in the pool (counting all hosts)",
     ...
     * <p/>
     * These connections are mostly idle and only used for DDL operations (like creating keyspaces). HugeGraph doesn't
     * need many of these connections in ordinary operation.
     */
    public static final ConfigOption<Integer> MAX_CLUSTER_CONNECTIONS_PER_HOST =
            new ConfigOption<Integer>(ASTYANAX_NS, "max-cluster-connections-per-host",
                    "Maximum pooled \"cluster\" connections per host", ConfigOption.Type.MASKABLE, 3);

    /**
     * How Astyanax discovers Cassandra cluster nodes. This must be one of the values of the Astyanax NodeDiscoveryType
     * enum.
     * <p/>
     */
    public static final ConfigOption<String> NODE_DISCOVERY_TYPE =
            new ConfigOption<String>(ASTYANAX_NS, "node-discovery-type",
                    "How Astyanax discovers Cassandra cluster nodes", ConfigOption.Type.MASKABLE, "RING_DESCRIBE");

    /**
     * Astyanax specific host supplier useful only when discovery type set to DISCOVERY_SERVICE or TOKEN_AWARE. Excepts
     * fully qualified class name which extends google.common.base.Supplier<List<Host>>.
     */
    public static final ConfigOption<String> HOST_SUPPLIER = new ConfigOption<String>(ASTYANAX_NS, "host-supplier",
            "Host supplier to use when discovery type is set to DISCOVERY_SERVICE or TOKEN_AWARE",
            ConfigOption.Type.MASKABLE, String.class);

    /**
     * Astyanax's connection pooler implementation. This must be one of the values of the Astyanax ConnectionPoolType
     * enum.
     * <p/>
     */
    public static final ConfigOption<String> CONNECTION_POOL_TYPE =
            new ConfigOption<String>(ASTYANAX_NS, "connection-pool-type", "Astyanax's connection pooler implementation",
                    ConfigOption.Type.MASKABLE, "TOKEN_AWARE");

    /**
     * In Astyanax, RetryPolicy and RetryBackoffStrategy sound and look similar but are used for distinct purposes.
     * RetryPolicy is for retrying failed operations. RetryBackoffStrategy is for retrying attempts to talk to
     * uncommunicative hosts. This config option controls RetryPolicy.
     */
    public static final ConfigOption<String> RETRY_POLICY = new ConfigOption<String>(ASTYANAX_NS, "retry-policy",
            "Astyanax's retry policy implementation with configuration parameters", ConfigOption.Type.MASKABLE,
            "com.netflix.astyanax.retry.BoundedExponentialBackoff,100,25000,8");

    /**
     * If non-null, this must be the fully-qualified classname (i.e. the complete package name, a dot, and then the
     * class name) of an implementation of Astyanax's RetryBackoffStrategy interface. This string may be followed by a
     * sequence of integers, separated from the full classname and from each other by commas; in this case, the integers
     * are cast to native Java ints and passed to the class constructor as arguments. Here's an example setting that
     * would instantiate an Astyanax FixedRetryBackoffStrategy with an delay interval of 1s and suspend time of 5s:
     * <p/>
     * <code>
     * com.netflix.astyanax.connectionpool.impl.FixedRetryBackoffStrategy,1000,5000
     * </code>
     * <p/>
     * If null, then Astyanax uses its default strategy, which is an ExponentialRetryBackoffStrategy instance. The
     * instance parameters take Astyanax's built-in default values, which can be overridden via the following config
     * keys:
     * <ul>
     ...
     */
    public static final ConfigOption<String> RETRY_BACKOFF_STRATEGY = new ConfigOption<String>(ASTYANAX_NS,
            "retry-backoff-strategy", "Astyanax's retry backoff strategy with configuration parameters",
            ConfigOption.Type.MASKABLE, "com.netflix.astyanax.connectionpool.impl.FixedRetryBackoffStrategy,1000,5000");

    /**
     * Controls the retryDelaySlice parameter on Astyanax ConnectionPoolConfigurationImpl objects, which is in turn used
     * by ExponentialRetryBackoffStrategy. See the code for {@link ConnectionPoolConfigurationImpl},
     * {@link ExponentialRetryBackoffStrategy}, and the javadoc for {@link #RETRY_BACKOFF_STRATEGY} for more
     * information.
     * <p/>
     * This parameter is not meaningful for and has no effect on FixedRetryBackoffStrategy.
     */
    public static final ConfigOption<Integer> RETRY_DELAY_SLICE = new ConfigOption<Integer>(ASTYANAX_NS,
            "retry-delay-slice", "Astyanax's connection pool \"retryDelaySlice\" parameter", ConfigOption.Type.MASKABLE,
            ConnectionPoolConfigurationImpl.DEFAULT_RETRY_DELAY_SLICE);
    /**
     * Controls the retryMaxDelaySlice parameter on Astyanax ConnectionPoolConfigurationImpl objects, which is in turn
     * used by ExponentialRetryBackoffStrategy. See the code for {@link ConnectionPoolConfigurationImpl},
     * {@link ExponentialRetryBackoffStrategy}, and the javadoc for {@link #RETRY_BACKOFF_STRATEGY} for more
     * information.
     * <p/>
     * This parameter is not meaningful for and has no effect on FixedRetryBackoffStrategy.
     */
    public static final ConfigOption<Integer> RETRY_MAX_DELAY_SLICE = new ConfigOption<Integer>(ASTYANAX_NS,
            "retry-max-delay-slice", "Astyanax's connection pool \"retryMaxDelaySlice\" parameter",
            ConfigOption.Type.MASKABLE, ConnectionPoolConfigurationImpl.DEFAULT_RETRY_MAX_DELAY_SLICE);

    /**
     * Controls the retrySuspendWindow parameter on Astyanax ConnectionPoolConfigurationImpl objects, which is in turn
     * used by ExponentialRetryBackoffStrategy. See the code for {@link ConnectionPoolConfigurationImpl},
     * {@link ExponentialRetryBackoffStrategy}, and the javadoc for {@link #RETRY_BACKOFF_STRATEGY} for more
     * information.
     * <p/>
     * This parameter is not meaningful for and has no effect on FixedRetryBackoffStrategy.
     */
    public static final ConfigOption<Integer> RETRY_SUSPEND_WINDOW = new ConfigOption<Integer>(ASTYANAX_NS,
            "retry-suspend-window", "Astyanax's connection pool \"retryMaxDelaySlice\" parameter",
     ...
     */
    public static final ConfigOption<Integer> THRIFT_FRAME_SIZE = new ConfigOption<Integer>(ASTYANAX_NS, "frame-size",
    ...
    public static final ConfigOption<String> LOCAL_DATACENTER =
            new ConfigOption<String>(ASTYANAX_NS, "local-datacenter",
                    ...
                    /*
                     * It's between either LOCAL or MASKABLE. MASKABLE could be useful for cases where all the HugeGraph
                     * instances are closest to the same Cassandra DC.
                     */
                    ConfigOption.Type.MASKABLE, String.class);
    ...
    public AstyanaxStoreManager(Configuration config) throws BackendException {
        ...
        localDatacenter = config.has(LOCAL_DATACENTER) ? config.get(LOCAL_DATACENTER) : "";
        ...
        this.keyspaceContext =
                getContextBuilder(config, maxConnsPerHost, "Keyspace").buildKeyspace(ThriftFamilyFactory.getInstance());
    ...
    @Override
    public synchronized AstyanaxKeyColumnValueStore openDatabase(String name, StoreMetaData.Container metaData)
            throws BackendException {
        if (openStores.containsKey(name))
            return openStores.get(name);
        else {
            ensureColumnFamilyExists(name);
            AstyanaxKeyColumnValueStore store =
                    new AstyanaxKeyColumnValueStore(name, keyspaceContext.getClient(), this, retryPolicy);
    ...
    private void ensureColumnFamilyExists(String name, String comparator) throws BackendException {
        ...
        try {
            ...
            if (!found) {
                ColumnFamilyDefinition cfDef = cl.makeColumnFamilyDefinition().setName(name).setKeyspace(keySpaceName)
    ...
    private AstyanaxContext.Builder getContextBuilder(Configuration config, int maxConnsPerHost, String usedFor) {
        ...
        ConnectionPoolConfigurationImpl cpool =
                new ConnectionPoolConfigurationImpl(usedFor + "HugeGraphConnectionPool").setPort(port)
                        .setMaxOperationsPerConnection(maxOperationsPerConnection).setMaxConnsPerHost(maxConnsPerHost)
                        .setRetryDelaySlice(retryDelaySlice).setRetryMaxDelaySlice(retryMaxDelaySlice)
                        .setRetrySuspendWindow(retrySuspendWindow).setSocketTimeout(connectionTimeout)
                        .setConnectTimeout(connectionTimeout).setSeeds(StringUtils.join(hostnames, ","));
        ...
        AstyanaxConfigurationImpl aconf = new AstyanaxConfigurationImpl().setConnectionPoolType(poolType)
                .setDiscoveryType(discType).setTargetCassandraVersion("1.2").setMaxThriftSize(thriftFrameSizeBytes);
        ...
        if (config.get(SSL_ENABLED)) {
            cpool.setSSLConnectionContext(
                    new SSLConnectionContext(config.get(SSL_TRUSTSTORE_LOCATION), config.get(SSL_TRUSTSTORE_PASSWORD)));
        }

        AstyanaxContext.Builder ctxBuilder = new AstyanaxContext.Builder();

        // Standard context builder options
        ctxBuilder.forCluster(clusterName).forKeyspace(keySpaceName).withAstyanaxConfiguration(aconf)
                .withConnectionPoolConfiguration(cpool).withConnectionPoolMonitor(new CountingConnectionPoolMonitor());
    ...
    private void ensureKeyspaceExists(Cluster cl) throws BackendException {
        ...
        try {
            ksDef = cl.makeKeyspaceDefinition().setName(keySpaceName)
                    .setStrategyClass(storageConfig.get(REPLICATION_STRATEGY)).setStrategyOptions(strategyOptions);
    ...
    private static RetryBackoffStrategy getRetryBackoffStrategy(String desc) throws PermanentBackendException {
        ...
        try {
            ...
            throw new PermanentBackendException("Failed to instantiate Astyanax RetryBackoffStrategy implementation",
                    e);
        }
    }
