 ...
 * janusgraph-hadoop-core should only touch parts of Hadoop core ABI that have remained stable since 1.0, and everything
 ...
 * are released, or as janusgraph-hadoop-core uses additional Hadoop features, or as bugs are discovered. It's possible
 * to write and use a third-party implementation, but be prepared to update it when upgrading to a newer JanusGraph
 * release.
 */
public interface HadoopCompat {
    ...
    /**
     * Construct a {@link org.janusgraph.hadoop.config.job.JobClasspathConfigurer} that sets the Mapreduce job jar
    ...
    /**
     * Construct a {@link org.janusgraph.hadoop.config.job.JobClasspathConfigurer} that walks the classpath and adds all
