...
import com.baidu.hugegraph.diskstorage.util.EntryArrayList;
...
public class HadoopScanMapper extends Mapper<StaticBuffer, Iterable<Entry>, NullWritable, NullWritable> {
    ...
    protected com.baidu.hugegraph.diskstorage.configuration.Configuration jobConf;
    ...
    static Configuration getJobConfiguration(ModifiableHadoopConfiguration scanConf) {
        if (!scanConf.has(HugeGraphHadoopConfiguration.SCAN_JOB_CONFIG_ROOT)) {
            log.debug("No job configuration root provided");
            return null;
        }
        ConfigNamespace jobRoot = getJobRoot(scanConf.get(HugeGraphHadoopConfiguration.SCAN_JOB_CONFIG_ROOT));
        return ModifiableHadoopConfiguration.prefixView(jobRoot, HugeGraphHadoopConfiguration.SCAN_JOB_CONFIG_KEYS,
                scanConf);
    }
