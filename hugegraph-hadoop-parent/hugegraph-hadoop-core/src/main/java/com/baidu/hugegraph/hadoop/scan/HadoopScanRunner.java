...
import com.baidu.hugegraph.hadoop.compat.HadoopCompatLoader;
...
public class HadoopScanRunner {
     ...
     * <p>
     * The {@code confRootField} parameter must be a string in the format {@code package.package...class#fieldname},
     * where {@code fieldname} is the name of a public static field on the class specified by the portion of the string
     * before the {@code #}. The {@code #} itself is just a separator and is discarded.
     * <p>
     * When a MapReduce task process prepares to execute the {@code ScanJob}, it will read the public static field named
     * by {@code confFieldRoot} and cast it to a {@link ConfigNamespace}. This namespace object becomes the root of a
     * {@link Configuration} instantiated, populated with the key-value pairs from the {@code conf} parameter, and then
     * passed into the {@code ScanJob}.
     * <p>
     * This method blocks until the ScanJob completes, then returns the metrics generated by the job during its
     * execution. It does not timeout.
     *
     ...
     * @param inputFormat the InputFormat<StaticBuffer, Iterable<Entry>> that reads (row, columns) pairs out of a
     *            HugeGraph edgestore
     * @return metrics generated by the ScanJob
     * @throws IOException if the job fails for any reason
     * @throws ClassNotFoundException if {@code scanJob.getClass()} or if Hadoop MapReduce's internal
     *             job-submission-related reflection fails
     * @throws InterruptedException if interrupted while waiting for the Hadoop MapReduce job to complete
     */
    public static ScanMetrics runJob(Configuration conf, String confRootField,
            org.apache.hadoop.conf.Configuration hadoopConf, Class<? extends InputFormat> inputFormat, String jobName,
            Class<? extends Mapper> mapperClass) throws IOException, InterruptedException, ClassNotFoundException {
        ...
        if (null != confRootField) {
            ...
            scanConf.set(HugeGraphHadoopConfiguration.SCAN_JOB_CONFIG_ROOT, confRootField);

            // Instantiate scanjob configuration root
            ConfigNamespace confRoot = HadoopScanMapper.getJobRoot(confRootField);

            // Create writable view of scanjob configuration atop the Hadoop Configuration instance, where all keys are
            // prefixed with SCAN_JOB_CONFIG_KEYS
            ModifiableConfiguration hadoopJobConf = ModifiableHadoopConfiguration.prefixView(confRoot,
                    HugeGraphHadoopConfiguration.SCAN_JOB_CONFIG_KEYS, scanConf);

            // Copy scanjob settings from the HugeGraph Configuration instance to the Hadoop Configuration instance
            Map<String, Object> jobConfMap = conf.getSubset(confRoot);
            for (Map.Entry<String, Object> jobConfEntry : jobConfMap.entrySet()) {
                hadoopJobConf.set((ConfigOption) ConfigElement.parse(confRoot, jobConfEntry.getKey()).element,
                        jobConfEntry.getValue());
            }
        }
    ...
    public static ScanMetrics runScanJob(ScanJob scanJob, Configuration conf, String confRootField,
            org.apache.hadoop.conf.Configuration hadoopConf, Class<? extends InputFormat> inputFormat)
            throws IOException, InterruptedException, ClassNotFoundException {
        ...
        scanConf.set(HugeGraphHadoopConfiguration.SCAN_JOB_CLASS, scanJob.getClass().getName());
    ...
    public static ScanMetrics runVertexScanJob(VertexScanJob vertexScanJob, Configuration conf, String confRootField,
        ...
        scanConf.set(HugeGraphHadoopConfiguration.SCAN_JOB_CLASS, vertexScanJob.getClass().getName());
