...
pattern='hugegraph\.compatible.versions'
POM_FILE=pom.xml
...
echo "Current value of hugegraph.compatible.versions:"
echo "  $compat"
...
if [ "$line" != "n" ]; then
    ...
    if echo "$compat" | grep '<hugegraph\.compatible\.versions[ 	]*/>' >/dev/null; then
        compat=$(echo "$compat" | sed -r 's#^(.*hugegraph\.compatible\.versions).+$#\1>'"$ver"'</hugegraph.compatible.versions>#')
    else
        compat=$(echo "$compat" | sed -r 's#^(.+>)(.+)(<.+)$#\1\2,'"$ver"'\3#' )
    fi
    echo "New value of hugegraph.compatible.versions:"
    echo "  $compat"
    require_yes
    echo "Generating $POM_FILE.new with updated hugegraph.compatible.versions..."
    cat "$POM_FILE" | sed -r 's#^.*<hugegraph\.compatible\.versions.*$#'"$compat"'#' > "${POM_FILE}.new"
    ...
    echo "Saved new hugegraph.compatible.versions value to $POM_FILE"
    echo "Committing changes to ${POM_FILE}..."
    git add "${POM_FILE}" 
    git commit -m 'Updating hugegraph.compatible.versions prop' "${POM_FILE}"
fi
...
hugegraph-dist/src/release/rewrite-tag.sh release.properties

# Add the release to the list used in forward-compatibility testing
# during subsequent releases.
hugegraph-dist/src/release/commit-compat-version.sh release.properties
...
mvn_cmd="mvn clean javadoc:jar deploy -Phugegraph-release,s3doc -DskipTests=true"
...
hugegraph-dist/target/release/gh-pages-update.sh release.properties
