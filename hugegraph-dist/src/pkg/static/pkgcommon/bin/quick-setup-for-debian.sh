...
echo "Installing Cassandra, ES, and hugegraph with apt-get"
apt-get update
apt-get install cassandra=2.0.7 elasticsearch=1.0.3 hugegraph

# Reduce Cassandra and Gremlin Server/hugegraph heapsizes
echo 'export MAX_HEAP_SIZE=512M
export HEAP_NEWSIZE=128M' > /etc/default/cassandra

echo 'export MAX_HEAP_SIZE=512M
export HEAP_NEWSIZE=128M' > /etc/default/hugegraph
