...
abstract class GroovySpeedTestSupport {
    ...
    public static final String RELATION_FILE = "../hugegraph-test/data/v10k.graphml.gz"
...
//        Preconditions.checkArgument(TX_COUNT * DEFAULT_OPS_PER_TX <= VERTEX_COUNT);
    ...
    protected abstract StandardHugeGraph getGraph() throws BackendException;
    ...
    protected void chunkedSequentialUidTask(int chunksize = DEFAULT_VERTICES_PER_TX, int verticesPerTx = DEFAULT_VERTICES_PER_TX, closure) {
        ...
        HugeGraphVertex[] vbuf = new HugeGraphVertex[chunksize]
        int vloaded = 0

        while (uids.hasNext()) {
            long u = uids.next()
            hugegraphVertex v = Iterables.getOnlyElement(QueryUtil.getVertices(tx,Schema.UID_PROP, u))
            assertNotNull(v)
            vbuf[vloaded++] = v
...
//            def v = graph.V(Schema.UID_PROP, uid).next()
    ...
    protected void initializeGraph(HugeGraph g) throws BackendException {
