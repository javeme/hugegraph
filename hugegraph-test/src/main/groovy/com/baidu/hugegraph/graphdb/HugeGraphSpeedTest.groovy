...
import com.baidu.hugegraph.testutil.JUnitBenchmarkProvider
...
public abstract class HugeGraphSpeedTest extends GroovySpeedTestSupport {

    private static final Logger log = LoggerFactory.getLogger(HugeGraphSpeedTest)

    @Rule
    public TestRule benchmark = JUnitBenchmarkProvider.get()

    HugeGraphSpeedTest(WriteConfiguration conf) throws BackendException {
        super(conf)
    }
     ...
     * c.t.hugegraph.testutil.gen.{Schema, GraphGenerator}.
     */
    ...
    void testZEdgeAddition() {
        ...
        sequentialUidTask { tx, vertex ->
            if (-1 != last && last != vertex.id()) {
                HugeGraphVertex target = tx.v(last)
...
//                hugegraphKey uidKey = tx.getPropertyKey(Schema.UID_PROP);
...
//    }
    ...
    private void multiVertexQueryTask(HugeGraphTransaction tx, HugeGraphVertex[] vbuf, int vcount) {
        if (vcount != vbuf.length) {
            def newbuf = new HugeGraphVertex[vcount]
        ...
        for (int i = 0; i < schema.edgeLabels; i++) {
            Map<HugeGraphVertex, Iterable<HugeGraphEdge>> m = tx.multiQuery(vbuf).labels(schema.edgeLabelNames[i]).edges()
            for (Iterable<HugeGraphEdge> iter : m.values()) {
                for (HugeGraphEdge e : iter) {
                    n++
                }
            }
        }
    ...
    private void singleVertexQueryTask(HugeGraphTransaction tx, HugeGraphVertex v) {
        int n = 0
        for (int i = 0; i < schema.edgeLabels; i++) {
            for (HugeGraphEdge iter : v.query().labels(schema.edgeLabelNames[i]).edges()) {
                n++
            }
        }
        assertTrue(0 < n)
