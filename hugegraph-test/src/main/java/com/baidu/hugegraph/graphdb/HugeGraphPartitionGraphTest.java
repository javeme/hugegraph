...
import com.baidu.hugegraph.graphdb.database.idassigner.placement.PropertyPlacementStrategy;
...
import com.baidu.hugegraph.util.datastructures.AbstractLongListUtil;
...
public abstract class HugeGraphPartitionGraphTest extends HugeGraphBaseTest {
    ...
    public void testVertexPartitioning() throws Exception {
        ...
        for (int i = 0; i < numG; i++) {
            HugeGraphVertex g = tx.addVertex("group");
        ...
        for (int i = 0; i < numG; i++) {
            HugeGraphVertex g = getV(tx, gids[i]);
        ...
        for (int t = 1; t <= numTx; t++) {
            HugeGraphVertex g1 = getV(tx, gids[0]), g2 = getV(tx, gids[1]);
            assertNotNull(g1);
            HugeGraphVertex[] vs = new HugeGraphVertex[vPerTx];
            ...
            for (int vi = 0; vi < vPerTx; vi++) {
                ...
                for (HugeGraphRelation r : v.query().relations()) {
                    ...
                    if (r instanceof HugeGraphEdge) {
                        HugeGraphVertex o = ((HugeGraphEdge) r).otherVertex(v);
