...
//        } catch (IllegalArgumentException e) {}
        ...
        */
        ...
        v.addEdge("friend", v12);
        v2 = Iterables.<HugeGraphVertex>getOnlyElement(v12.query().direction(Direction.OUT).labels("connect").vertices());
   ...
   /* ==================================================================================
    ...
    /**
     * Test the correct application of {@link com.baidu.hugegraph.graphdb.types.system.ImplicitKey}
     * to vertices, edges, and properties.
     * <p/>
     * Additionally tests RelationIdentifier since this is closely related to ADJACENT and hugegraphID implicit keys.
     */
    @Test
    ...
    public void testThreadBoundTx() {
        ...
        for (HugeGraphVertex v : new HugeGraphVertex[]{v1, v2, v3}) {
            ...
            HugeGraphEdge tmpE = Iterables.<HugeGraphEdge>getOnlyElement(v.query().direction(Direction.OUT).labels("knows").edges());
        ...
        graph.tx().commit();
        e4.property("type", 2);
        HugeGraphEdge ef = Iterables.<HugeGraphEdge>getOnlyElement(v1.query().direction(Direction.OUT).labels("friend").edges());
    ...
    public void testFixedGraphConfig() {
        setIllegalGraphOption(INITIAL_hugegraph_VERSION, ConfigOption.Type.FIXED, "foo");
    }
    ...
    public void testTransactionConfiguration() {
        ...
        StandardHugeGraphTx customTimeTx = (StandardHugeGraphTx) graph.buildTransaction().commitTime(customTimestamp).start();
   ...
   /* ==================================================================================
    ...
    public void testConsistencyEnforcement() {
        ...
        executeLockConflictingTransactionJobs(graph, new TransactionJob() {
            ...
            @Override
            public void run(HugeGraphTransaction tx) {
                HugeGraphVertex u1 = getV(tx, vids[0]),
                        u2 = getV(tx, vids[pos++]);
        ...
        executeLockConflictingTransactionJobs(graph, new TransactionJob() {
            ...
            @Override
            public void run(HugeGraphTransaction tx) {
                HugeGraphVertex u1 = getV(tx, vids[pos++]),
                        u2 = getV(tx, vids[0]);
                u1.addEdge("spouse", u2);
            }
        });
    ...
    public void testConcurrentConsistencyEnforcement() throws Exception {
        PropertyKey name = mgmt.makePropertyKey("name").dataType(String.class).cardinality(Cardinality.SINGLE).make();
        HugeGraphIndex nameIndex = mgmt.buildIndex("name", Vertex.class)
...
//        System.out.println(numA + " - " + numB);
   ...
   /* ==================================================================================
    ...
    public void testVertexCentricQuery(int noVertices) {
        ...
        for (int i = 1; i < noVertices; i++) {
            for (HugeGraphVertex vertex : new HugeGraphVertex[]{v, u}) {
                for (Direction d : new Direction[]{OUT, IN}) {
                    ...
                    HugeGraphEdge e = d == OUT ? vertex.addEdge(n(label), vs[i]) :
        ...
        for (Iterable<HugeGraphEdge> result : results.values()) assertEquals(1, size(result));
        results = tx.multiQuery(Sets.newHashSet(qvs)).labels("connect").edges();
        for (Iterable<HugeGraphEdge> result : results.values()) assertEquals(2, size(result));
        results = tx.multiQuery(qvs).labels("knows").edges();
        for (Iterable<HugeGraphEdge> result : results.values()) assertEquals(0, size(result));
        results = tx.multiQuery(qvs).edges();
        for (Iterable<HugeGraphEdge> result : results.values()) assertEquals(4, size(result));
        results2 = tx.multiQuery(qvs).properties();
        for (Iterable<HugeGraphVertexProperty> result : results2.values()) assertEquals(1, size(result));
...
//        } catch (IllegalArgumentException e) {}
    ...
    public static void evaluateQuery(HugeGraphVertexQuery query, RelationCategory resultType,
    ...
    public void testTinkerPopOptimizationStrategies() {
        ...
        assertNumStep(numV, 2, gts.V(sv[0]).outE("knows").order().by("weight", decr), HugeGraphVertexStep.class, OrderGlobalStep.class);
        assertNumStep(10, 1, gts.V(sv[0]).local(__.outE("knows").order().by("weight", decr).limit(10)), LocalStep.class);
        assertNumStep(numV / 5, 2, gts.V(sv[0]).outE("knows").has("weight", 1).order().by("weight", incr), HugeGraphVertexStep.class, OrderGlobalStep.class);
        ...
        assertNumStep(1, 2, gts.V(sv[0]).bothE("knows").filter(__.inV().is(vs[50])), HugeGraphVertexStep.class, TraversalFilterStep.class);
        ...
        assertNumStep(numV, 2, gts.V(sv[0]).outE("knows").values("weight"), HugeGraphVertexStep.class, HugeGraphPropertiesStep.class);
        ...
        assertNumStep(superV * (numV / 5), 2, gts.V().has("id", sid).outE("knows").has("weight", 1), HugeGraphStep.class, HugeGraphVertexStep.class);
        assertNumStep(superV * (numV / 5 * 2), 2, gts.V().has("id", sid).outE("knows").has("weight", P.gte(1)).has("weight", P.lt(3)), HugeGraphStep.class, HugeGraphVertexStep.class);
        assertNumStep(superV * (numV / 5 * 2), 2, gts.V().has("id", sid).outE("knows").has("weight", P.between(1, 3)), HugeGraphStep.class, HugeGraphVertexStep.class);
        assertNumStep(superV * 10, 2, gts.V().has("id", sid).local(__.outE("knows").has("weight", P.gte(1)).has("weight", P.lt(3)).limit(10)), HugeGraphStep.class, HugeGraphVertexStep.class);
        assertNumStep(superV * 10, 2, gts.V().has("id", sid).local(__.outE("knows").has("weight", P.between(1, 3)).order().by("weight", decr).limit(10)), HugeGraphStep.class, LocalStep.class);
        ...
        assertNumStep(superV * 10, 2, gts.V().has("id", sid).local(__.outE("knows").has("weight", P.gte(1)).has("weight", P.lt(3)).limit(10)), HugeGraphStep.class, HugeGraphVertexStep.class);
        assertNumStep(superV * 10, 2, gts.V().has("id", sid).local(__.outE("knows").has("weight", P.between(1, 3)).order().by("weight", decr).limit(10)), HugeGraphStep.class, LocalStep.class);
        assertNumStep(superV * numV, 2, gts.V().has("id", sid).values("names"), HugeGraphStep.class, HugeGraphPropertiesStep.class);
                ...
                .addProcessor(new ChangeProcessor() {
                    ...
                    public void process(HugeGraphTransaction tx, TransactionId txId, ChangeState changes) {
                        ...
                        if (txTimeMicro.isBefore(txTimes[1])) {
                            ...
                            assertEquals(v1id, getId(v));
                            HugeGraphEdge e = Iterables.getOnlyElement(changes.getEdges(v, Change.REMOVED, Direction.OUT, "knows"));
                            assertFalse(e.property("weight").isPresent());
...
//                    assertTrue(txNo + " - " + v2, v2 == null || v2.isRemoved());
   ...
   /* ==================================================================================
    ...
    public void testGlobalGraphIndexingAndQueriesForInternalIndexes() {
        ...
        HugeGraphIndex edge2 = mgmt.buildIndex("edge2", Edge.class).indexOnly(connect).addKey(text).buildCompositeIndex();

        HugeGraphIndex prop1 = mgmt.buildIndex("prop1", HugeGraphVertexProperty.class).addKey(time).buildCompositeIndex();
        HugeGraphIndex prop2 = mgmt.buildIndex("prop2", HugeGraphVertexProperty.class).addKey(weight).addKey(text).buildCompositeIndex();

        HugeGraphIndex vertex1 = mgmt.buildIndex("vertex1", Vertex.class).addKey(time).indexOnly(person).unique().buildCompositeIndex();
        HugeGraphIndex vertex12 = mgmt.buildIndex("vertex12", Vertex.class).addKey(text).indexOnly(person).buildCompositeIndex();
        HugeGraphIndex vertex2 = mgmt.buildIndex("vertex2", Vertex.class).addKey(time).addKey(name).indexOnly(organization).buildCompositeIndex();
    ...
    public void testIndexUniqueness() {
        ...
        HugeGraphIndex vindex1 = mgmt.buildIndex("vindex1", Vertex.class).addKey(time).indexOnly(person).unique().buildCompositeIndex();
        HugeGraphIndex vindex2 = mgmt.buildIndex("vindex2", Vertex.class).addKey(time).addKey(text).unique().buildCompositeIndex();
        finishSchema();
    ...
    public static void evaluateQuery(HugeGraphQuery query, ElementCategory resultType,
    ...
    public void testIndexQueryWithLabelsAndContainsIN() {
        ...
        HugeGraphIndex uidCompositeIndex = mgmt.buildIndex("uidIndex", Vertex.class).indexOnly(label).addKey(uid).unique().buildCompositeIndex();
        ...
        Iterable<HugeGraphVertex> vertexes = graph.query()
   ...
   /* ==================================================================================
    ...
    public void testPropertyTTLTiming() throws Exception {
        ...
        HugeGraphIndex index2 = mgmt.buildIndex("index2", Vertex.class).addKey(name).addKey(place).buildCompositeIndex();
