...
import com.baidu.hugegraph.core.SchemaViolationException;
import com.baidu.hugegraph.core.HugeGraphConfigurationException;
...
import com.baidu.hugegraph.core.log.TransactionId;
...
import com.baidu.hugegraph.core.util.ManagementUtil;
...
import com.baidu.hugegraph.diskstorage.log.MessageReader;
import com.baidu.hugegraph.diskstorage.log.ReadMarker;
import com.baidu.hugegraph.diskstorage.log.kcvs.KCVSLog;
...
import com.baidu.hugegraph.graphdb.database.management.ManagementSystem;
...
import com.baidu.hugegraph.graphdb.query.profile.SimpleQueryProfiler;
...
import com.baidu.hugegraph.graphdb.schema.EdgeLabelDefinition;
import com.baidu.hugegraph.graphdb.schema.PropertyKeyDefinition;
import com.baidu.hugegraph.graphdb.schema.SchemaContainer;
import com.baidu.hugegraph.graphdb.schema.VertexLabelDefinition;
import com.baidu.hugegraph.graphdb.serializer.SpecialInt;
import com.baidu.hugegraph.graphdb.serializer.SpecialIntSerializer;
import com.baidu.hugegraph.graphdb.tinkerpop.optimize.HugeGraphStep;
import com.baidu.hugegraph.graphdb.tinkerpop.optimize.HugeGraphPropertiesStep;
...
import com.baidu.hugegraph.testcategory.BrittleTests;
import com.baidu.hugegraph.testutil.TestGraphConfigs;
...
import static com.baidu.hugegraph.graphdb.internal.RelationCategory.*;
import static com.baidu.hugegraph.testutil.HugeGraphAssert.*;
...
public abstract class HugeGraphTest extends HugeGraphBaseTest {
    ...
    private Logger log = LoggerFactory.getLogger(HugeGraphTest.class);
    ...
    public void testBasic() {
        ...
        HugeGraphVertex n1 = tx.addVertex();
    ...
    public void testVertexRemoval() {
        ...
        HugeGraphVertex v1 = graph.addVertex(namen, "v1");
        HugeGraphVertex v2 = graph.addVertex(namen, "v2");
        v1.addEdge("knows", v2);
    ...
    public void testGlobalIteration() {
        ...
        HugeGraphVertex previous = tx.addVertex("count", 0);
        for (int i = 1; i < numV; i++) {
            HugeGraphVertex next = tx.addVertex("count", i);
    ...
    public void testMediumCreateRetrieve() {
        ...
        HugeGraphVertex[] nodes = new HugeGraphVertex[noVertices];
        ...
        for (int i = 0; i < noVertices; i++) {
            ...
            for (int k : knowsOff) {
                HugeGraphVertex n2 = nodes[wrapAround(i + k, noVertices)];
                Edge r = n.addEdge("knows", n2, "uid",
                        ((Number) n.value("uid")).intValue() + ((Number) n2.value("uid")).intValue(), "weight", k * 1.5,
                        "name", i + "-" + k);
                nodeEdges[i].add(r);
            }
            if (i % 100 == 99)
        ...
        for (int i = 0; i < noVertices; i++) {
            ...
            for (Object r : nodeEdges[i]) {
                nodeEdgeIds[i].add(((HugeGraphEdge) r).longId());
            }
        }
        clopen();

        nodes = new HugeGraphVertex[noVertices];
        for (int i = 0; i < noVertices; i++) {
            HugeGraphVertex n = getVertex("uid", ids[i]);
        ...
        for (int i = 0; i < noVertices; i++) {
            ...
            for (Object o : n.query().direction(Direction.OUT).labels("knows").edges()) {
                HugeGraphEdge r = (HugeGraphEdge) o;
                HugeGraphVertex n2 = r.vertex(Direction.IN);
            ...
            for (Object r : n.query().direction(Direction.OUT).edges()) {
                edgeIds.add(((HugeGraphEdge) r).longId());
            }
    ...
    private void verifyVerticesRetrieval(long[] vids, List<HugeGraphVertex> vs) {
    ...
    /**
     * Test the definition and inspection of various schema types and ensure their correct interpretation within the
     * graph
     */
    ...
    public void testSchemaTypes() {
        ...
        PropertyKey value = mgmt.makePropertyKey("value").dataType(Double.class).signature(weight)
        ...
        try {
            ...
            ((StandardEdgeLabelMaker) mgmt.makeEdgeLabel("other")).multiplicity(Multiplicity.SIMPLE).sortKey(weight)
                    .make();
            fail();
        } catch (IllegalArgumentException e) {
        }
        try {
            // sort key requires the label to be non-constrained
            ((StandardEdgeLabelMaker) mgmt.makeEdgeLabel("other")).multiplicity(Multiplicity.MANY2ONE).sortKey(weight)
                    .make();
            fail();
        } catch (IllegalArgumentException e) {
        }
        ...
        // test edges
        HugeGraphVertex v12 = tx.addVertex("person"), v13 = tx.addVertex("person");
        ...
        v.addEdge("link", v13);
        HugeGraphVertex v2 = tx.addVertex("tweet");
        v2.addEdge("link", v13);
        ...
        for (Object prop : v.query().labels("name").properties()) {
            String nstr = ((HugeGraphVertexProperty<String>) prop).value();
        ...
        for (Object o : v.query().labels("value").properties()) {
            HugeGraphVertexProperty<Double> prop = (HugeGraphVertexProperty<Double>) o;
        ...
        edge = Iterables.<HugeGraphEdge> getOnlyElement(v.query().direction(Direction.BOTH).labels("connect").edges());
        ...
        v.addEdge("friend", v12);
        v2 = Iterables
                .<HugeGraphVertex> getOnlyElement(v12.query().direction(Direction.OUT).labels("connect").vertices());
        ...
        for (Object o : v.query().labels("name").properties()) {
            HugeGraphVertexProperty<String> prop = (HugeGraphVertexProperty<String>) o;
        ...
        try {
            HugeGraphVertex vx = tx2.addVertex();
            try {
                // property is unique
            ...
            HugeGraphVertex vx2 = tx2.addVertex();
            try {
                // property unique
    ...
    /**
     * Test the different data types that HugeGraph supports natively and ensure that invalid data types aren't allowed
     */
    @Test
    public void testDataTypes() throws Exception {
        ...
        for (Object prop : v.query().labels("any").properties()) {
            Object value = ((HugeGraphVertexProperty<?>) prop).value();
    ...
    /**
     * This tests a special scenario under which a schema type is defined in a (management) transaction and then
     * accessed in a concurrent transaction. Also ensures that unique property values are enforced within and across
     * transactions
     */
    @Test
    ...
    public void testSchemaNameChange() {
        ...
        // CREATE SMALL GRAPH
        HugeGraphVertex v = tx.addVertex("people");
        v.property(VertexProperty.Cardinality.single, "time", 5);
        v.addEdge("knows", v, "time", 11);

        newTx();
        v = Iterables.<HugeGraphVertex> getOnlyElement(tx.query().has("time", 5).vertices());
        assertNotNull(v);
        ...
        HugeGraphIndex gindex = mgmt.getGraphIndex("timeIndex");
    ...
    public void testGotGIndexRemoval() throws InterruptedException, ExecutionException {
        clopen(option(LOG_SEND_DELAY, MANAGEMENT_LOG), Duration.ZERO, option(KCVSLog.LOG_READ_LAG_TIME, MANAGEMENT_LOG),
                Duration.ofMillis(50), option(LOG_READ_INTERVAL, MANAGEMENT_LOG), Duration.ofMillis(250));

        final String name = "name";

        // Load Graph of the Gods
        GraphOfTheGodsFactory.loadWithoutMixedIndex(graph, true); // True makes the index on names unique. Test fails
                                                                  // when this is true.
        // Change to false and test will pass.
        newTx();
        finishSchema();

        HugeGraphIndex gindex = mgmt.getGraphIndex(name);
        ...
        HugeGraphManagement.IndexJobFuture gmetrics = mgmt.getIndexJobStatus(gindex);
        finishSchema();
    ...
    public void testIndexUpdatesWithReindexAndRemove() throws InterruptedException, ExecutionException {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            v.property("name", "v" + i);
            HugeGraphVertex o = tx.addVertex();
            v.addEdge("friend", o, "time", i);
        }
        newTx();
        // Indexes should not yet be in use
        v = getV(tx, v);
        evaluateQuery(v.query().keys("sensor").interval("time", 1, 5).orderBy("time", decr), PROPERTY, 4, 1,
                new boolean[] { false, false }, tx.getPropertyKey("time"), Order.DESC);
        evaluateQuery(v.query().keys("sensor").interval("time", 101, 105).orderBy("time", decr), PROPERTY, 0, 1,
                new boolean[] { false, false }, tx.getPropertyKey("time"), Order.DESC);
        evaluateQuery(v.query().labels("friend").direction(OUT).interval("time", 1, 5).orderBy("time", decr), EDGE, 4,
                1, new boolean[] { false, false }, tx.getPropertyKey("time"), Order.DESC);
        evaluateQuery(v.query().labels("friend").direction(OUT).interval("time", 101, 105).orderBy("time", decr), EDGE,
                0, 1, new boolean[] { false, false }, tx.getPropertyKey("time"), Order.DESC);
        ...
        v = getV(tx, v);
        evaluateQuery(v.query().keys("sensor").interval("time", 1, 5).orderBy("time", decr), PROPERTY, 4, 1,
                new boolean[] { true, true }, tx.getPropertyKey("time"), Order.DESC);
        evaluateQuery(v.query().keys("sensor").interval("time", 101, 105).orderBy("time", decr), PROPERTY, 4, 1,
                new boolean[] { true, true }, tx.getPropertyKey("time"), Order.DESC);
        evaluateQuery(v.query().keys("sensor").interval("time", 201, 205).orderBy("time", decr), PROPERTY, 4, 1,
                new boolean[] { true, true }, tx.getPropertyKey("time"), Order.DESC);
        evaluateQuery(v.query().labels("friend").direction(OUT).interval("time", 1, 5).orderBy("time", decr), EDGE, 0,
                1, new boolean[] { true, true }, tx.getPropertyKey("time"), Order.DESC);
        evaluateQuery(v.query().labels("friend").direction(OUT).interval("time", 101, 105).orderBy("time", decr), EDGE,
                4, 1, new boolean[] { true, true }, tx.getPropertyKey("time"), Order.DESC);
        evaluateQuery(v.query().labels("friend").direction(OUT).interval("time", 201, 205).orderBy("time", decr), EDGE,
                4, 1, new boolean[] { true, true }, tx.getPropertyKey("time"), Order.DESC);
        evaluateQuery(tx.query().has("name", "v5"), ElementCategory.VERTEX, 1, new boolean[] { true, true },
                "bySensorReading");
        evaluateQuery(tx.query().has("name", "v105"), ElementCategory.VERTEX, 1, new boolean[] { true, true },
                "bySensorReading");
        evaluateQuery(tx.query().has("name", "v205"), ElementCategory.VERTEX, 1, new boolean[] { true, true },
                "bySensorReading");
    ...
    public void testIndexUpdateSyncWithMultipleInstances() throws InterruptedException {
        ...
        StandardHugeGraph graph2 = (StandardHugeGraph) HugeGraphFactory.open(config);
        HugeGraphTransaction tx2;
        ...
        HugeGraphTransaction tx3 = graph2.newTransaction();
        ...
        assertTrue(ManagementSystem.awaitGraphIndexStatus(graph, "theIndex").status(SchemaStatus.REGISTERED)
                .timeout(TestGraphConfigs.getSchemaConvergenceTime(ChronoUnit.SECONDS), ChronoUnit.SECONDS).call()
                .getSucceeded());
        finishSchema();
        mgmt.updateIndex(mgmt.getGraphIndex("theIndex"), SchemaAction.ENABLE_INDEX);
        finishSchema();
        ...
        newTx();
        evaluateQuery(tx.query().has("name", "v1"), ElementCategory.VERTEX, 0, new boolean[] { true, true },
                "theIndex");
        evaluateQuery(tx.query().has("name", "v2"), ElementCategory.VERTEX, 1, new boolean[] { true, true },
                "theIndex");
        evaluateQuery(tx.query().has("name", "v3"), ElementCategory.VERTEX, 1, new boolean[] { true, true },
                "theIndex");
        evaluateQuery(tx.query().has("name", "v4"), ElementCategory.VERTEX, 1, new boolean[] { true, true },
                "theIndex");

        Thread.sleep(2000);
        tx2 = graph2.newTransaction();
        evaluateQuery(tx2.query().has("name", "v1"), ElementCategory.VERTEX, 0, new boolean[] { true, true },
                "theIndex");
        evaluateQuery(tx2.query().has("name", "v2"), ElementCategory.VERTEX, 1, new boolean[] { true, true },
                "theIndex");
        evaluateQuery(tx2.query().has("name", "v3"), ElementCategory.VERTEX, 1, new boolean[] { true, true },
                "theIndex");
        evaluateQuery(tx2.query().has("name", "v4"), ElementCategory.VERTEX, 1, new boolean[] { true, true },
                "theIndex");
        tx2.commit();
    ...
    /**
     * This test exercises different types of updates against cardinality restricted properties to ensure that the
     * resulting behavior is fully consistent.
     */
    @Test
    ...
    /**
     * Test the correct application of {@link com.baidu.hugegraph.graphdb.types.system.ImplicitKey} to vertices, edges,
     * and properties.
     * <p/>
    ...
    public void testImplicitKey() {
        HugeGraphVertex v = graph.addVertex("name", "Dan"), u = graph.addVertex();
        ...
        assertCount(0, v.query().direction(Direction.BOTH).labels("knows")
                .has(ID_NAME, RelationIdentifier.get(new long[] { 4, 5, 6, 7 })).edges());
    ...
    public void testArrayEqualityUsingImplicitKey() {
        HugeGraphVertex v = graph.addVertex();
    ...
    /**
     * Tests that elements can be accessed beyond their transactional boundaries if they are bound to single-threaded
     * graph transactions
     */
    @Test
    ...
    public void testThreadBoundTx() {
        ...
        HugeGraphVertex v1 = graph.addVertex("name", "Vertex1", "age", 35);
        HugeGraphVertex v2 = graph.addVertex("name", "Vertex2", "age", 45);
        HugeGraphVertex v3 = graph.addVertex("name", "Vertex3", "age", 55);
        ...
        for (HugeGraphVertex v : new HugeGraphVertex[] { v1, v2, v3 }) {
            ...
            HugeGraphEdge tmpE = Iterables
                    .<HugeGraphEdge> getOnlyElement(v.query().direction(Direction.OUT).labels("knows").edges());
        ...
        e4.property("type", 2);
        HugeGraphEdge ef =
                Iterables.<HugeGraphEdge> getOnlyElement(v1.query().direction(Direction.OUT).labels("friend").edges());
        ...
        for (HugeGraphVertex v : new HugeGraphVertex[] { v1, v2, v3 }) {
            assertCount(2, v.query().direction(Direction.BOTH).labels("knows").edges());
            assertCount(1, v.query().direction(Direction.OUT).labels("knows").edges());
            assertEquals(5, ((Edge) getOnlyElement(v.query().direction(Direction.OUT).labels("knows").edges()))
                    .<Integer> value("time").intValue() % 10);
        }
        ...
        prop = (VertexProperty) ((Iterable) graph.multiQuery(v1).properties().values().iterator().next()).iterator()
                .next();
        assertTrue(getId(prop) > 0);
    ...
    public void testStaleVertex() {
        ...
        HugeGraphVertex cartman = graph.addVertex("name", "cartman", "age", 10);
        graph.addVertex("name", "stan", "age", 8);

        graph.tx().commit();

        cartman = Iterables.<HugeGraphVertex> getOnlyElement(graph.query().has("name", "cartman").vertices());

        graph.tx().commit();

        HugeGraphVertexProperty p = (HugeGraphVertexProperty) cartman.properties().next();
    ...
    public void testTransactionIsolation() {
        ...
        HugeGraphVertex v11 = tx1.addVertex();
        HugeGraphVertex v12 = tx1.addVertex();
        v11.addEdge("knows", v12);

        HugeGraphVertex v21 = tx2.addVertex();
        try {
            v21.addEdge("knows", v11);
            fail();
        } catch (IllegalStateException e) {
        }
        HugeGraphVertex v22 = tx2.addVertex();
        ...
        HugeGraphVertex v3 = tx.addVertex();
        v21.addEdge("link", v3);
        newTx();
        v21 = getV(tx, v21);
        v3 = Iterables.<HugeGraphVertex> getOnlyElement(v21.query().direction(Direction.OUT).labels("link").vertices());
        ...
        HugeGraphTransaction tx3 = graph.buildTransaction().checkInternalVertexExistence(true).start();
        v21 = getV(tx3, v21);
        v3 = Iterables.<HugeGraphVertex> getOnlyElement(v21.query().direction(Direction.OUT).labels("link").vertices());
    ...
    /**
     * Tests multi-valued properties with special focus on indexing and incident unidirectional edges which is not
     * tested in {@link #testSchemaTypes()}
     * <p/>
    ...
    public <V> void testMultivaluedVertexProperty() {
         ...
         * Constant test data
         *
         * The values list below must have at least two elements. The string literals were chosen arbitrarily and have
        ...
        PropertyKey f = ((StandardPropertyKeyMaker) mgmt.makePropertyKey(foo)).dataType(String.class)
                .cardinality(Cardinality.LIST).sortKey(w).sortOrder(Order.DESC).make();
        ...
        for (String prop : new String[] { foo, bar }) {
            ...
            for (Object o : v.query().labels(foo).properties()) {
                HugeGraphVertexProperty<String> p = (HugeGraphVertexProperty<String>) o;
    ...
    public void testManagedOptionMasking() throws BackendException {
        ...
        } catch (HugeGraphConfigurationException e) {
    ...
    public void testTransactionConfiguration() {
        ...
        HugeGraphTransaction readOnlyTx = graph.buildTransaction().readOnly().start();
        try {
        ...
        String logID = "spam";
        StandardHugeGraphTx customLogIDTx = (StandardHugeGraphTx) graph.buildTransaction().logIdentifier(logID).start();
        ...
        StandardHugeGraphTx customTimeTx =
                (StandardHugeGraphTx) graph.buildTransaction().commitTime(customTimestamp).start();
    ...
    private <T> void setAndCheckGraphOption(ConfigOption<T> opt, ConfigOption.Type requiredType, T firstValue,
            T secondValue) {
        ...
        final EnumSet<ConfigOption.Type> allowedTypes =
                EnumSet.of(ConfigOption.Type.GLOBAL, ConfigOption.Type.GLOBAL_OFFLINE, ConfigOption.Type.MASKABLE);
        ...
        HugeGraph g2 = HugeGraphFactory.open(config);
        HugeGraphManagement m2 = g2.openManagement();
        ...
        if (opt.getType().equals(ConfigOption.Type.GLOBAL_OFFLINE)) {
            try {
                mgmt.set(path, firstValue);
                mgmt.commit();
                fail("Option " + path + " with type " + ConfigOption.Type.GLOBAL_OFFLINE
                        + " should not be modifiable with concurrent instances");
    ...
    private <T> void setIllegalGraphOption(ConfigOption<T> opt, ConfigOption.Type requiredType, T attemptedValue) {
        ...
        Preconditions.checkArgument(
                requiredType.equals(ConfigOption.Type.LOCAL) || requiredType.equals(ConfigOption.Type.FIXED));
    ...
    public void testConsistencyEnforcement() {
        ...
        HugeGraphVertex v1 = tx.addVertex("uid", 1);
        HugeGraphVertex v2 = tx.addVertex("uid", 2);
        HugeGraphVertex v3 = tx.addVertex("uid", 3);
        ...
        long e1id = getId(e1);
        long e2id = getId(e2);
        e1 = Iterables.<HugeGraphEdge> getOnlyElement(v1.query().direction(Direction.OUT).labels("connect").edges());
        assertEquals("e1", e1.value("name"));
        assertEquals(e1id, getId(e1));
        e2 = Iterables.<HugeGraphEdge> getOnlyElement(v1.query().direction(Direction.OUT).labels("related").edges());
        ...
        executeLockConflictingTransactionJobs(graph, new TransactionJob() {
            ...
            @Override
            public void run(HugeGraphTransaction tx) {
                HugeGraphVertex u = getV(tx, vids[pos++]);
        ...
        executeLockConflictingTransactionJobs(graph, new TransactionJob() {
            @Override
            public void run(HugeGraphTransaction tx) {
                HugeGraphVertex u = getV(tx, vids[0]);
        ...
        executeLockConflictingTransactionJobs(graph, new TransactionJob() {
            @Override
            public void run(HugeGraphTransaction tx) {
                HugeGraphVertex u1 = getV(tx, vids[0]), u2 = getV(tx, vids[1]);
                u1.addEdge("knows", u2);
            }
        });
        ...
        executeLockConflictingTransactionJobs(graph, new TransactionJob() {
            ...
            @Override
            public void run(HugeGraphTransaction tx) {
                HugeGraphVertex u1 = getV(tx, vids[0]), u2 = getV(tx, vids[pos++]);
        ...
        executeLockConflictingTransactionJobs(graph, new TransactionJob() {
            ...
            @Override
            public void run(HugeGraphTransaction tx) {
                HugeGraphVertex u1 = getV(tx, vids[pos++]), u2 = getV(tx, vids[0]);
                u1.addEdge("spouse", u2);
            }
        });
    ...
    /**
     * Executes a transaction job in two parallel transactions under the assumptions that the two transactions should
     * conflict and the one committed later should throw a locking exception.
     *
     * @param graph
     * @param job
     */
    private void executeLockConflictingTransactionJobs(HugeGraph graph, TransactionJob job) {
        HugeGraphTransaction tx1 = graph.newTransaction();
        HugeGraphTransaction tx2 = graph.newTransaction();
        job.run(tx1);
        job.run(tx2);
        /*
         * Under pessimistic locking, tx1 should abort and tx2 should commit. Under optimistic locking, tx1 may commit
         * and tx2 may abort.
         */
    ...
    public void testConcurrentConsistencyEnforcement() throws Exception {
        PropertyKey name = mgmt.makePropertyKey("name").dataType(String.class).cardinality(Cardinality.SINGLE).make();
        HugeGraphIndex nameIndex = mgmt.buildIndex("name", Vertex.class).addKey(name).unique().buildCompositeIndex();
        ...
        HugeGraphVertex baseV = tx.addVertex("name", "base");
        newTx();
        ...
        int numSuccess = executeParallelTransactions(new TransactionJob() {
            @Override
            public void run(HugeGraphTransaction tx) {
                HugeGraphVertex a = tx.addVertex();
                HugeGraphVertex base = getV(tx, baseVid);
        ...
        numSuccess = executeParallelTransactions(new TransactionJob() {
            ...
            public void run(HugeGraphTransaction tx) {
                ...
                HugeGraphVertex b = tx.addVertex("name", nameB);
                b.addEdge("friend", b);
            }
        }, parallelThreads);
    ...
    public void testVertexCentricQuery(int noVertices) {
        ...
        ((StandardEdgeLabelMaker) mgmt.makeEdgeLabel("friend")).sortKey(weight, time).sortOrder(Order.ASC)
                .signature(number).make();
        ((StandardEdgeLabelMaker) mgmt.makeEdgeLabel("friendDesc")).sortKey(weight, time).sortOrder(Order.DESC)
                .signature(number).make();
        ((StandardEdgeLabelMaker) mgmt.makeEdgeLabel("knows")).sortKey(number, weight).make();
        mgmt.makeEdgeLabel("follows").make();
        finishSchema();

        HugeGraphVertex v = tx.addVertex("name", "v");
        HugeGraphVertex u = tx.addVertex("name", "u");
        assertEquals(0, (noVertices - 1) % 3);
        HugeGraphVertex[] vs = new HugeGraphVertex[noVertices];
        for (int i = 1; i < noVertices; i++) {
            vs[i] = tx.addVertex("name", "v" + i);
        }
        EdgeLabel[] labelsV = { tx.getEdgeLabel("connect"), tx.getEdgeLabel("friend"), tx.getEdgeLabel("knows") };
        EdgeLabel[] labelsU =
                { tx.getEdgeLabel("connectDesc"), tx.getEdgeLabel("friendDesc"), tx.getEdgeLabel("knows") };
        for (int i = 1; i < noVertices; i++) {
            for (HugeGraphVertex vertex : new HugeGraphVertex[] { v, u }) {
                for (Direction d : new Direction[] { OUT, IN }) {
                    EdgeLabel label = vertex == v ? labelsV[i % 3] : labelsU[i % 3];
                    HugeGraphEdge e = d == OUT ? vertex.addEdge(n(label), vs[i]) : vs[i].addEdge(n(label), vertex);
                    e.property("time", i);
        ...
        VertexList vl;
        Map<HugeGraphVertex, Iterable<HugeGraphEdge>> results;
        Map<HugeGraphVertex, Iterable<HugeGraphVertexProperty>> results2;
        HugeGraphVertex[] qvs;
        int lastTime;
        ...
        for (int i = 0; i < vidsubset.length; i++)
            vidsubset[i] = vs[i + 3].longId();
        ...
        clopen();
        for (int i = 1; i < noVertices; i++)
            vs[i] = getV(tx, vs[i].longId());
        v = getV(tx, v.longId());
        u = getV(tx, u.longId());
        qvs = new HugeGraphVertex[] { vs[6], vs[9], vs[12], vs[15], vs[60] };
        ...
        lastTime = 0;
        for (HugeGraphEdge e : (Iterable<HugeGraphEdge>) v.query().labels("connect").direction(OUT).limit(20).edges()) {
            int nowTime = e.value("time");
            assertTrue(lastTime + " vs. " + nowTime, lastTime <= nowTime);
            lastTime = nowTime;
        }
        lastTime = Integer.MAX_VALUE;
        for (Edge e : (Iterable<HugeGraphEdge>) u.query().labels("connectDesc").direction(OUT).limit(20).edges()) {
            int nowTime = e.value("time");
            assertTrue(lastTime + " vs. " + nowTime, lastTime >= nowTime);
            lastTime = nowTime;
        }
        assertEquals(10, size(
                v.query().labels("connect").direction(OUT).has("time", Cmp.GREATER_THAN, 60).limit(10).vertices()));
        assertEquals(10, size(
                u.query().labels("connectDesc").direction(OUT).has("time", Cmp.GREATER_THAN, 60).limit(10).vertices()));

        outer = v.query().labels("connect").direction(OUT).limit(20).edges().iterator();
        for (Edge e : (Iterable<HugeGraphEdge>) v.query().labels("connect").direction(OUT).limit(10).edges()) {
            assertEquals(e, outer.next());
        }

        evaluateQuery(v.query().labels("connect").direction(OUT).interval("time", 3, 31), EDGE, 10, 1,
                new boolean[] { true, true });
        evaluateQuery(v.query().labels("connect").direction(OUT).has("time", 15).has("weight", 3.5), EDGE, 1, 1,
                new boolean[] { false, true });
        evaluateQuery(u.query().labels("connectDesc").direction(OUT).interval("time", 3, 31), EDGE, 10, 1,
                new boolean[] { true, true });
        ...
        assertEquals(edgesPerLabel - 10,
                v.query().labels("connect").direction(OUT).has("time", Cmp.GREATER_THAN, 31).count());
        ...
        evaluateQuery(v.query().labels("friend").direction(OUT).has("weight", 0.5).limit(3), EDGE, 3, 1,
                new boolean[] { true, true });
        evaluateQuery(v.query().labels("friend").direction(OUT).interval("time", 3, 33).has("weight", 0.5), EDGE, 3, 1,
                new boolean[] { true, true });
        evaluateQuery(v.query().labels("friend").direction(OUT).interval("time", 3, 33).has("weight", Contain.IN,
                ImmutableList.of(0.5)), EDGE, 3, 1, new boolean[] { true, true });
        evaluateQuery(v.query().labels("friend").direction(OUT)
                .has("weight", Contain.IN, ImmutableList.of(0.5, 1.5, 2.5)).interval("time", 3, 33), EDGE, 7, 3,
                new boolean[] { true, true });
        evaluateQuery(v.query().labels("friend").direction(OUT).has("weight", Contain.IN, ImmutableList.of(0.5, 1.5)),
                EDGE, 1667, 2, new boolean[] { true, true });
        assertEquals(3,
                u.query().labels("friendDesc").direction(OUT).interval("time", 3, 33).has("weight", 0.5).edgeCount());
        assertEquals(1,
                v.query().labels("friend").direction(OUT).has("weight", 0.5).interval("time", 4, 10).edgeCount());
        assertEquals(1,
                u.query().labels("friendDesc").direction(OUT).has("weight", 0.5).interval("time", 4, 10).edgeCount());
        assertEquals(3,
                v.query().labels("friend").direction(OUT).interval("time", 3, 33).has("weight", 0.5).edgeCount());
        assertEquals(4, v.query().labels("friend").direction(OUT).has("time", Cmp.LESS_THAN_EQUAL, 10).edgeCount());
        assertEquals(edgesPerLabel - 4,
                v.query().labels("friend").direction(OUT).has("time", Cmp.GREATER_THAN, 10).edgeCount());
        assertEquals(20, v.query().labels("friend", "connect").direction(OUT).interval("time", 3, 33).edgeCount());

        assertEquals((int) Math.ceil(edgesPerLabel / 5.0),
                v.query().labels("knows").direction(OUT).has("number", 0).edgeCount());
        assertEquals((int) Math.ceil(edgesPerLabel / 5.0),
                v.query().labels("knows").direction(OUT).has("number", 0).interval("weight", 0.0, 4.0).edgeCount());
        assertEquals((int) Math.ceil(edgesPerLabel / (5.0 * 2)),
                v.query().labels("knows").direction(OUT).has("number", 0).interval("weight", 0.0, 2.0).edgeCount());
        assertEquals((int) Math.floor(edgesPerLabel / (5.0 * 2)),
                v.query().labels("knows").direction(OUT).has("number", 0).interval("weight", 2.1, 4.0).edgeCount());
        assertEquals(20, size(v.query().labels("connect", "friend").direction(OUT).interval("time", 3, 33).vertices()));
        assertEquals(20,
                size(v.query().labels("connect", "friend").direction(OUT).interval("time", 3, 33).vertexIds()));
        assertEquals(30,
                v.query().labels("friend", "connect", "knows").direction(OUT).interval("time", 3, 33).edgeCount());
        assertEquals(noVertices - 2, v.query().labels("friend", "connect", "knows").direction(OUT)
                .has("time", Cmp.NOT_EQUAL, 10).edgeCount());
        ...
        assertEquals(2 * (int) Math.ceil((noVertices - 1) / 4.0),
                size(v.query().labels("connect", "friend", "knows").has("weight", 1.5).vertexIds()));
        ...
        for (Direction dir : new Direction[] { IN, OUT }) {
            ...
            for (int i = 0; i < vl.size(); i++)
                assertEquals(vidsubset[i], vl.getID(i));
        }
        assertCount(2 * (noVertices - 1), v.query().direction(Direction.BOTH).edges());
        ...
        for (Iterable<HugeGraphEdge> result : results.values())
        ...
        for (Iterable<HugeGraphEdge> result : results.values())
            assertEquals(0, size(result));
        results = tx.multiQuery(qvs).edges();
        for (Iterable<HugeGraphEdge> result : results.values())
            assertEquals(4, size(result));
        results2 = tx.multiQuery(qvs).properties();
        for (Iterable<HugeGraphVertexProperty> result : results2.values())
            assertEquals(1, size(result));
        ...
        lastTime = 0;
        for (Edge e : (Iterable<HugeGraphEdge>) v.query().labels("connect").direction(OUT).limit(20).edges()) {
            int nowTime = e.value("time");
            assertTrue(lastTime + " vs. " + nowTime, lastTime <= nowTime);
            lastTime = nowTime;
        }
        ...
        v = Iterables.<HugeGraphVertex> getOnlyElement(tx.query().has("name", "v").vertices());
        assertNotNull(v);
        ...
        HugeGraphVertex[] qvs2 = new HugeGraphVertex[qvs.length + 2];
        qvs2[0] = tx.addVertex();
        for (int i = 0; i < qvs.length; i++)
            qvs2[i + 1] = getV(tx, qvs[i].longId());
    ...
    public void testRelationTypeIndexes() {
        ...
        final int numV = 100;
        HugeGraphVertex v = tx.addVertex();
        HugeGraphVertex ns[] = new HugeGraphVertex[numV];
        ...
        evaluateQuery(v.query().keys("name").has("weight", Cmp.GREATER_THAN, 3.6), PROPERTY, 2 * numV / 10, 1,
                new boolean[] { false, true });
        evaluateQuery(v.query().keys("name").has("weight", Cmp.LESS_THAN, 0.9).orderBy("weight", incr), PROPERTY,
                2 * numV / 10, 1, new boolean[] { true, true }, weight, Order.ASC);
        ...
        evaluateQuery(v.query().keys("name").has("time", Cmp.EQUAL, 5).orderBy("weight", decr), PROPERTY, 1, 1,
                new boolean[] { false, false }, weight, Order.DESC);
        evaluateQuery(v.query().keys("name"), PROPERTY, numV, 1, new boolean[] { true, true });

        evaluateQuery(v.query().labels("child").direction(OUT).has("time", Cmp.EQUAL, 5), EDGE, 1, 1,
                new boolean[] { true, true });
        evaluateQuery(v.query().labels("child").direction(BOTH).has("time", Cmp.EQUAL, 5), EDGE, 1, 2, new boolean[0]);
        evaluateQuery(
                v.query().labels("child").direction(OUT).interval("time", 10, 20).orderBy("weight", decr).limit(5),
        ...
        evaluateQuery(v.query().labels("child").direction(OUT).interval("weight", 0.0, 1.0), EDGE, 2 * numV / 10, 1,
                new boolean[] { false, true });
        ...
        evaluateQuery(v.query().labels("child").interval("weight", 0.0, 1.0).direction(OUT), EDGE, 2 * numV / 10, 1,
                new boolean[] { false, true });
        vl = v.query().labels("child").interval("weight", 0.0, 1.0).direction(OUT).vertexIds();
        assertEquals(2 * numV / 10, vl.size());
        assertTrue(vl.isSorted());
        assertTrue(isSortedByID(vl));
        evaluateQuery(v.query().labels("child").interval("time", 70, 80).direction(OUT).orderBy("time", incr), EDGE, 10,
                1, new boolean[] { true, true }, time, Order.ASC);
        ...
        evaluateQuery(
                v.query().labels("connect").has("time", Cmp.EQUAL, 5).interval("weight", 0.0, 5.0).direction(BOTH),
        ...
        evaluateQuery(v.query().labels("connect").direction(OUT).orderBy("weight", incr).limit(10), EDGE, 10, 1,
                new boolean[] { true, true }, weight, Order.ASC);
        evaluateQuery(v.query().labels("connect").direction(OUT).orderBy("weight", decr).limit(10), EDGE, 10, 1,
                new boolean[] { true, true }, weight, Order.DESC);
        ...
        evaluateQuery(v.query().labels("connect").direction(OUT).has("weight", Cmp.EQUAL, 0.0).adjacent(u), EDGE, 1, 1,
                new boolean[] { true, true });
        evaluateQuery(v.query().labels("connect").direction(OUT).interval("weight", 0.0, 1.0).adjacent(u), EDGE, 1, 1,
                new boolean[] { false, true });
        evaluateQuery(v.query().labels("connect").direction(OUT).interval("time", 50, 100).adjacent(u), EDGE, 1, 1,
                new boolean[] { false, true });
        ...
        for (Object o : v.query().direction(BOTH).edges()) {
            HugeGraphEdge e = (HugeGraphEdge) o;
            if (e.<Long> value("time") < (numV / 2))
                e.remove();
        }
        ns = new HugeGraphVertex[numV * 3 / 2];
        for (int i = numV; i < numV * 3 / 2; i++) {
            ...
            ns[i] = tx.addVertex();
            for (String label : new String[] { "connect", "child", "link" }) {
                HugeGraphEdge e = v.addEdge(label, ns[i], "weight", w, "time", t);
            }
        }
        ...
        evaluateQuery(v.query().keys("name").has("weight", Cmp.GREATER_THAN, 3.6), PROPERTY, 2 * numV / 10, 1,
                new boolean[] { false, true });
        evaluateQuery(v.query().keys("name").interval("time", numV / 2 - 10, numV / 2 + 10), PROPERTY, 10, 1,
                new boolean[] { false, true });
        ...
        evaluateQuery(v.query().keys("name").interval("time", numV, numV + 10).limit(5), PROPERTY, 5, 1,
                new boolean[] { false, true });

        evaluateQuery(v.query().labels("child").direction(OUT).has("time", Cmp.EQUAL, 5), EDGE, 0, 1,
                new boolean[] { true, true });
        evaluateQuery(v.query().labels("child").direction(OUT).has("time", Cmp.EQUAL, numV + 5), EDGE, 1, 1,
                new boolean[] { true, true });
        evaluateQuery(
                v.query().labels("child").direction(OUT).interval("time", 10, 20).orderBy("weight", decr).limit(5),
                EDGE, 0, 1, new boolean[] { true, false }, weight, Order.DESC);
        evaluateQuery(v.query().labels("child").direction(OUT).interval("time", numV + 10, numV + 20)
                .orderBy("weight", decr).limit(5), EDGE, 5, 1, new boolean[] { true, false }, weight, Order.DESC);
    ...
    private boolean isSortedByID(VertexList vl) {
        for (int i = 1; i < vl.size(); i++) {
            if (vl.getID(i - 1) > vl.getID(i))
                return false;
        }
        return true;
    }

    public static void evaluateQuery(HugeGraphVertexQuery query, RelationCategory resultType, int expectedResults,
            int numSubQueries, boolean[] subQuerySpecs) {
        evaluateQuery(query, resultType, expectedResults, numSubQueries, subQuerySpecs,
                ImmutableMap.<PropertyKey, Order> of());
    }

    public static void evaluateQuery(HugeGraphVertexQuery query, RelationCategory resultType, int expectedResults,
            int numSubQueries, boolean[] subQuerySpecs, PropertyKey orderKey, Order order) {
        evaluateQuery(query, resultType, expectedResults, numSubQueries, subQuerySpecs,
                ImmutableMap.of(orderKey, order));
    }

    public static void evaluateQuery(HugeGraphVertexQuery query, RelationCategory resultType, int expectedResults,
            int numSubQueries, boolean[] subQuerySpecs, Map<PropertyKey, Order> orderMap) {
        SimpleQueryProfiler profiler = new SimpleQueryProfiler();
        ((BasicVertexCentricQueryBuilder) query).profiler(profiler);

        Iterable<? extends HugeGraphElement> result;
        switch (resultType) {
            case PROPERTY:
        ...
        int no = 0;
        HugeGraphElement previous = null;
        for (HugeGraphElement e : result) {
            assertNotNull(e);
            no++;
        ...
        for (PropertyKey key : orderMap.keySet())
            assertTrue(orders.containsKey(key));
    ...
    public void testEdgesExceedCacheSize() {
        ...
        HugeGraphVertex parentVertex = graph.addVertex();
        for (int i = 0; i < numEdges; i++) {
            HugeGraphVertex childVertex = graph.addVertex();
    ...
    public void testTinkerPopOptimizationStrategies() {
        ...
        int numV = 100;
        HugeGraphVertex[] vs = new HugeGraphVertex[numV];
        for (int i = 0; i < numV; i++) {
            vs[i] = graph.addVertex("id", i, "weight", i % 5);
        }
        int superV = 10;
        int sid = -1;
        HugeGraphVertex[] sv = new HugeGraphVertex[superV];
        ...
        // Edge
        assertNumStep(numV / 5, 1, gts.V(sv[0]).outE("knows").has("weight", 1), HugeGraphVertexStep.class);
        assertNumStep(numV, 1, gts.V(sv[0]).outE("knows"), HugeGraphVertexStep.class);
        assertNumStep(numV, 1, gts.V(sv[0]).out("knows"), HugeGraphVertexStep.class);
        assertNumStep(10, 1, gts.V(sv[0]).local(__.outE("knows").limit(10)), HugeGraphVertexStep.class);
        assertNumStep(10, 1, gts.V(sv[0]).local(__.outE("knows").range(10, 20)), LocalStep.class);
        assertNumStep(numV, 2, gts.V(sv[0]).outE("knows").order().by("weight", decr), HugeGraphVertexStep.class,
                OrderGlobalStep.class);
        assertNumStep(10, 1, gts.V(sv[0]).local(__.outE("knows").order().by("weight", decr).limit(10)),
                LocalStep.class);
        assertNumStep(numV / 5, 2, gts.V(sv[0]).outE("knows").has("weight", 1).order().by("weight", incr),
                HugeGraphVertexStep.class, OrderGlobalStep.class);
        assertNumStep(10, 1, gts.V(sv[0]).local(__.outE("knows").has("weight", 1).order().by("weight", incr).limit(10)),
                LocalStep.class);
        assertNumStep(5, 1,
                gts.V(sv[0]).local(
                        __.outE("knows").has("weight", 1).has("weight", 1).order().by("weight", incr).range(10, 15)),
                LocalStep.class);
        assertNumStep(1, 1, gts.V(sv[0]).outE("knows").filter(__.inV().is(vs[50])), HugeGraphVertexStep.class);
        assertNumStep(1, 1, gts.V(sv[0]).outE("knows").filter(__.otherV().is(vs[50])), HugeGraphVertexStep.class);
        assertNumStep(1, 1, gts.V(sv[0]).bothE("knows").filter(__.otherV().is(vs[50])), HugeGraphVertexStep.class);
        assertNumStep(1, 2, gts.V(sv[0]).bothE("knows").filter(__.inV().is(vs[50])), HugeGraphVertexStep.class,
                TraversalFilterStep.class);

        // Property
        assertNumStep(numV / 5, 1, gts.V(sv[0]).properties("names").has("weight", 1), HugeGraphPropertiesStep.class);
        assertNumStep(numV, 1, gts.V(sv[0]).properties("names"), HugeGraphPropertiesStep.class);
        assertNumStep(10, 1, gts.V(sv[0]).local(__.properties("names").order().by("weight", decr).limit(10)),
                LocalStep.class);
        assertNumStep(numV, 2, gts.V(sv[0]).outE("knows").values("weight"), HugeGraphVertexStep.class,
                HugeGraphPropertiesStep.class);

        // Global graph queries
        assertNumStep(1, 1, gts.V().has("id", numV / 5), HugeGraphStep.class);
        assertNumStep(1, 1, gts.V().has("id", numV / 5).has("weight", (numV / 5) % 5), HugeGraphStep.class);
        assertNumStep(numV / 5, 1, gts.V().has("weight", 1), HugeGraphStep.class);
        assertNumStep(10, 1, gts.V().has("weight", 1).range(0, 10), HugeGraphStep.class);

        assertNumStep(superV, 1, gts.V().has("id", sid), HugeGraphStep.class);
        // Ensure that as steps don't interfere
        assertNumStep(1, 1, gts.V().has("id", numV / 5).as("x"), HugeGraphStep.class);
        assertNumStep(1, 1, gts.V().has("id", numV / 5).has("weight", (numV / 5) % 5).as("x"), HugeGraphStep.class);

        assertNumStep(superV * (numV / 5), 2, gts.V().has("id", sid).outE("knows").has("weight", 1),
                HugeGraphStep.class, HugeGraphVertexStep.class);
        assertNumStep(superV * (numV / 5 * 2), 2,
                gts.V().has("id", sid).outE("knows").has("weight", P.gte(1)).has("weight", P.lt(3)),
                HugeGraphStep.class, HugeGraphVertexStep.class);
        assertNumStep(superV * (numV / 5 * 2), 2, gts.V().has("id", sid).outE("knows").has("weight", P.between(1, 3)),
                HugeGraphStep.class, HugeGraphVertexStep.class);
        assertNumStep(superV * 10, 2,
                gts.V().has("id", sid).local(__.outE("knows").has("weight", P.gte(1)).has("weight", P.lt(3)).limit(10)),
                HugeGraphStep.class, HugeGraphVertexStep.class);
        assertNumStep(superV * 10, 2,
                gts.V().has("id", sid)
                        .local(__.outE("knows").has("weight", P.between(1, 3)).order().by("weight", decr).limit(10)),
                HugeGraphStep.class, LocalStep.class);
        ...
        assertNumStep(superV * 10, 2,
                gts.V().has("id", sid)
                        .local(__.outE("knows").has("weight", P.between(1, 3)).order().by("weight", decr).limit(10)),
                HugeGraphStep.class, LocalStep.class);
        assertNumStep(superV * numV, 2, gts.V().has("id", sid).values("names"), HugeGraphStep.class,
                HugeGraphPropertiesStep.class);
        ...
        t = gts.V().has("id", sid).local(
                __.outE("knows").has("weight", P.gte(1)).has("weight", P.lt(3)).order().by("weight", decr).limit(10))
                .profile("~metrics");
        assertCount(superV * 10, t);
        metrics = (TraversalMetrics) t.asAdmin().getSideEffects().get("~metrics");
    ...
    private static void assertNumStep(int expectedResults, int expectedSteps, GraphTraversal traversal,
            Class<? extends Step>...expectedStepTypes) {
        int num = 0;
        ...
        // Verify that steps line up with what is expected after HugeGraph's optimizations are applied
    ...
    public void testSimpleTinkerPopTraversal() {
        ...
        Object id = graph.traversal().V().has("name", "josh").outE("created").as("e").inV().has("name", "lop")
                .<Edge> select("e").next().id();
        assertNotNull(id);
    }
    ...
    public void simpleLogTest(final boolean withLogFailure) throws InterruptedException {
        ...
        clopen(option(SYSTEM_LOG_TRANSACTIONS), true, option(LOG_BACKEND, USER_LOG),
                ...
                option(KCVSLog.LOG_READ_LAG_TIME, USER_LOG), Duration.ofMillis(50), option(LOG_READ_INTERVAL, USER_LOG),
                Duration.ofMillis(250), option(LOG_SEND_DELAY, USER_LOG), Duration.ofMillis(100),
                option(KCVSLog.LOG_READ_LAG_TIME, TRANSACTION_LOG), Duration.ofMillis(50),
                option(LOG_READ_INTERVAL, TRANSACTION_LOG), Duration.ofMillis(250), option(MAX_COMMIT_TIME),
        ...
        HugeGraphVertex n1 = tx.addVertex("weight", 10.5);
        newTx();

        final Instant txTimes[] = new Instant[4];
        // Transaction with custom userlog name
        txTimes[0] = times.getTime();
        HugeGraphTransaction tx2 = graph.buildTransaction().logIdentifier(userlogName).start();
        HugeGraphVertex v1 = tx2.addVertex("weight", 111.1);
        v1.addEdge("knows", v1);
        tx2.commit();
        final long v1id = getId(v1);
        txTimes[1] = times.getTime();
        tx2 = graph.buildTransaction().logIdentifier(userlogName).start();
        HugeGraphVertex v2 = tx2.addVertex("weight", 222.2);
        ...
        v1 = getV(tx2, v1id);
        assertEquals(111.1, v1.<Float> value("weight").doubleValue(), 0.01);
        Edge e = Iterables.<HugeGraphEdge> getOnlyElement(v1.query().direction(Direction.OUT).labels("knows").edges());
        ...
        final EnumMap<LogTxStatus, AtomicInteger> txMsgCounter =
                new EnumMap<LogTxStatus, AtomicInteger>(LogTxStatus.class);
        ...
        txlog.registerReader(startMarker, new MessageReader() {
            ...
            public void read(Message message) {
                ...
                TransactionLogHeader.Entry txEntry =
                        TransactionLogHeader.parse(message.getContent(), serializer, times);
                ...
                if (status == LogTxStatus.PRECOMMIT) {
                    ...
                    if (status == LogTxStatus.SECONDARY_FAILURE) {
                        TransactionLogHeader.SecondaryFailures secFail =
                                txEntry.getContentAsSecondaryFailures(serializer);
        ...
        LogProcessorFramework userlogs = HugeGraphFactory.openTransactionLog(graph);
        userlogs.addLogProcessor(userlogName).setStartTime(startTime).setRetryAttempts(1)
                .addProcessor(new ChangeProcessor() {
                    @Override
                    public void process(HugeGraphTransaction tx, TransactionId txId, ChangeState changes) {
                        assertEquals(instanceid, txId.getInstanceId());
                        assertTrue(txId.getTransactionId() > 0 && txId.getTransactionId() < 100); // Just some
                                                                                                  // reasonable upper
                                                                                                  // bound
                        final Instant txTime = txId.getTransactionTime();
                        assertTrue(
                                String.format("tx timestamp %s not between start %s and end time %s", txTime, startTime,
                                        endTime),
                                (txTime.isAfter(startTime) || txTime.equals(startTime))
                                        && (txTime.isBefore(endTime) || txTime.equals(endTime))); // Times should be
                                                                                                  // within a second
                        ...
                        if (txTimeMicro.isBefore(txTimes[1])) {
                            ...
                            HugeGraphVertex v = Iterables.getOnlyElement(changes.getVertices(Change.ADDED));
                            ...
                            HugeGraphVertex v = Iterables.getOnlyElement(changes.getVertices(Change.REMOVED));
                            assertEquals(v2id, getId(v));
                            VertexProperty<Float> p =
                            ...
                            HugeGraphVertex v = Iterables.getOnlyElement(changes.getVertices(Change.ANY));
                            assertEquals(v1id, getId(v));
                            HugeGraphEdge e = Iterables
                                    .getOnlyElement(changes.getEdges(v, Change.REMOVED, Direction.OUT, "knows"));
                            assertFalse(e.property("weight").isPresent());
                        ...
                        HugeGraphVertex v1 = getV(tx, v1id);
                        assertNotNull(v1);
    ...
    public void testGlobalGraphIndexingAndQueriesForInternalIndexes() {
        ...
        HugeGraphIndex edge1 = mgmt.buildIndex("edge1", Edge.class).addKey(time).addKey(weight).buildCompositeIndex();
        HugeGraphIndex edge2 =
                mgmt.buildIndex("edge2", Edge.class).indexOnly(connect).addKey(text).buildCompositeIndex();

        HugeGraphIndex prop1 =
                mgmt.buildIndex("prop1", HugeGraphVertexProperty.class).addKey(time).buildCompositeIndex();
        HugeGraphIndex prop2 = mgmt.buildIndex("prop2", HugeGraphVertexProperty.class).addKey(weight).addKey(text)
                .buildCompositeIndex();

        HugeGraphIndex vertex1 =
                mgmt.buildIndex("vertex1", Vertex.class).addKey(time).indexOnly(person).unique().buildCompositeIndex();
        HugeGraphIndex vertex12 =
                mgmt.buildIndex("vertex12", Vertex.class).addKey(text).indexOnly(person).buildCompositeIndex();
        HugeGraphIndex vertex2 = mgmt.buildIndex("vertex2", Vertex.class).addKey(time).addKey(name)
                .indexOnly(organization).buildCompositeIndex();
        HugeGraphIndex vertex3 = mgmt.buildIndex("vertex3", Vertex.class).addKey(name).buildCompositeIndex();
        ...
        assertEquals(2, Iterables.size(mgmt.getGraphIndexes(HugeGraphVertexProperty.class)));
        ...
        assertTrue(HugeGraphVertexProperty.class.isAssignableFrom(prop1.getIndexedElement()));
        ...
        for (int i = 0; i < numV; i++) {
            ...
            HugeGraphVertex u = ns[(i > 0 ? i - 1 : i)]; // previous or self-loop
        ...
        // ########## QUERIES ################
        evaluateQuery(tx.query().has("time", Cmp.EQUAL, 10).has("weight", Cmp.EQUAL, 0), ElementCategory.EDGE, 1,
                new boolean[] { true, sorted }, edge1.name());
        ...
        evaluateQuery(
                tx.query().has("time", Cmp.EQUAL, 10).has("weight", Cmp.EQUAL, 0).has("text", Cmp.EQUAL,
                        strs[10 % strs.length]),
                ElementCategory.EDGE, 1, new boolean[] { false, sorted }, edge1.name());
        evaluateQuery(tx.query().has("time", Cmp.EQUAL, 10).has("weight", Cmp.EQUAL, 1), ElementCategory.EDGE, 1,
                new boolean[] { true, sorted }, edge1.name());
        evaluateQuery(tx.query().has("time", Cmp.EQUAL, 20).has("weight", Cmp.EQUAL, 0), ElementCategory.EDGE, 1,
                new boolean[] { true, sorted }, edge1.name());
        evaluateQuery(tx.query().has("time", Cmp.EQUAL, 20).has("weight", Cmp.EQUAL, 3), ElementCategory.EDGE, 0,
                new boolean[] { true, sorted }, edge1.name());
        evaluateQuery(tx.query().has("text", Cmp.EQUAL, strs[0]).has(LABEL_NAME, "connect"), ElementCategory.EDGE,
                numV / strs.length, new boolean[] { true, sorted }, edge2.name());
        ...
        evaluateQuery(tx.query().has("text", Cmp.EQUAL, strs[0]), ElementCategory.EDGE, numV / strs.length * 2,
                new boolean[] { false, sorted });
        evaluateQuery(tx.query().has("weight", Cmp.EQUAL, 1.5), ElementCategory.EDGE, numV / 10 * 2,
                new boolean[] { false, sorted });

        evaluateQuery(tx.query().has("time", Cmp.EQUAL, 50), ElementCategory.PROPERTY, 2,
                new boolean[] { true, sorted }, prop1.name());
        ...
        evaluateQuery(
                tx.query().has("weight", Cmp.EQUAL, 0.0).has("text", Cmp.EQUAL, strs[0]).has("time", Cmp.EQUAL, 0),
                ElementCategory.PROPERTY, 2, new boolean[] { true, sorted }, prop2.name(), prop1.name());
        evaluateQuery(tx.query().has("weight", Cmp.EQUAL, 1.5), ElementCategory.PROPERTY, 2 * numV / 10,
                new boolean[] { false, sorted });

        evaluateQuery(tx.query().has("time", Cmp.EQUAL, 50).has(LABEL_NAME, "person"), ElementCategory.VERTEX, 1,
                new boolean[] { true, sorted }, vertex1.name());
        evaluateQuery(tx.query().has("text", Cmp.EQUAL, strs[2]).has(LABEL_NAME, "person"), ElementCategory.VERTEX,
                numV / strs.length, new boolean[] { true, sorted }, vertex12.name());
        evaluateQuery(tx.query().has("text", Cmp.EQUAL, strs[3]).has(LABEL_NAME, "person"), ElementCategory.VERTEX, 0,
                new boolean[] { true, sorted }, vertex12.name());
        ...
        evaluateQuery(
                tx.query().has("time", Cmp.EQUAL, 51).has("name", Cmp.EQUAL, "v51").has(LABEL_NAME, "organization"),
                ElementCategory.VERTEX, 1, new boolean[] { true, sorted }, vertex2.name());
        evaluateQuery(
                tx.query().has("time", Cmp.EQUAL, 51).has("name", Cmp.EQUAL, "u1").has(LABEL_NAME, "organization"),
                ElementCategory.VERTEX, 1, new boolean[] { true, sorted }, vertex2.name());
        evaluateQuery(
                tx.query().has("time", Contain.IN, ImmutableList.of(51, 61, 71, 31, 41)).has("name", Cmp.EQUAL, "u1")
                        .has(LABEL_NAME, "organization"),
                ElementCategory.VERTEX, 5, new boolean[] { true, sorted }, vertex2.name());
        evaluateQuery(tx.query().has("time", Contain.IN, ImmutableList.of()), ElementCategory.VERTEX, 0,
                new boolean[] { true, false });
        evaluateQuery(
                tx.query().has("text", Cmp.EQUAL, strs[2]).has(LABEL_NAME, "person").has("time", Contain.NOT_IN,
                        ImmutableList.of()),
                ElementCategory.VERTEX, numV / strs.length, new boolean[] { true, sorted }, vertex12.name());

        evaluateQuery(tx.query().has("time", Cmp.EQUAL, 51).has(LABEL_NAME, "organization"), ElementCategory.VERTEX, 1,
                new boolean[] { false, sorted });
        evaluateQuery(tx.query().has("name", Cmp.EQUAL, "u1"), ElementCategory.VERTEX, numV / 5,
                new boolean[] { true, sorted }, vertex3.name());
        evaluateQuery(tx.query().has("name", Cmp.EQUAL, "v1"), ElementCategory.VERTEX, 1,
                new boolean[] { true, sorted }, vertex3.name());
        evaluateQuery(tx.query().has("name", Cmp.EQUAL, "v1").has(LABEL_NAME, "organization"), ElementCategory.VERTEX,
                1, new boolean[] { false, sorted }, vertex3.name());
        ...
        for (int i = 0; i < numV / 2; i++) {
            HugeGraphVertex v = getV(tx, ns[i]);
            v.remove();
        }
        ns = new HugeGraphVertex[numV * 3 / 2];
        for (int i = numV; i < numV * 3 / 2; i++) {
            ...
            HugeGraphVertex u = ns[(i > numV ? i - 1 : i)]; // previous or self-loop
            for (String label : new String[] { "connect", "related" }) {
                Edge e = ns[i].addEdge(label, u, "weight", (w++) % 5, "time", t, "text", txt);
            }
        }
        ...
        evaluateQuery(tx.query().has("time", Cmp.EQUAL, 10).has("weight", Cmp.EQUAL, 0), ElementCategory.EDGE, 0,
                new boolean[] { true, sorted }, edge1.name());
        evaluateQuery(tx.query().has("time", Cmp.EQUAL, numV + 10).has("weight", Cmp.EQUAL, 0), ElementCategory.EDGE, 1,
                new boolean[] { true, sorted }, edge1.name());
        evaluateQuery(tx.query().has("text", Cmp.EQUAL, strs[0]).has(LABEL_NAME, "connect").limit(10),
                ElementCategory.EDGE, 10, new boolean[] { true, sorted }, edge2.name());
        evaluateQuery(tx.query().has("weight", Cmp.EQUAL, 1.5), ElementCategory.EDGE, numV / 10 * 2,
        ...
        evaluateQuery(tx.query().has("time", Cmp.EQUAL, 20), ElementCategory.PROPERTY, 0,
                new boolean[] { true, sorted }, prop1.name());
        evaluateQuery(tx.query().has("time", Cmp.EQUAL, numV + 20), ElementCategory.PROPERTY, 2,
                new boolean[] { true, sorted }, prop1.name());

        evaluateQuery(tx.query().has("time", Cmp.EQUAL, 30).has(LABEL_NAME, "person"), ElementCategory.VERTEX, 0,
                new boolean[] { true, sorted }, vertex1.name());
        evaluateQuery(tx.query().has("time", Cmp.EQUAL, numV + 30).has(LABEL_NAME, "person"), ElementCategory.VERTEX, 1,
                new boolean[] { true, sorted }, vertex1.name());
        evaluateQuery(tx.query().has("name", Cmp.EQUAL, "u1"), ElementCategory.VERTEX, numV / 5,
                new boolean[] { true, sorted }, vertex3.name());
    ...
    public void testIndexUniqueness() {
        ...
        HugeGraphIndex vindex1 =
                mgmt.buildIndex("vindex1", Vertex.class).addKey(time).indexOnly(person).unique().buildCompositeIndex();
        HugeGraphIndex vindex2 =
                mgmt.buildIndex("vindex2", Vertex.class).addKey(time).addKey(text).unique().buildCompositeIndex();
        finishSchema();
        ...
        failTransactionOnCommit(new TransactionJob() {
            @Override
            public void run(HugeGraphTransaction tx) {
                HugeGraphVertex v0 = tx.addVertex("person");
                v0.property(VertexProperty.Cardinality.single, "time", 1);
                HugeGraphVertex v1 = tx.addVertex("person");
        ...
        HugeGraphVertex v1 = tx.addVertex("organization");
        v1.property(VertexProperty.Cardinality.single, "time", 1);
        newTx();
        ...
        failTransactionOnCommit(new TransactionJob() {
            @Override
            public void run(HugeGraphTransaction tx) {
                HugeGraphVertex v0 = tx.addVertex("time", 2, "text", "hello");
                HugeGraphVertex v1 = tx.addVertex("time", 2, "text", "hello");
            }
        });
    ...
    public static void evaluateQuery(HugeGraphQuery query, ElementCategory resultType, int expectedResults,
            boolean[] subQuerySpecs, PropertyKey orderKey1, Order order1, String...intersectingIndexes) {
        evaluateQuery(query, resultType, expectedResults, subQuerySpecs, ImmutableMap.of(orderKey1, order1),
                intersectingIndexes);
    }

    public static void evaluateQuery(HugeGraphQuery query, ElementCategory resultType, int expectedResults,
            boolean[] subQuerySpecs, PropertyKey orderKey1, Order order1, PropertyKey orderKey2, Order order2,
    ...
    public static void evaluateQuery(HugeGraphQuery query, ElementCategory resultType, int expectedResults,
            boolean[] subQuerySpecs, String...intersectingIndexes) {
        evaluateQuery(query, resultType, expectedResults, subQuerySpecs, ImmutableMap.<PropertyKey, Order> of(),
                intersectingIndexes);
    }

    public static void evaluateQuery(HugeGraphQuery query, ElementCategory resultType, int expectedResults,
            boolean[] subQuerySpecs, Map<PropertyKey, Order> orderMap, String...intersectingIndexes) {
        ...
        // Check subqueries
        SimpleQueryProfiler subp = Iterables
                .getOnlyElement(Iterables.filter(profiler, p -> !p.getGroupName().equals(QueryProfiler.OPTIMIZATION)));
        if (subQuerySpecs.length == 2) { // 0=>fitted, 1=>ordered
    ...
    public void testForceIndexUsage() {
        ...
        for (int i = 1; i <= 10; i++) {
            HugeGraphVertex v = tx.addVertex("time", i, "age", i);
        }
    ...
    public void testLargeJointIndexRetrieval() {
        ...
        for (int i = 0; i < numV; i++) {
            HugeGraphVertex v = graph.addVertex("color", colors[i % colors.length], "sid", i % sids);
        }
        clopen();
    ...
    public void testIndexQueryWithLabelsAndContainsIN() {
        ...
        HugeGraphIndex uidCompositeIndex =
                mgmt.buildIndex("uidIndex", Vertex.class).indexOnly(label).addKey(uid).unique().buildCompositeIndex();
        mgmt.setConsistency(uidCompositeIndex, ConsistencyModifier.LOCK);
        finishSchema();

        HugeGraphVertex foo = graph.addVertex(labelName);
        HugeGraphVertex bar = graph.addVertex(labelName);
        foo.property("uid", "foo");
        bar.property("uid", "bar");
        graph.tx().commit();

        Iterable<HugeGraphVertex> vertexes = graph.query().has("uid", Contain.IN, ImmutableList.of("foo", "bar"))
                .has(LABEL_NAME, labelName).vertices();
        assertEquals(2, Iterables.size(vertexes));
        for (HugeGraphVertex v : vertexes) {
    ...
    public void testLimitWithMixedIndexCoverage() {
        ...
        HugeGraphVertex a = tx.addVertex(vt, user, fn, "alice");
        HugeGraphVertex b = tx.addVertex(vt, user, fn, "bob");
        HugeGraphVertex v;

        v = Iterables.<HugeGraphVertex> getOnlyElement(tx.query().has(vt, user).has(fn, bob).limit(1).vertices());
        assertEquals(bob, v.value(fn));
        assertEquals(user, v.value(vt));

        v = Iterables.<HugeGraphVertex> getOnlyElement(tx.query().has(vt, user).has(fn, alice).limit(1).vertices());
    ...
    public void testWithoutIndex() {
        ...
        for (int i = 0; i < numV; i++) {
            HugeGraphVertex v = graph.addVertex("kid", random.nextInt(numV), "name", "v" + i);
            if (previous != null) {
    ...
    public void testHasNot() {
        HugeGraphVertex v1, v2;
        v1 = graph.addVertex();

        v2 = (HugeGraphVertex) graph.query().hasNot("abcd").vertices().iterator().next();
        assertEquals(v1, v2);
        v2 = (HugeGraphVertex) graph.query().hasNot("abcd", true).vertices().iterator().next();
        assertEquals(v1, v2);
    }
    ...
    public void testVertexCentricIndexWithNull() {
        ...
        HugeGraphVertex v1 = tx.addVertex(), v2 = tx.addVertex();
    ...
    public void testCreateDelete() {
        ...
        HugeGraphVertex v1 = graph.addVertex(), v3 = graph.addVertex("uid", 445);
        ...
        e = Iterables.<HugeGraphEdge> getOnlyElement(v3.query().direction(Direction.OUT).labels("knows").edges());
        assertEquals(111, e.<Integer> value("uid").intValue());
        ...
        e2 = Iterables.<HugeGraphEdge> getOnlyElement(v1.query().direction(Direction.OUT).labels("friend").edges());
        e2.property("uid", 1);
    ...
    public void testEdgeTTLTiming() throws Exception {
        ...
        HugeGraphVertex v1 = graph.addVertex(), v2 = graph.addVertex(), v3 = graph.addVertex();
    ...
    public void testPropertyTTLTiming() throws Exception {
        ...
        HugeGraphIndex index1 = mgmt.buildIndex("index1", Vertex.class).addKey(name).buildCompositeIndex();
        HugeGraphIndex index2 =
                mgmt.buildIndex("index2", Vertex.class).addKey(name).addKey(place).buildCompositeIndex();
        ...
        HugeGraphVertex v1 = tx.addVertex(T.label, "event", "name", "some event", "place", "somewhere");
    ...
    public void testVertexTTLWithCompositeIndex() throws Exception {
        ...
        HugeGraphIndex index1 = mgmt.buildIndex("index1", Vertex.class).addKey(name).buildCompositeIndex();
        HugeGraphIndex index2 = mgmt.buildIndex("index2", Vertex.class).addKey(name).addKey(time).buildCompositeIndex();
        VertexLabel label1 = mgmt.makeVertexLabel("event").setStatic().make();
        mgmt.setTTL(label1, Duration.ofSeconds(1));
        ...
        HugeGraphVertex v1 = tx.addVertex(T.label, "event", "name", "some event", "time", System.currentTimeMillis());
        tx.commit();
        Object id = v1.id();
    ...
    public void testEdgeTTLLimitedByVertexTTL() throws Exception {
        ...
        if (dbCache) {
            /*
             * TODO: uncomment assertNotEmpty(v2.query().direction(Direction.IN).labels("likes").edges());
             * assertNotEmpty(v2.query().direction(Direction.IN).labels("dislikes").edges());
             * assertNotEmpty(v2.query().direction(Direction.IN).labels("indifferentTo").edges());
             */
        } else {
    ...
    public void testSettingTTLOnUnsupportedType() throws Exception {
        ...
        HugeGraphSchemaType type = ImplicitKey.ID;
    ...
    public void testUnsettingTTL() throws InterruptedException {
        ...
        HugeGraphVertex v1 = graph.addVertex();
    ...
    public void testEdgeTTLImplicitKey() throws Exception {
        ...
        e1 = Iterables.<HugeGraphEdge> getOnlyElement(v1.query().direction(Direction.OUT).labels("likes").edges());
        d = e1.value("~ttl");
        assertEquals(Duration.ofDays(1), d);
    ...
    public void testVertexTTLImplicitKey() throws Exception {
        ...
        HugeGraphVertex v1 = tx.addVertex("event");
        HugeGraphVertex v2 = tx.addVertex();
        tx.commit();

        /*
         * TODO: this fails d = v1.getProperty("~ttl"); assertEquals(1, d); d = v2.getProperty("~ttl"); assertEquals(0,
         * d);
         */
