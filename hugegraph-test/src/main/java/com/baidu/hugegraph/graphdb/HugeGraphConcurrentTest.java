...
public abstract class HugeGraphConcurrentTest extends HugeGraphBaseTest {
    ...
    private static class RandomPropertyMaker implements Runnable {
        ...
        public void run() {
            while (true) {
                ...
                HugeGraphVertex n = getOnlyVertex(tx.query().has(idKey, RandomGenerator.randomInt(0, nodeCount)));
        ...
        protected void doRun() throws Exception {
            ...
            for (int i = 0; i < nodeTraversalCount; i++) {
                ...
                for (Object r : v.query().direction(Direction.OUT).labels(label2Traverse).edges()) {
                    v = ((HugeGraphEdge) r).vertex(Direction.IN);
                }
            }
        }
    }
    ...
    private abstract static class BarrierRunnable implements Runnable {
        ...
        protected final HugeGraphTransaction tx;
        protected final CountDownLatch startLatch;
        protected final CountDownLatch stopLatch;

        public BarrierRunnable(HugeGraphTransaction tx, CountDownLatch startLatch, CountDownLatch stopLatch) {
            this.tx = tx;
