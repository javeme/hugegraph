...
import com.baidu.hugegraph.diskstorage.locking.consistentkey.*;
...
public class ConsistentKeyLockerTest {
    ...
    public void setupMocks() throws BackendException, NoSuchMethodException, SecurityException {
        ...
        /*
         * relaxedControl doesn't care about the order in which its mocks' methods are called. This is useful for mocks
        ...
        expect(manager.beginTransaction(cmp(null, defaultTxCfgChecker, LogicalOperator.EQUAL))).andReturn(defaultTx)
                .anyTimes();
        ...
        expect(manager.beginTransaction(cmp(null, otherTxCfgChecker, LogicalOperator.EQUAL))).andReturn(otherTx)
                .anyTimes();

        /*
         * ctrl requires that the complete, order-sensitive sequence of actual method invocations on its mocks exactly
         * match the expected sequence hard-coded into each test method. Either an unexpected actual invocation or
         * expected invocation that fails to actually occur will cause a test failure.
         */
    ...
    /**
     * Test a single lock using stub objects. Doesn't test unlock ("leaks" the lock, but since it's backed by stubs, it
     * doesn't matter).
     *
     * @throws com.baidu.hugegraph.diskstorage.BackendException shouldn't happen
     */
    @Test
    ...
    /**
     * Test locker when first attempt to write to the store takes too long (but succeeds). Expected behavior is to call
     * mutate on the store, adding a column with a new timestamp and deleting the column with the old
    ...
    /**
     * Test locker when all three attempts to write a lock succeed but take longer than the wait limit. We expect the
     * locker to delete all three columns that it wrote and locally unlock the KeyColumn, then emit an exception.
     *
     * @throws com.baidu.hugegraph.diskstorage.BackendException shouldn't happen
     */
    @Test
    ...
    /**
     * Test that the first {@link com.baidu.hugegraph.diskstorage.PermanentBackendException} thrown by the locker's
     * store causes it to attempt to delete outstanding lock writes and then emit the exception without retrying.
     *
     * @throws com.baidu.hugegraph.diskstorage.BackendException shouldn't happen
     */
    @Test
    ...
    /**
     * Test the locker retries a lock write after the initial store mutation fails with a
     * {@link com.baidu.hugegraph.diskstorage.TemporaryBackendException}. The retry should both attempt to write the and
     * delete the failed mutation column.
     *
     * @throws com.baidu.hugegraph.diskstorage.BackendException shouldn't happen
     */
    @Test
    ...
    /**
     * Claim a lock without errors using {@code defaultTx}, the check that {@code otherTx} can't claim it, instead
     * throwing a TemporaryLockingException
     *
     * @throws com.baidu.hugegraph.diskstorage.BackendException shouldn't happen
     */
    @Test
    ...
    /**
     * Test that multiple calls to {@link ConsistentKeyLocker#writeLock(KeyColumn, StoreTransaction)} with the same
     * arguments have no effect after the first call (until {@link ConsistentKeyLocker#deleteLocks(StoreTransaction)} is
     * called).
     *
     * @throws com.baidu.hugegraph.diskstorage.BackendException shouldn't happen
     */
    ...
    /**
     * Test a single checking a single lock under optimal conditions (no timeouts, no errors)
     *
     * @throws com.baidu.hugegraph.diskstorage.BackendException shouldn't happen
    ...
    public void testCheckLocksInSimplestCase() throws BackendException, InterruptedException {
        ...
        recordLockGetSliceAndReturnSingleEntry(
                StaticArrayEntry.of(codec.toLockCol(ls.getWriteTimestamp(), defaultLockRid, times), defaultLockVal));
        ctrl.replay();
    ...
    /**
     * A transaction that writes a lock, waits past expiration, and attempts to check locks should receive an
     * {@code ExpiredLockException} during the check stage.
     *
     * @throws com.baidu.hugegraph.diskstorage.BackendException shouldn't happen
    ...
    public void testCheckOwnExpiredLockThrowsException() throws BackendException, InterruptedException {
        ...
        currentTimeNS = currentTimeNS.plus(100, ChronoUnit.DAYS); // pretend a huge multiple of the expiration time has
                                                                  // passed
        ...
        recordLockGetSliceAndReturnSingleEntry(StaticArrayEntry
                .of(codec.toLockCol(expired.getWriteTimestamp(), defaultLockRid, times), defaultLockVal));
    ...
    /**
     * A transaction that detects expired locks from other transactions, or from its own transaction but with a
     * different timestamp than the one currently stored in memory by the transaction (presumably from an earlier
    ...
    public void testCheckLocksIgnoresOtherExpiredLocks() throws BackendException, InterruptedException {
        ...
        recordLockGetSlice(StaticArrayEntryList.of(
                StaticArrayEntry.of(codec.toLockCol(otherExpired.getWriteTimestamp(), otherLockRid, times),
                        defaultLockVal),
                StaticArrayEntry.of(codec.toLockCol(ownExpired.getWriteTimestamp(), defaultLockRid, times),
                        defaultLockVal),
                StaticArrayEntry.of(codec.toLockCol(recent.getWriteTimestamp(), defaultLockRid, times),
                        defaultLockVal)));
    ...
    /**
     * Each written lock should be checked at most once. Test this by faking a single previously written lock using
     * mocks and stubs and then calling checkLocks() twice. The second call should have no effect.
     *
     * @throws InterruptedException shouldn't happen
     * @throws com.baidu.hugegraph.diskstorage.BackendException shouldn't happen
     */
    ...
    /**
     * If the checker reads its own lock column preceeded by a lock column from another rid with an earlier timestamp
     * and the timestamps on both columns are unexpired, then the checker must throw a TemporaryLockingException.
     *
     * @throws InterruptedException shouldn't happen
     * @throws com.baidu.hugegraph.diskstorage.BackendException shouldn't happen (we expect a TemporaryLockingException
     *             but we catch and swallow it)
     */
    @Test
    public void testCheckLocksFailsWithSeniorClaimsByOthers() throws InterruptedException, BackendException {
        ...
        recordLockGetSlice(StaticArrayEntryList.of(StaticArrayEntry.of(otherSeniorLockCol, defaultLockVal),
    ...
    /**
     * When the checker retrieves its own lock column followed by a lock column with a later timestamp (both with
     * unexpired timestamps), it should consider the lock successfully checked.
     *
     * @throws InterruptedException shouldn't happen
     * @throws com.baidu.hugegraph.diskstorage.BackendException shouldn't happen
     */
    ...
    public void testCheckLocksSucceedsWithJuniorClaimsByOthers() throws InterruptedException, BackendException {
        ...
        // When the checker slices the store, return the senior lock col by a
        // foreign tx and the junior lock col by defaultTx (in that order)
        recordLockGetSlice(StaticArrayEntryList.of(StaticArrayEntry.of(ownSeniorLockCol, defaultLockVal),
    ...
    /**
     * If the checker retrieves a timestamp-ordered list of columns, where the list starts with an unbroken series of
     * columns with the checker's rid but differing timestamps, then consider the lock successfully checked if the
     * checker's expected timestamp occurs anywhere in that series of columns.
     * <p/>
     * This relaxation of the normal checking rules only triggers when either writeLock(...) issued mutate calls that
     * appeared to fail client-side but which actually succeeded (e.g. hinted handoff or timeout)
     *
     * @throws InterruptedException shouldn't happen
     * @throws com.baidu.hugegraph.diskstorage.BackendException shouldn't happen
     */
    ...
    public void testCheckLocksSucceedsWithSeniorAndJuniorClaimsBySelf() throws InterruptedException, BackendException {
        ...
        // When the checker slices the store, return the senior lock col by a
        // foreign tx and the junior lock col by defaultTx (in that order)
        recordLockGetSlice(StaticArrayEntryList.of(StaticArrayEntry.of(myFirstLockCol, defaultLockVal),
    ...
    /**
     * The checker should retry getSlice() in the face of a TemporaryStorageException so long as the number of
     * exceptional getSlice()s is fewer than the lock retry count. The retry count applies on a per-lock basis.
     *
     * @throws com.baidu.hugegraph.diskstorage.BackendException shouldn't happen
     * @throws InterruptedException shouldn't happen
     */
    @Test
    public void testCheckLocksRetriesAfterSingleTemporaryStorageException()
    ...
    /**
     * The checker will throw a TemporaryStorageException if getSlice() throws fails with a TemporaryStorageException as
     * many times as there are configured lock retries.
     *
     * @throws InterruptedException shouldn't happen
     * @throws com.baidu.hugegraph.diskstorage.BackendException shouldn't happen
     */
    @Test
    public void testCheckLocksThrowsExceptionAfterMaxTemporaryStorageExceptions()
        ...
        recordExceptionalLockGetSlice(
                new TemporaryBackendException("Storage cluster is in a dissociative fugue state"));
        recordExceptionalLockGetSlice(
                new TemporaryBackendException("Storage cluster has gone to Prague to find itself"));
    ...
    /**
     * A single PermanentStorageException on getSlice() for a single lock is sufficient to make the method return
     * immediately (regardless of whether other locks are waiting to be checked).
     *
     * @throws InterruptedException shouldn't happen
     * @throws com.baidu.hugegraph.diskstorage.BackendException shouldn't happen
     */
    ...
    /**
     * The lock checker should do nothing when passed a transaction for which it holds no locks.
     *
     * @throws com.baidu.hugegraph.diskstorage.BackendException shouldn't happen
     */
    @Test
    ...
    /**
     * Lock deletion should retry if the first store mutation throws a temporary exception.
     *
     * @throws com.baidu.hugegraph.diskstorage.BackendException shouldn't happen
     */
    @Test
    ...
    /**
     * If lock deletion exceeds the temporary exception retry count when trying to delete a lock, it should move onto
     * the next lock rather than returning and potentially leaving the remaining locks undeleted.
     *
     * @throws com.baidu.hugegraph.diskstorage.BackendException shouldn't happen
     */
    @Test
    ...
    /**
     * Same as {@link #testDeleteLocksSkipsToNextLockAfterMaxTemporaryStorageExceptions()} , except instead of exceeding
     * the temporary exception retry count on a lock, that lock throws a single permanent exception.
     *
     * @throws com.baidu.hugegraph.diskstorage.BackendException shoudn't happen
     */
    @Test
    ...
    /**
     * Deletion should remove previously written locks regardless of whether they were ever checked; this method fakes
     * and verifies deletion on a single unchecked lock
     *
     * @throws com.baidu.hugegraph.diskstorage.BackendException shouldn't happen
     */
    @Test
    ...
    /**
     * When delete is called multiple times with no intervening write or check calls, all calls after the first should
     * have no effect.
     *
     * @throws com.baidu.hugegraph.diskstorage.BackendException shouldn't happen
     */
    ...
    public void testDeleteLocksIdempotence() throws BackendException {
        ...
        ctrl.reset();
        expect(lockState.getLocksForTx(defaultTx))
                .andReturn(Maps.newLinkedHashMap(ImmutableMap.<KeyColumn, ConsistentKeyLockStatus> of()));
        ctrl.replay();
        locker.deleteLocks(defaultTx);
    }

    /**
     * Delete should do nothing when passed a transaction for which it holds no locks.
     *
     * @throws com.baidu.hugegraph.diskstorage.BackendException shouldn't happen
     */
    @Test
    ...
    /**
     * Checking locks when the expired lock cleaner is enabled should trigger one call to the LockCleanerService.
     *
     * @throws com.baidu.hugegraph.diskstorage.BackendException shouldn't happen
     */
    @Test
    ...
    private ConsistentKeyLockStatus makeStatus(Instant currentNS) {
        return new ConsistentKeyLockStatus(currentTimeNS, Instant.EPOCH.plus(defaultExpireNS));
    }
    ...
    private LockInfo recordSuccessfulLockWrite(long duration, TemporalUnit tu, StaticBuffer del)
            throws BackendException {
        return recordSuccessfulLockWrite(defaultTx, duration, tu, del);
    }

    private LockInfo recordSuccessfulLockWrite(StoreTransaction tx, long duration, TemporalUnit tu, StaticBuffer del)
            throws BackendException {
        currentTimeNS = currentTimeNS.plusNanos(1);
        expect(times.getTime()).andReturn(currentTimeNS);
    ...
    private StaticBuffer recordExceptionLockWrite(long duration, TemporalUnit tu, StaticBuffer del, Throwable t)
            throws BackendException {
        currentTimeNS = currentTimeNS.plusNanos(1);
        expect(times.getTime()).andReturn(currentTimeNS);
        StaticBuffer lockCol = codec.toLockCol(currentTimeNS, defaultLockRid, times);
    ...
    private ConsistentKeyLocker.Builder getDefaultBuilder() {
        return new ConsistentKeyLocker.Builder(store, manager).times(times).mediator(mediator).internalState(lockState)
                .lockExpire(defaultExpireNS).lockWait(defaultWaitNS).rid(defaultLockRid);
    }

    /*
     * This class supports partial mocking of TimestampProvider.
     *
     * It's impossible to mock Timestamps.NANO because that is an enum; EasyMock will fail to do it at runtime with
     * cryptic "incompatible return value type" exceptions.
     */
