...
public class StaticArrayEntryTest {
    ...
    public void testArrayBuffer() {
        ...
        for (int i = 1; i <= 6; i++)
            assertEquals(i, entry.getInt((i - 1) * 4));
        ReadBuffer rb = entry.asReadBuffer();
        for (int i = 1; i <= 6; i++)
            assertEquals(i, rb.getInt());
    ...
    public void testEntryList() {
        ...
        el[5] = StaticArrayEntryList
                .of(Iterables.transform(entries.entrySet(), new Function<Map.Entry<Integer, Long>, Entry>() {
                    @Nullable
                    @Override
    ...
    private static enum StaticEntryGetter
            implements StaticArrayEntry.GetColVal<Map.Entry<Integer, Long>, StaticBuffer> {
