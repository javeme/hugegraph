...
public class VertexIDAssignerTest {
    ...
    public static Collection<Object[]> configs() {
        ...
        for (int maxPerPartition : new int[] { Integer.MAX_VALUE, 100, 300 }) {
            for (int numPartitions : new int[] { 2, 4, 10 }) {
                for (int[] local : new int[][] { null, { 0, 2, numPartitions }, { 235, 234, 8 }, { 1, 1, 2 },
                        { 0, 1 << (numPartitions - 1), numPartitions } }) {
    ...
    /**
     *
     * @param numPartitionsBits The number of partitions bits to use. This means there are exactly (1<<numPartitionBits)
     *            partitions.
     * @param partitionMax The maxium number of ids that can be allocated per partition. This is artifically constraint
     *            by the MockIDAuthority
     * @param localPartitionDef This array contains three integers: 1+2) lower and upper bounds for the local partition
     *            range, and 3) the bit width of the local bounds. The bounds will be bitshifted forward to consume the
     *            bit width
     */
    public VertexIDAssignerTest(int numPartitionsBits, int partitionMax, int[] localPartitionDef) {
        ...
        if (null != localPartitionDef) {
            ...
            idAuthority.setLocalPartition(
                    PartitionIDRangeTest.convert(localPartitionDef[0], localPartitionDef[1], localPartitionDef[2]));
        }
        ...
        System.out.println(String.format("Configuration [%s|%s|%s]", numPartitionsBits, partitionMax,
                Arrays.toString(localPartitionDef)));

        if (localPartitionDef != null && localPartitionDef[0] < localPartitionDef[1]
                && localPartitionDef[2] <= numPartitionsBits) {
            this.maxIDAssignments =
                    ((localPartitionDef[1] - localPartitionDef[0]) << (numPartitionsBits - localPartitionDef[2]))
                            * ((long) partitionMax);
        } else {
    ...
    private static HugeGraph getInMemoryGraph() {
        ModifiableConfiguration config = GraphDatabaseConfiguration.buildGraphConfiguration();
        ...
        return HugeGraphFactory.open(config);
    }
    ...
    public void testIDAssignment() {
        ...
        for (int trial = 0; trial < 10; trial++) {
            for (boolean flush : new boolean[] { true, false }) {
                HugeGraph graph = getInMemoryGraph();
                int numVertices = 1000;
                List<HugeGraphVertex> vertices = new ArrayList<HugeGraphVertex>(numVertices);
                List<InternalRelation> relations = new ArrayList<InternalRelation>();
                HugeGraphVertex old = null;
                totalRelations += 2 * numVertices;
                totalVertices += numVertices;
                try {
                    for (int i = 0; i < numVertices; i++) {
                        HugeGraphVertex next = graph.addVertex();
