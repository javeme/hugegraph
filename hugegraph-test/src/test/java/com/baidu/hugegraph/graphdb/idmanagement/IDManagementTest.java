...
import com.baidu.hugegraph.diskstorage.ReadBuffer;
...
public class IDManagementTest {
    ...
    public void testEntityID(int partitionBits, int partition, long minCount, long maxCount) {
        ...
        for (long count = minCount; count < maxCount; count++) {
            ...
            for (IDManager.VertexIDType vtype : USER_VERTEX_TYPES) {
                if (partitionBits == 0 && vtype == IDManager.VertexIDType.PartitionedVertex)
                    continue;
                if (vtype == IDManager.VertexIDType.PartitionedVertex)
    ...
    public void writingInlineEdgeTypes() {
        ...
        for (long i = 1; i <= numTries; i++) {
            IDHandler.writeInlineRelationType(out,
                    IDManager.getSchemaId(IDManager.VertexIDType.UserEdgeLabel, i * 1000));
        }
    ...
    public void testEdgeTypeWriting() {
        ...
        for (int i = 0; i < 1000; i++) {
            IDManager.VertexIDType type = random.nextDouble() < 0.5 ? IDManager.VertexIDType.UserPropertyKey
                    : IDManager.VertexIDType.UserEdgeLabel;
    ...
    public void partitionIDRangeTest() {
        ...
        List<PartitionIDRange> result =
                PartitionIDRange.getIDRanges(16, ImmutableList.of(getKeyRange(120 << 16, 6, 140 << 16, 8)));
