 ...
 *
 * Helper functions for declaring hugegraph schema elements
 * (vertex labels, edge labels, property keys) to accommodate
 * TP3 sample data.
 *
 * Sample usage in a gremlin.sh session:
 *
 * gremlin> :load data/hugegraph-schema-grateful-dead.groovy
 * ==>true
 * ==>true
 * gremlin> t = HugeGraphFactory.open('conf/hugegraph-cassandra.properties')
 * ==>standardhugegraph[cassandrathrift:[127.0.0.1]]
...
def defineGratefulDeadSchema(hugegraph) {
    m = hugegraph.openManagement()
    // vertex labels
