...
public enum MetricManager {
     ...
     *
     * @return the single {@code MetricRegistry} used for all of HugeGraph's Metrics monitoring
     */
    ...
    /**
     * Stop a {@link ConsoleReporter} previously created by a call to {@link #addConsoleReporter(long)} and release it
     * for GC. Idempotent between calls to the associated add method. Does nothing before the first call to the
     * associated add method.
     */
     ...
     * <p>
     * The {@code output} argument must be non-null but need not exist. If it doesn't already exist, this method
     * attempts to create it by calling {@link File#mkdirs()}.
     *
     * @param reportInterval time to wait between dumping metrics to CSV files in the configured directory
     * @param output the path to a directory into which Metrics will periodically write CSV data
     */
    ...
    /**
     * Stop a {@link CsvReporter} previously created by a call to {@link #addCsvReporter(long, String)} and release it
     * for GC. Idempotent between calls to the associated add method. Does nothing before the first call to the
     * associated add method.
     */
     ...
     * <p>
     * If {@code agentId} is non-null, then {@link MBeanServerFactory#findMBeanServer(agentId)} must return exactly one
     * {@code MBeanServer}. The reporter will register with that server. If the {@code findMBeanServer(agentId)} call
     * returns no or multiple servers, then this method logs an error and falls back on the Metrics default for
     * {@code agentId}.
     *
    ...
    /**
     * Stop a {@link JmxReporter} previously created by a call to {@link #addJmxReporter(String, String)} and release it
     * for GC. Idempotent between calls to the associated add method. Does nothing before the first call to the
     * associated add method.
     */
     ...
     * <p>
     * If {@code loggerName} is null, or if it is non-null but {@link LoggerFactory#getLogger(loggerName)} returns null,
     * then Metrics's default Slf4j logger name is used instead.
     *
     * @param reportInterval time to wait between writing metrics to the Slf4j logger
     ...
     * it for GC. Idempotent between calls to the associated add method. Does nothing before the first call to the
     * associated add method.
     */
    ...
    /**
     * Create a {@link GangliaReporter} attached to the HugeGraph Metrics registry.
     * <p>
     * {@code groupOrHost} and {@code addressingMode} must be non-null. The remaining non-primitive arguments may be
     * null. If {@code protocol31} is null, then true is assumed. Null values of {@code hostUUID} or {@code spoof} are
     * passed into the {@link GMetric} constructor, which causes Ganglia to use its internal logic for generating a
     * default UUID and default reporting hostname (respectively).
     *
     * @param groupOrHost the multicast group or unicast hostname to which Ganglia events are sent
     ...
     * @param spoof override this machine's IP/hostname as it appears on the Ganglia server
     * @param reportInterval titme to wait before sending data to the ganglia unicast host or multicast group
     * @throws IOException when a {@link GMetric} can't be instantiated using the provided arguments
     */
    public synchronized void addGangliaReporter(String groupOrHost, int port, UDPAddressingMode addressingMode, int ttl,
            Boolean protocol31, UUID hostUUID, String spoof, Duration reportInterval) throws IOException {
        ...
        log.info(
                "Configured Ganglia Metrics reporter host={} interval={} port={} addrmode={} ttl={} proto31={} uuid={} spoof={}",
     ...
     * {@link #addGangliaReporter(String, int, UDPAddressingMode, int, Boolean, UUID, long)} and release it for GC.
     * Idempotent between calls to the associated add method. Does nothing before the first call to the associated add
     * method.
     */
    ...
    /**
     * Create a {@link GraphiteReporter} attached to the HugeGraph Metrics registry.
     * <p>
     * If {@code prefix} is null, then Metrics's internal default prefix is used (empty string at the time this comment
     * was written).
     *
     ...
     * @param reportInterval time to wait between sending metrics to the configured Graphite host and port
     */
     ...
     * {@link #addGraphiteReporter(String, int, String, long)} and release it for GC. Idempotent between calls to the
     * associated add method. Does nothing before the first call to the associated add method.
     */
    ...
    /**
     * Remove all HugeGraph Metrics reporters previously configured through the {@code add*} methods on this class.
     */
