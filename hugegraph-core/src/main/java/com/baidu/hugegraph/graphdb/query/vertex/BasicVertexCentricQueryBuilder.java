...
/**
 * Builds a {@link BaseVertexQuery}, optimizes the query and compiles the result into a
 * {@link com.baidu.hugegraph.graphdb.query.vertex.BaseVertexCentricQuery} which is then executed by one of the
 * extending classes.
 *
...
public abstract class BasicVertexCentricQueryBuilder<Q extends BaseVertexQuery<Q>>
     ...
     * vertex object only. This is used in combination with
     * {@link com.baidu.hugegraph.graphdb.vertices.PreloadedVertex}, for example.
     */
    ...
    /**
     * Calling this method will cause this query to only included loaded (i.e. unmodified) relations in the result set.
     * 
     * @return
     */
    ...
    protected boolean hasAllCanonicalTypes() {
        ...
        for (String typeName : types) {
            ...
            if (type != null && !type.isPropertyKey() && !type.multiplicity().isUnique(dir))
                return false;
        }
        return true;
    }
    ...
    protected static Iterable<HugeGraphVertex> edges2Vertices(final Iterable<HugeGraphEdge> edges,
            final HugeGraphVertex other) {
    ...
    /**
     * If {@link #isImplicitKeyQuery(com.baidu.hugegraph.graphdb.internal.RelationCategory)} is true, this method
     * provides the result set for the query based on the evaluation of the {@link ImplicitKey}.
     * </p>
    ...
    protected Iterable<HugeGraphRelation> executeImplicitKeyQuery(InternalVertex v) {
        ...
        if (dir == Direction.IN || limit < 1)
            return ImmutableList.of();
        ImplicitKey key = (ImplicitKey) tx.getRelationType(types[0]);
        return ImmutableList.of((HugeGraphRelation) new StandardVertexProperty(0, key, v, key.computeProperty(v),
            ...
            if (!hasAllCanonicalTypes()) {
                ...
                for (InternalVertex rep : representatives) {
                    ...
                    if (merge == null)
                        ...
                        merge = ResultMergeSortIterator.mergeSort(merge, iter, (Comparator) orders, false);
                }
    ...
    private Iterable<HugeGraphRelation> executeIndividualRelations(InternalVertex vertex,
        ...
        if (useSimpleQueryProcessor(query, vertex))
            ...
            return new QueryProcessor<VertexCentricQuery, HugeGraphRelation, SliceQuery>(query, tx.edgeProcessor);
    }
            ...
            if (!hasAllCanonicalTypes()) {
                ...
                for (InternalVertex rep : representatives) {
                    ...
                    if (merge == null)
                        ...
                        merge = ResultMergeSortIterator.mergeSort(merge, iter, VertexArrayList.VERTEX_ID_COMPARATOR,
                                false);
                }
                return ResultSetIterator.wrap(merge, baseQuery.getLimit());
            } else
    ...
    private Iterable<HugeGraphVertex> executeIndividualVertices(InternalVertex vertex,
            BaseVertexCentricQuery baseQuery) {
        VertexCentricQuery query = constructQuery(vertex, baseQuery);
        if (useSimpleQueryProcessor(query, vertex))
            return new SimpleVertexQueryProcessor(query, tx).vertexIds();
        else
            return edges2Vertices((Iterable) executeIndividualRelations(vertex, baseQuery), query.getVertex());
    }
    ...
    /**
     * Constructs a {@link VertexCentricQuery} for this query builder. The query construction and optimization logic is
     * taken from {@link #constructQuery(com.baidu.hugegraph.graphdb.internal.RelationCategory)} This method only adds
     * the additional conditions that are based on the base vertex.
     *
    ...
    protected VertexCentricQuery constructQuery(InternalVertex vertex, BaseVertexCentricQuery baseQuery) {
        ...
        if (!baseQuery.isEmpty()) {
            // Add adjacent-vertex and direction related conditions; copy conditions to so that baseQuery does not
            // change
    ...
    protected BaseVertexCentricQuery constructQueryWithoutProfile(RelationCategory returnType) {
        ...
        // Don't be smart with query limit adjustments - it just messes up the caching layer and penalizes when
        // appropriate limits are set by the user!
        ...
        if (!hasTypes()) {
            BackendQueryHolder<SliceQuery> query = new BackendQueryHolder<SliceQuery>(
            ...
            if (sliceLimit != Query.NO_LIMIT && sliceLimit < Integer.MAX_VALUE / 3) {
                // If only one direction is queried, ask for twice the limit from backend since approximately half will
                // be filtered
            ...
            conditions.add(new VisibilityFilterCondition<HugeGraphRelation>( // Need this to filter out newly created
                                                                             // invisible relations in the transaction
            ...
            for (Interval pint : intervalConstraints.values()) { // Check if one of the constraints leads to an empty
                                                                 // result set
                if (pint.isEmpty())
                    return BaseVertexCentricQuery.emptyQuery();
            }
            ...
            for (String typeName : types) {
                ...
                if (type instanceof ImplicitKey)
                    throw new UnsupportedOperationException(
                            "Implicit types are not supported in complex queries: " + type);
                ts.add(type);
                ...
                if (type.isEdgeLabel()) {
                    ...
                    if (!type.isUnidirected(Direction.BOTH)) {
                        ...
                        } else if (!type.isUnidirected(typeDir))
                            continue; // Directions are incompatible
                    }
                }
                ...
                if (type.isEdgeLabel() && typeDir == Direction.BOTH && intervalConstraints.isEmpty()
                    ...
                    for (Direction direction : dirs) {
                        /*
                         * Find best scoring relation type to answer this query with. We score each candidate by the
                         * number of conditions that each sort-keys satisfy. Equality conditions score higher than
                         * interval conditions since they are more restrictive. We assign additional points if the sort
                         * key satisfies the order of this query.
                         */
                        ...
                        for (InternalRelationType candidate : type.getRelationIndexes()) {
                            ...
                            boolean supportsOrder =
                                    orders.isEmpty() ? true : orders.getCommonOrder() == candidate.getSortOrder();
                            int currentOrder = 0;
    ...
    private void constructSliceQueries(PropertyKey[] extendedSortKey, EdgeSerializer.TypedInterval[] sortKeyConstraints,
            int position, InternalRelationType bestCandidate, Direction direction,
            Map<RelationType, Interval> intervalConstraints, int sliceLimit, boolean isIntervalFittedConditions,
            boolean bestCandidateSupportsOrder, List<BackendQueryHolder<SliceQuery>> queries) {
        if (position < extendedSortKey.length) {
            ...
            if (interval != null && interval.isPoints()) {
                ...
                for (Object point : interval.getPoints()) {
                    ...
                    constructSliceQueries(extendedSortKey, clonedSKC, position, bestCandidate, direction,
                            intervalConstraints, sliceLimit, isIntervalFittedConditions, bestCandidateSupportsOrder,
                            queries);
                }
                return;
    ...
    /**
     * Returns the extended sort key of the given type. The extended sort key extends the type's primary sort key by
     * ADJACENT_ID and ID depending on the multiplicity of the type in the given direction. It also converts the type
     * ids to actual types.
     *
     * @param type
     * @param dir
    ...
    /**
     * Converts the constraint conditions of this query into a constraintMap which is passed as an argument. If all the
     * constraint conditions could be accounted for in the constraintMap, this method returns true, else - if some
     * constraints cannot be captured in an interval - it returns false to indicate that further in-memory filtering
     * will be necessary.
     * </p>
     * This constraint map is used in constructing the SliceQueries and query optimization since this representation is
     * easier to handle.
     *
     * @param conditions
     * @param constraintMap
    ...
    private static Interval intersectConstraints(Interval pint, RelationType type, HugeGraphPredicate predicate,
            Object value) {
        Interval newInt;
     ...
     * this "waste" in order to not have to adjust the limit too often in
     * {@link com.baidu.hugegraph.graphdb.query.LimitAdjustingIterator}.
     *
