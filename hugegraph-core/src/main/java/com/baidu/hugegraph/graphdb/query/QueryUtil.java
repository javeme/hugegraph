...
public class QueryUtil {
    ...
    public static int adjustLimitForTxModifications(StandardHugeGraphTx tx, int uncoveredAndConditions, int limit) {
    ...
    public static InternalRelationType getType(StandardHugeGraphTx tx, String typeName) {
    ...
    public static Iterable<HugeGraphVertex> getVertices(StandardHugeGraphTx tx, PropertyKey key,
            Object equalityCondition) {
        return tx.query().has(key, Cmp.EQUAL, equalityCondition).vertices();
    }

    public static Iterable<HugeGraphVertex> getVertices(StandardHugeGraphTx tx, String key, Object equalityCondition) {
        return tx.query().has(key, Cmp.EQUAL, equalityCondition).vertices();
    }

    public static Iterable<HugeGraphEdge> getEdges(StandardHugeGraphTx tx, PropertyKey key, Object equalityCondition) {
        return tx.query().has(key, Cmp.EQUAL, equalityCondition).edges();
    }

    public static Iterable<HugeGraphEdge> getEdges(StandardHugeGraphTx tx, String key, Object equalityCondition) {
        return tx.query().has(key, Cmp.EQUAL, equalityCondition).edges();
    }

    /**
     * Query-normal-form (QNF) for HugeGraph is a variant of CNF (conjunctive normal form) with negation inlined where
     * possible
     *
     * @param condition
     * @return
    ...
    public static final <E extends HugeGraphElement> Condition<E> simplifyQNF(Condition<E> condition) {
        Preconditions.checkArgument(isQueryNormalForm(condition));
        if (condition.numChildren() == 1) {
            Condition<E> child = ((And) condition).get(0);
            if (child.getType() == Condition.Type.LITERAL)
                return child;
        }
        return condition;
    }
    ...
    /**
     * Prepares the constraints from the query builder into a QNF compliant condition. If the condition is invalid or
     * trivially false, it returns null.
     *
     * @param tx
     * @param constraints
     * @param <E>
     * @return
     * @see #isQueryNormalForm(com.baidu.hugegraph.graphdb.query.condition.Condition)
     */
    public static <E extends HugeGraphElement> And<E> constraints2QNF(StandardHugeGraphTx tx,
        ...
        for (PredicateCondition<String, E> atom : constraints) {
            ...
            HugeGraphPredicate predicate = atom.getPredicate();

            if (type.isPropertyKey()) {
                PropertyKey key = (PropertyKey) type;
                assert predicate.isValidCondition(value);
                Preconditions.checkArgument(
                        key.dataType() == Object.class || predicate.isValidValueType(key.dataType()),
                        "Data type of key is not compatible with condition");
            } else { // its a label
                Preconditions.checkArgument(((EdgeLabel) type).isUnidirected());
                Preconditions.checkArgument(predicate.isValidValueType(HugeGraphVertex.class),
                        "Data type of key is not compatible with condition");
            }
            ...
            if (predicate instanceof Contain) {
                ...
                if (predicate == Contain.NOT_IN) {
                    if (values.isEmpty())
                        continue; // Simply ignore since trivially satisfied
    ...
    private static <E extends HugeGraphElement> void addConstraint(RelationType type, HugeGraphPredicate predicate,
            Object value, MultiCondition<E> conditions, StandardHugeGraphTx tx) {
        if (type.isPropertyKey()) {
            if (value != null)
                value = tx.verifyAttribute((PropertyKey) type, value);
        } else { // t.isEdgeLabel()
            Preconditions.checkArgument(value instanceof HugeGraphVertex);
        }
    ...
    public static Map.Entry<RelationType, Collection> extractOrCondition(Or<HugeGraphRelation> condition) {
    ...
    public static <R> List<R> processIntersectingRetrievals(List<IndexCall<R>> retrievals, final int limit) {
        ...
        List<R> results;
        /*
         * Iterate over the clauses in the and collection query.getCondition().getChildren(), taking the intersection of
         * current results with cumulative results on each iteration.
         */
        ...
        do {
            ...
            for (IndexCall<R> call : retrievals) {
                ...
                try {
                    ...
                    throw new HugeGraphException("Could not process individual retrieval call ", e);
                }
                ...
                if (results == null) {
                    ...
                    while (riter.hasNext()) {
                        if (!subresultset.contains(riter.next()))
                            riter.remove();
                    }
                }
            }
