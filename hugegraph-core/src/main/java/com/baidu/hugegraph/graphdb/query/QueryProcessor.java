...
import com.baidu.hugegraph.core.QueryException;
 ...
 * At the highest level, the OuterIterator ensures that the correct (up to the given limit) number of elements is
 * returned. It also provides the implementation of remove() by calling the element's remove() method. </br>
 * The OuterIterator wraps the "unfolded" iterator which is a combination of the individual result set iterators of the
 * sub-queries of the given query (see {@link ElementQuery#getSubQuery(int)}. The unfolded iterator combines this
 * iterators by checking whether 1) the result sets need additional filtering (if so, a filter iterator is wrapped
 * around it) and 2) whether the final result set needs to be sorted and in what order. If the result set needs to be
 * sorted and the individual sub-query result sets aren't, then a PreSortingIterator is wrapped around the iterator
 * which effectively iterates the result set out, sorts it and then returns an iterator (i.e. much more expensive than
 * exploiting existing sort orders).</br>
 * In this way, the individual sub-result sets are prepared and then merged together the MergeSortIterator (which
 * conserves sort order if present). The semantics of the queries is OR, meaning the result sets are combined. However,
 * when {@link com.baidu.hugegraph.graphdb.query.ElementQuery#hasDuplicateResults()} is true (which assumes that the
 * result set is sorted) then the merge sort iterator filters out immediate duplicates.
 *
 *
 * @author Matthias Broecheler (me@matthiasb.com)
 */
public class QueryProcessor<Q extends ElementQuery<R, B>, R extends HugeGraphElement, B extends BackendQuery<B>>
    ...
    private Iterator<R> getUnfoldedIterator() {
        ...
        if (query.isSorted()) {
            for (int i = query.numSubQueries() - 1; i >= 0; i--) {
                ...
                Iterator<R> subqiter = getFilterIterator(
                        (subq.isSorted()) ? new LimitAdjustingIterator(subq) : new PreSortingIterator(subq),
                        hasDeletions, !subq.isFitted());

                iter = (iter == null) ? subqiter
                        : new ResultMergeSortIterator<R>(subqiter, iter, query.getSortOrder(),
                                query.hasDuplicateResults());
            }
            ...
            if (newElements.hasNext()) {
                ...
                iter = new ResultMergeSortIterator<R>(allNew.iterator(), iter, query.getSortOrder(),
                        query.hasDuplicateResults());
            }
        } else {
            final Set<R> allNew;
    ...
    private Iterator<R> getFilterIterator(final Iterator<R> iter, final boolean filterDeletions,
    ...
    private final class PreSortingIterator implements Iterator<R> {
        ...
        private PreSortingIterator(BackendQueryHolder<B> backendQueryHolder) {
            List<R> all = Lists.newArrayList(
                    executor.execute(query, backendQueryHolder.getBackendQuery().updateLimit(MAX_SORT_ITERATION),
    ...
    /*
     * TODO: Make the returned iterator smarter about limits: If less than LIMIT elements are returned, it checks if the
     * underlying iterators have been exhausted. If not, then it doubles the limit, discards the first count elements
     * and returns the remaining ones. Tricky bit: how to keep track of which iterators have been exhausted?
     */

    private final class LimitAdjustingIterator extends com.baidu.hugegraph.graphdb.query.LimitAdjustingIterator<R> {
