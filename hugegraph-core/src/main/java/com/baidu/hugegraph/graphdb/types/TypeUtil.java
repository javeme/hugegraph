...
public class TypeUtil {
    ...
    public static boolean hasSimpleInternalVertexKeyIndex(HugeGraphRelation rel) {
        if (!(rel instanceof HugeGraphVertexProperty))
            return false;
        else
            return hasSimpleInternalVertexKeyIndex((HugeGraphVertexProperty) rel);
    }
    ...
    public static boolean hasAnyIndex(PropertyKey key) {
        InternalRelationType type = (InternalRelationType) key;
        return !Iterables.isEmpty(type.getKeyIndexes()) || Iterables.size(type.getRelationIndexes()) > 1; // The type
                                                                                                          // itself is
                                                                                                          // also
                                                                                                          // returned as
                                                                                                          // an index
    ...
    private static <T> T getTypeModifier(final SchemaSource schema, final ModifierType modifierType,
