...
/**
 * An iterator implementation that wraps around another iterator to iterate it up to a given limit. The idea is that the
 * wrapped iterator is based on data that is fairly expensive to retrieve (e.g. from a database). As such, we don't want
 * to retrieve all of it but "just enough". However, if more data is requested, then we want the wrapped iterator to be
 * updated (i.e. additional data be retrieved).
 * </p>
...
public abstract class LimitAdjustingIterator<R> implements Iterator<R> {
    ...
    /**
     * Initializes this iterator with the current limit and the maximum number of elements that may be retrieved from
    ...
    public LimitAdjustingIterator(final int maxLimit, final int currentLimit) {
        Preconditions.checkArgument(currentLimit > 0 && maxLimit > 0, "Invalid limits: current [%s], max [%s]",
    ...
    public boolean hasNext() {
        ...
        /*
         * We need to iterate out the iterator to the point where we last left of. This is pretty expensive and hence it
         * should be ensured that the initial limit is a good guesstimate.
         */
