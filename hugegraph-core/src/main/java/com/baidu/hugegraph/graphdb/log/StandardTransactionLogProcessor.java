...
public class StandardTransactionLogProcessor implements TransactionRecovery {
    ...
    public StandardTransactionLogProcessor(StandardHugeGraph graph, Instant startTime) {
        ...
        this.verboseLogging =
                graph.getConfiguration().getConfiguration().get(GraphDatabaseConfiguration.VERBOSE_TX_RECOVERY);
        this.txCache = CacheBuilder.newBuilder().concurrencyLevel(2).initialCapacity(100)
                ...
                .removalListener(new RemovalListener<StandardTransactionId, TxEntry>() {
                    ...
                    public void onRemoval(RemovalNotification<StandardTransactionId, TxEntry> notification) {
                        ...
                        if (entry.status == LogTxStatus.SECONDARY_FAILURE
                                || entry.status == LogTxStatus.PRIMARY_SUCCESS) {
    ...
    private void fixSecondaryFailure(final StandardTransactionId txId, final TxEntry entry) {
        ...
        if (userLogFailure && logTxIdentifier != null) {
            TransactionLogHeader txHeader =
                    new TransactionLogHeader(txCounter.incrementAndGet(), times.getTime(), times);
    ...
    private class TxLogMessageReader implements MessageReader {
        ...
        public void read(Message message) {
            ...
            StandardTransactionId transactionId =
                    new StandardTransactionId(senderId, txheader.getId(), txheader.getTimestamp());
        ...
        synchronized void update(TransactionLogHeader.Entry e) {
            switch (e.getStatus()) {
                case PRECOMMIT:
                    entry = e;
                    if (status == null)
                        status = LogTxStatus.PRECOMMIT;
                    break;
                case PRIMARY_SUCCESS:
                    if (status == null || status == LogTxStatus.PRECOMMIT)
                        status = LogTxStatus.PRIMARY_SUCCESS;
                    break;
                case COMPLETE_SUCCESS:
                    if (status == null || status == LogTxStatus.PRECOMMIT)
                        status = LogTxStatus.COMPLETE_SUCCESS;
                    break;
    ...
    private class BackgroundCleaner extends BackgroundThread {
        ...
        protected void waitCondition() throws InterruptedException {
            if (lastInvocation != null)
                times.sleepPast(lastInvocation.plus(CLEAN_SLEEP_TIME));
        }
