...
package com.baidu.hugegraph.graphdb.database.idhandling;
...
public class IDHandler {
    ...
    public static int relationTypeLength(long relationTypeId) {
        ...
        return VariableLong.positiveWithPrefixLength(IDManager.stripEntireRelationTypePadding(relationTypeId) << 1,
                PREFIX_BIT_LEN);
    }

    /**
     * The edge type is written as follows: [ Invisible & System (2 bit) | Relation-Type-ID (1 bit) |
     * Relation-Type-Count (variable) | Direction-ID (1 bit)] Would only need 1 bit to store relation-type-id, but using
     * two so we can upper bound.
     *
     *
     * @param out
    ...
    public static void writeRelationType(WriteBuffer out, long relationTypeId, DirectionID dirID, boolean invisible) {
        ...
        VariableLong.writePositiveWithPrefix(out, strippedId,
                dirID.getPrefix(invisible, IDManager.isSystemRelationTypeId(relationTypeId)), PREFIX_BIT_LEN);
    }
