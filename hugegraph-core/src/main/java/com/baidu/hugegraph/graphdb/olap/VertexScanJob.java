 ...
 * (which need to be vertex-centric) for the data that is needed. Only this data is then pulled. If the user attempts to
 * access additional data during processing, the behavior is undefined.
 *
...
public interface VertexScanJob extends Cloneable {
    ...
    /**
     * @see com.baidu.hugegraph.diskstorage.keycolumnvalue.scan.ScanJob
     *
     * @param graph
     * @param config
     * @param metrics
     */
    public default void workerIterationStart(HugeGraph graph, Configuration config, ScanMetrics metrics) {
    }
    ...
    /**
     * Returns a clone of this VertexScanJob. The clone will not yet be initialized for computation but all of its
     * internal state (if any) must match that of the original copy.
     *
