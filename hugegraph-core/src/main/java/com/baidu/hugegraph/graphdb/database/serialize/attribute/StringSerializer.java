...
import com.baidu.hugegraph.graphdb.database.serialize.SupportsNullSerializer;
...
/**
 * Serializes Strings by trying to find the most efficient serialization format: 1) ASCII encoding (one byte per char)
 * 2) Full UTF encoding (for non-ASCII strings) 3) Using compression algorithms for long strings
 *
...
public class StringSerializer implements OrderPreservingSerializer<String>, SupportsNullSerializer {
    ...
    public void writeByteOrder(WriteBuffer buffer, String attribute) {
        ...
        for (int i = 0; i < attribute.length(); i++) {
            ...
            Preconditions.checkArgument(((int) c) > 0, "No null characters allowed in string @ position %s: %s", i,
                    attribute);
        ...
        if (compression == CompressionType.NO_COMPRESSION) {
            if ((length & 1) == 0) { // ASCII encoding
                ...
                if (length == 1)
                    ...
                    throw new IllegalArgumentException("Invalid ASCII encoding offset: " + length);
                ...
                for (int i = 0; i < length; i++) {
                    ...
                    switch (b >> 4) {
                        ...
                        case 13:
                            ...
                            break;
                        case 14:
                            sb.append((char) ((b & 0x0F) << 12 | (buffer.getByte() & 0x3F) << 6
                                    | buffer.getByte() & 0x3F));
                            break;
                    }
                }
        ...
        if (compression == CompressionType.NO_COMPRESSION) {
            ...
            if (StringEncoding.isAsciiString(attribute)) {
                if (attribute.length() == 0)
                    VariableLong.writePositive(buffer, 1l << NO_COMPRESSION_OFFSET);
                else
                    VariableLong.writePositive(buffer, 2l << NO_COMPRESSION_OFFSET);
                for (int i = 0; i < attribute.length(); i++) {
                ...
                VariableLong.writePositive(buffer,
                        (((long) attribute.length()) << NO_COMPRESSION_OFFSET) + (1l << COMPRESSOR_BIT_LEN)); // Marker
                                                                                                              // for
                                                                                                              // full
                                                                                                              // UTF
                                                                                                              // encoding
