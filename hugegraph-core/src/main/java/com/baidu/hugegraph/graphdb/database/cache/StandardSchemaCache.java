...
public class StandardSchemaCache implements SchemaCache {
    ...
    private static final int CACHE_RELATION_MULTIPLIER = 3; // 1) type-name, 2) type-definitions, 3) modifying edges
                                                            // [index, lock]
    ...
    private static final int SCHEMAID_TOTALFORW_SHIFT = 3; // Total number of bits appended - the 1 is for the 1 bit
                                                           // direction
    private static final int SCHEMAID_BACK_SHIFT = 2; // Number of bits to remove from end of schema id since its just
                                                      // the padding
    {
    ...
    public StandardSchemaCache(final int size, final StoreRetrieval retriever) {
        ...
        typeNamesBackup = CacheBuilder.newBuilder().concurrencyLevel(CONCURRENCY_LEVEL)
                .initialCapacity(INITIAL_CACHE_SIZE).maximumSize(maxCachedTypes).build();
        typeNames = new ConcurrentHashMap<String, Long>(INITIAL_CAPACITY, 0.75f, CONCURRENCY_LEVEL);

        schemaRelationsBackup = CacheBuilder.newBuilder().concurrencyLevel(CONCURRENCY_LEVEL)
                .initialCapacity(INITIAL_CACHE_SIZE * CACHE_RELATION_MULTIPLIER).maximumSize(maxCachedRelations)
                .build();
        // typeRelations = new ConcurrentHashMap<Long,
        // EntryList>(INITIAL_CAPACITY*CACHE_RELATION_MULTIPLIER,0.75f,CONCURRENCY_LEVEL);
        schemaRelations = new NonBlockingHashMapLong<EntryList>(INITIAL_CAPACITY * CACHE_RELATION_MULTIPLIER); // TODO:
                                                                                                               // Is
                                                                                                               // this
                                                                                                               // data
                                                                                                               // structure
        ...
        if (types == null) {
            ...
            if (id == null) { // Retrieve it
                if (types.size() > maxCachedTypes) {
                    /*
                     * Safe guard against the concurrent hash map growing to large - this would be a VERY rare event as
                     * it only happens for graph databases with thousands of types.
                     */
                    typeNames = null;
    ...
    public void expireSchemaElement(final long schemaId) {
        ...
        if (names != null) {
            for (Iterator<Map.Entry<String, Long>> iter = names.entrySet().iterator(); iter.hasNext();) {
                ...
                if (next.getValue().equals(schemaId))
                    iter.remove();
            }
        }
