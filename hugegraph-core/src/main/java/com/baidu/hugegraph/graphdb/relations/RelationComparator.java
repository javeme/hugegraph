...
/**
 * A {@link Comparator} for {@link HugeGraphRelation} that uses a defined order to compare the relations with or
 * otherwise uses the natural order of relations.
 *
...
public class RelationComparator implements Comparator<InternalRelation> {
    ...
    public int compare(final InternalRelation r1, final InternalRelation r2) {
        ...
        // Breakout: If type&direction are the same and the type is unique in the direction it follows that the
        ...
        if (r1.isProperty()) {
            Object o1 = ((HugeGraphVertexProperty) r1).value();
            Object o2 = ((HugeGraphVertexProperty) r2).value();
        ...
        // Breakout: if type&direction are the same, and the end points of the relation are the same and the type is
        // constrained, the relations must be the same
    ...
    public static int compareValues(Object v1, Object v2) {
        if (v1 == null || v2 == null) {
            ...
            Preconditions.checkArgument(v1 instanceof Comparable && v2 instanceof Comparable,
                    "Encountered invalid values");
            return ((Comparable) v1).compareTo(v2);
        }
    }

    private int compareOnKey(HugeGraphRelation r1, HugeGraphRelation r2, long typeid, Order order) {
        return compareOnKey(r1, r2, tx.getExistingPropertyKey(typeid), order);
    }

    private int compareOnKey(HugeGraphRelation r1, HugeGraphRelation r2, PropertyKey type, Order order) {
