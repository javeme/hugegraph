...
/**
 * Implementation of {@link HugeGraphMultiVertexQuery} that extends {@link BasicVertexCentricQueryBuilder} for all the
 * query building and optimization and adds only the execution logic in
 ...
 */
public class MultiVertexCentricQueryBuilder extends BasicVertexCentricQueryBuilder<MultiVertexCentricQueryBuilder>
        implements HugeGraphMultiVertexQuery<MultiVertexCentricQueryBuilder> {
     ...
     * {@link BasicVertexCentricQueryBuilder#constructQuery(com.baidu.hugegraph.graphdb.internal.RelationCategory)}. If
     * the query asks for an implicit key, the resulting map is computed and returned directly. If the query is empty, a
     * map that maps each vertex to an empty list is returned. Otherwise, the query is executed for all vertices through
     * the transaction which will effectively pre-load the return result sets into the associated
     * {@link com.baidu.hugegraph.graphdb.vertices.CacheVertex} or don't do anything at all if the vertex is new (and
     * hence no edges in the storage backend). After that, a map is constructed that maps each vertex to the
     * corresponding VertexCentricQuery and wrapped into a QueryProcessor. Hence, upon iteration the query will be
     * executed like any other VertexCentricQuery with the performance difference that the SliceQueries will have
     * already been preloaded and not further calls to the storage backend are needed.
     *
     * @param returnType
     * @return
     */
    ...
    public Map<HugeGraphVertex, Iterable<? extends HugeGraphRelation>> executeImplicitKeyQuery() {
        return new HashMap<HugeGraphVertex, Iterable<? extends HugeGraphRelation>>(vertices.size()) {
            {
                for (InternalVertex v : vertices)
                    put(v, executeImplicitKeyQuery(v));
            }
        };
    }
    ...
    public Map<HugeGraphVertex, Iterable<HugeGraphVertexProperty>> properties() {
        return (Map) (isImplicitKeyQuery(RelationCategory.PROPERTY) ? executeImplicitKeyQuery()
                : execute(RelationCategory.PROPERTY, new RelationConstructor()));
    }

    @Override
    public Map<HugeGraphVertex, Iterable<HugeGraphRelation>> relations() {
        return (Map) (isImplicitKeyQuery(RelationCategory.RELATION) ? executeImplicitKeyQuery()
