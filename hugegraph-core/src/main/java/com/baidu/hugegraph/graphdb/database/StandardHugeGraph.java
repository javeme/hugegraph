...
import com.baidu.hugegraph.graphdb.database.management.ManagementLogger;
...
import com.baidu.hugegraph.graphdb.internal.InternalVertexLabel;
import com.baidu.hugegraph.graphdb.query.QueryUtil;
import com.baidu.hugegraph.graphdb.relations.EdgeDirection;
import com.baidu.hugegraph.graphdb.tinkerpop.HugeGraphBlueprintsGraph;
import com.baidu.hugegraph.graphdb.tinkerpop.HugeGraphFeatures;
import com.baidu.hugegraph.graphdb.tinkerpop.optimize.AdjacentVertexFilterOptimizerStrategy;
import com.baidu.hugegraph.graphdb.tinkerpop.optimize.HugeGraphStepStrategy;
...
import com.baidu.hugegraph.graphdb.util.ExceptionFactory;
import com.baidu.hugegraph.util.system.IOUtils;
import com.baidu.hugegraph.util.system.TXUtils;
...
import static com.baidu.hugegraph.graphdb.configuration.GraphDatabaseConfiguration.REGISTRATION_TIME;
...
public class StandardHugeGraph extends HugeGraphBlueprintsGraph {
    ...
    private Set<StandardHugeGraphTx> openTransactions;

    public StandardHugeGraph(GraphDatabaseConfiguration configuration) {
    ...
    @Override
    public synchronized void close() throws HugeGraphException {
        try {
            closeInternal();
        } finally {
            removeHook();
    ...
    @Override
    public HugeGraphTransaction newThreadBoundTransaction() {
    ...
    public StandardHugeGraphTx newTransaction(final TransactionConfiguration configuration) {
        ...
        try {
            ...
            throw new HugeGraphException("Could not start new transaction", e);
        }
    }

    private BackendTransaction openBackendTransaction(StandardHugeGraphTx tx) throws BackendException {
    ...
    public void closeTransaction(StandardHugeGraphTx tx) {
    ...
    private static class ShutdownThread extends Thread {
        ...
        public ShutdownThread(StandardHugeGraph graph) {
            this.graph = graph;
        }
