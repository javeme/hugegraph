...
/**
 * An instance of this class describes a range of partition ids. This range if defined by a lower id (inclusive) and
 * upper id (exclusive). When lowerId < upperId this partition range is called a proper range since it describes the
 * contiguous block of ids from lowerId until upperId. When lowerId >= upperID then the partition block "wraps around"
 * the specified idUpperBound. In other words, it describes the ids from [lowerId,idUpperBound) AND [0,upperId).
 *
...
public class PartitionIDRange {
    ...
    public PartitionIDRange(int lowerID, int upperID, int idUpperBound) {
        ...
        Preconditions.checkArgument(idUpperBound <= Integer.MAX_VALUE,
                "Partition limit cannot exceed representable range of an integer");
        Preconditions.checkArgument(lowerID >= 0, "Negative partition lower bound " + lowerID);
        Preconditions.checkArgument(lowerID < idUpperBound,
                "Partition lower bound " + lowerID + " exceeds limit " + idUpperBound);
        Preconditions.checkArgument(upperID >= 0, "Negative partition upper bound " + upperID);
        Preconditions.checkArgument(upperID <= idUpperBound,
                "Partition upper bound " + upperID + " exceeds limit " + idUpperBound);
    ...
    public int getRandomID() {
        ...
        int partitionWidth;
        if (lowerID < upperID)
            partitionWidth = upperID - lowerID; // ... for "proper" ranges
        else
            partitionWidth = (idUpperBound - lowerID) + upperID; // ... and those that "wrap around"
    ...
    public static List<PartitionIDRange> getGlobalRange(final int partitionBits) {
        Preconditions.checkArgument(partitionBits >= 0 && partitionBits < (Integer.SIZE - 1),
                "Invalid partition bits: %s", partitionBits);
    ...
    public static List<PartitionIDRange> getIDRanges(final int partitionBits, final List<KeyRange> locals) {
        ...
        for (KeyRange local : locals) {
            ...
            if (truncatedBits)
                lowerID += 1; // adjust to make sure we are inclusive
    ...
    private static void discardRange(KeyRange local) {
        log.warn("Individual key range is too small for partition block - result would be empty; hence ignored: {}",
                local);
    }
