...
public class StandardHugeGraphTx extends HugeGraphBlueprintsTransaction
        implements TypeInspector, SchemaInspector, VertexFactory {
    ...
    /**
     * This is a workaround for #893. Cache sizes small relative to the level of thread parallelism can lead to
     * HugeGraph generating multiple copies of a single vertex in a single transaction.
     */
    ...
    // ######## Index Caches
    /**
     * Caches the result of index calls so that repeated index queries don't need to be passed to the IndexProvider.
     * This cache will drop entries when it overflows since the result set can always be retrieved from the
     * IndexProvider
     */
    private final Cache<JointIndexQuery.Subquery, List<Object>> indexCache;
    /**
     * Builds an inverted index for newly added properties so they can be considered in index queries. This cache my not
     * release elements since that would entail an expensive linear scan over addedRelations
     */
     ...
     * Since type retrieval by name is common and there are only a few types, since cache is a simple map (i.e. no
     * release)
     */
    private final Map<String, Long> newTypeCache;

    /**
     * Used to assign temporary ids to new vertices and relations added in this transaction. If ids are assigned
     * immediately, this is not used. This IDPool is shared across all elements.
     */
    ...
    public StandardHugeGraphTx(StandardHugeGraph graph, TransactionConfiguration config) {
        ...
        if (!config.isReadOnly()) {
            ...
            log.debug("Guava vertex cache size: requested={} effective={} (min={})", config.getVertexCacheSize(),
                    effectiveVertexCacheSize, MIN_VERTEX_CACHE_SIZE);
        }
        ...
        if (null != config.getGroupName()) {
            ...
            elementProcessor = new MetricsQueryExecutor<GraphCentricQuery, HugeGraphElement, JointIndexQuery>(
                    config.getGroupName(), "graph", elementProcessorImpl);
            edgeProcessor = new MetricsQueryExecutor<VertexCentricQuery, HugeGraphRelation, SliceQuery>(
                    config.getGroupName(), "vertex", edgeProcessorImpl);
        } else {
    ...
    public StandardHugeGraphTx getNextTx() {
        ...
        if (!config.isThreadBound())
            ...
            return (StandardHugeGraphTx) graph.getCurrentThreadTx();
    }
    ...
    public InternalVertex getCanonicalVertex(InternalVertex partitionedVertex) {
        ...
        if (canonicalId == partitionedVertex.longId())
            ...
            return getExistingVertex(canonicalId);
    }
    ...
    public InternalVertex[] getAllRepresentatives(HugeGraphVertex partitionedVertex, boolean restrict2Partitions) {
        ...
        for (int i = 0; i < ids.length; i++)
            vertices[i] = getExistingVertex(ids[i]);
        return vertices;
    }
    ...
    private class VertexConstructor implements Retriever<Long, InternalVertex> {
        ...
        public InternalVertex get(Long vertexid) {
            ...
            Preconditions.checkArgument(idInspector.isSchemaVertexId(vertexid) || idInspector.isUserVertexId(vertexid),
                    "Not a valid vertex id: %s", vertexid);

            byte lifecycle = ElementLifeCycle.Loaded;
            long canonicalVertexId =
                    idInspector.isPartitionedVertex(vertexid) ? idManager.getCanonicalVertexId(vertexid) : vertexid;
            if (verifyExistence) {
            ...
            if (idInspector.isRelationTypeId(vertexid)) {
                ...
                if (createStubVertex)
                    vertex = new PreloadedVertex(StandardHugeGraphTx.this, vertexid, lifecycle);
                else
                    vertex = new CacheVertex(StandardHugeGraphTx.this, vertexid, lifecycle);
            } else
    ...
    public HugeGraphVertex addVertex(Long vertexId, VertexLabel label) {
        ...
        Preconditions.checkArgument(vertexId == null || IDManager.VertexIDType.NormalVertex.is(vertexId),
                "Not a valid vertex id: %s", vertexId);
        Preconditions.checkArgument(vertexId == null || ((InternalVertexLabel) label).hasDefaultConfiguration(),
                "Cannot only use default vertex labels: %s", label);
        Preconditions.checkArgument(
                vertexId == null || !config.hasVerifyExternalVertexExistence() || !containsVertex(vertexId),
                "Vertex with given id already exists: %s", vertexId);
    ...
    private Iterable<InternalVertex> getInternalVertices() {
        ...
        if (!addedRelations.isEmpty()) {
            ...
            while (viter.hasNext()) {
                if (viter.next() instanceof HugeGraphSchemaElement)
                    viter.remove();
            }
        ...
        return Iterables.filter(allVertices, new Predicate<InternalVertex>() {
            ...
            public boolean apply(@Nullable InternalVertex internalVertex) {
                return !isPartitionedVertex(internalVertex)
                        || internalVertex.longId() == idInspector.getCanonicalVertexId(internalVertex.longId());
            }
        });
    }
            ...
            if (!attribute.getClass().equals(datatype)) {
                ...
                if (converted == null)
                    throw new SchemaViolationException(
                            "Value [%s] is not an instance of the expected data type for property key [%s] and cannot be converted. Expected: %s, found: %s",
                            attribute, key.name(), datatype, attribute.getClass());
                attribute = converted;
            }
    ...
    public void removeRelation(InternalRelation relation) {
        ...
        if (relation.isNew()) {
            ...
            if (TypeUtil.hasSimpleInternalVertexKeyIndex(relation))
                newVertexIndexEntries.remove((HugeGraphVertexProperty) relation);
        } else {
    ...
    private TransactionLock getLock(final LockTuple la) {
        ...
        // TODO: clean out no longer used locks from uniqueLocks when it grows to large (use ReadWriteLock to protect
    ...
    private TransactionLock getUniquenessLock(final HugeGraphVertex out, final InternalRelationType type,
            final Object in) {
    ...
    public HugeGraphEdge addEdge(HugeGraphVertex outVertex, HugeGraphVertex inVertex, EdgeLabel label) {
        ...
        try {
            ...
            if (config.hasVerifyUniqueness()) {
                if (multiplicity == Multiplicity.SIMPLE) {
                    if (!Iterables
                            .isEmpty(query(outVertex).type(label).direction(Direction.OUT).adjacent(inVertex).edges()))
                        throw new SchemaViolationException(
                                "An edge with the given label already exists between the pair of vertices and the label [%s] is simple",
                                label.name());
                }
                if (multiplicity.isUnique(Direction.OUT)) {
                    if (!Iterables.isEmpty(query(outVertex).type(label).direction(Direction.OUT).edges()))
                        throw new SchemaViolationException(
                                "An edge with the given label already exists on the out-vertex and the label [%s] is out-unique",
                                label.name());
                }
                if (multiplicity.isUnique(Direction.IN)) {
                    if (!Iterables.isEmpty(query(inVertex).type(label).direction(Direction.IN).edges()))
                        throw new SchemaViolationException(
                                "An edge with the given label already exists on the in-vertex and the label [%s] is in-unique",
                                label.name());
                }
            }
            StandardEdge edge = new StandardEdge(IDManager.getTemporaryRelationID(temporaryIds.nextID()), label,
                    (InternalVertex) outVertex, (InternalVertex) inVertex, ElementLifeCycle.New);
    ...
    private void connectRelation(InternalRelation r) {
        for (int i = 0; i < r.getLen(); i++) {
            ...
            if (!success)
                throw new AssertionError("Could not connect relation: " + r);
        }
        addedRelations.add(r);
        for (int pos = 0; pos < r.getLen(); pos++)
            vertexCache.add(r.getVertex(pos), r.getVertex(pos).longId());
        if (TypeUtil.hasSimpleInternalVertexKeyIndex(r))
            newVertexIndexEntries.add((HugeGraphVertexProperty) r);
    }
    ...
    public HugeGraphVertexProperty addProperty(VertexProperty.Cardinality cardi, HugeGraphVertex vertex,
            PropertyKey key, Object value) {
        if (key.cardinality().convert() != cardi && cardi != VertexProperty.Cardinality.single)
            throw new SchemaViolationException(String.format(
                    "Key is defined for %s cardinality which conflicts with specified: %s", key.cardinality(), cardi));
        verifyWriteAccess(vertex);
        Preconditions.checkArgument(!(key instanceof ImplicitKey), "Cannot create a property of implicit type: %s",
                key.name());
        ...
        for (IndexLockTuple lockTuple : uniqueIndexTuples)
            uniqueLock = new CombinerLock(uniqueLock, getLock(lockTuple), times);
        uniqueLock.lock(LOCK_TIMEOUT);
        try {
            ...
            // throw new SchemaViolationException("A property with the given key [%s] already exists on the vertex [%s]
            // and the property key is defined as single-valued", key.name(), vertex);
            // }
            // if (cardinality == Cardinality.SET) {
            // if (!Iterables.isEmpty(Iterables.filter(query(vertex).type(key).properties(), new
            // Predicate<HugeGraphVertexProperty>() {
            // @Override
            ...
            // }
            // })))
            // throw new SchemaViolationException("A property with the given key [%s] and value [%s] already exists on
            // the vertex and the property key is defined as set-valued", key.name(), normalizedValue);
            // }
            // }
            ...
            if (cardi == VertexProperty.Cardinality.single || cardi == VertexProperty.Cardinality.set) {
                ...
                if (cardi == VertexProperty.Cardinality.single)
                    ...
                    propertyRemover = p -> {
                        if (((HugeGraphVertexProperty) p).value().equals(normalizedValue))
                            p.remove();
                    };

                /*
                 * If we are simply overwriting a vertex property, then we don't have to explicitly remove it thereby
                 * saving a read operation However, this only applies if 1) we don't lock on the property key or
                 * consistency checks are disabled and 2) there are no indexes for this property key 3) the
                 * cardinalities match (if we overwrite a set with single, we need to read all other values to delete)
                 */
                ...
                if ((!config.hasVerifyUniqueness()
                    ...
                    // Only delete in-memory so as to not trigger a read from the database which isn't necessary because
                    // we will overwrite blindly
            ...
            if (config.hasVerifyUniqueness()) {
                ...
                for (IndexLockTuple lockTuple : uniqueIndexTuples) {
                    if (!Iterables.isEmpty(IndexHelper.getQueryResults(lockTuple.getIndex(), lockTuple.getAll(), this)))
                        throw new SchemaViolationException(
                                "Adding this property for key [%s] and value [%s] violates a uniqueness constraint [%s]",
                                key.name(), normalizedValue, lockTuple.getIndex());
                }
            }
            StandardVertexProperty prop =
                    new StandardVertexProperty(IDManager.getTemporaryRelationID(temporaryIds.nextID()), key,
                            (InternalVertex) vertex, normalizedValue, ElementLifeCycle.New);
            if (config.hasAssignIDsImmediately())
                graph.assignID(prop);
    ...
    public final HugeGraphSchemaVertex makeSchemaVertex(HugeGraphSchemaCategory schemaCategory, String name,
            TypeDefinitionMap definition) {
        verifyOpen();
        Preconditions.checkArgument(!schemaCategory.hasName() || StringUtils.isNotBlank(name),
                "Need to provide a valid name for type [%s]", schemaCategory);
        ...
        if (schemaCategory.isRelationType()) {
            if (schemaCategory == HugeGraphSchemaCategory.PROPERTYKEY) {
                schemaVertex = new PropertyKeyVertex(this,
                        IDManager.getTemporaryVertexID(IDManager.VertexIDType.UserPropertyKey, temporaryIds.nextID()),
                        ElementLifeCycle.New);
            } else {
                assert schemaCategory == HugeGraphSchemaCategory.EDGELABEL;
                schemaVertex = new EdgeLabelVertex(this,
                        IDManager.getTemporaryVertexID(IDManager.VertexIDType.UserEdgeLabel, temporaryIds.nextID()),
                        ElementLifeCycle.New);
            }
        } else if (schemaCategory == HugeGraphSchemaCategory.VERTEXLABEL) {
            schemaVertex = new VertexLabelVertex(this,
                    IDManager.getTemporaryVertexID(IDManager.VertexIDType.GenericSchemaType, temporaryIds.nextID()),
                    ElementLifeCycle.New);
        } else {
            schemaVertex = new HugeGraphSchemaVertex(this,
                    IDManager.getTemporaryVertexID(IDManager.VertexIDType.GenericSchemaType, temporaryIds.nextID()),
                    ElementLifeCycle.New);
        }
    ...
    public void updateSchemaVertex(HugeGraphSchemaVertex schemaVertex) {
        addProperty(VertexProperty.Cardinality.single, schemaVertex, BaseKey.SchemaUpdateTime,
                times.getTime(times.getTime()));
    }
    ...
    public PropertyKey getPropertyKey(String name) {
        ...
        Preconditions.checkArgument(pk == null || pk.isPropertyKey(),
    ...
    public EdgeLabel getEdgeLabel(String name) {
        ...
        Preconditions.checkArgument(el == null || el.isEdgeLabel(),
                "The relation type with name [%s] is not an edge label", name);
        return (EdgeLabel) el;
    }
    ...
    public void executeMultiQuery(final Collection<InternalVertex> vertices, final SliceQuery sq,
        ...
        if (!vids.isEmpty()) {
            List<EntryList> results =
                    QueryProfiler.profile(profiler, sq, true, q -> graph.edgeMultiQuery(vids, q, txHandle));
            int pos = 0;
            for (HugeGraphVertex v : vertices) {
    ...
    public final QueryExecutor<VertexCentricQuery, HugeGraphRelation, SliceQuery> edgeProcessorImpl =
            new QueryExecutor<VertexCentricQuery, HugeGraphRelation, SliceQuery>() {
                @Override
                ...
                public boolean hasDeletions(VertexCentricQuery query) {
                    ...
                    if (vertex.hasRemovedRelations() || vertex.hasAddedRelations())
                        return true;
                    return false;
                }
                ...
                @Override
                public Iterator<HugeGraphRelation> execute(final VertexCentricQuery query, final SliceQuery sq,
                    ...
                    EntryList iter = v.loadRelations(sq, new Retriever<SliceQuery, EntryList>() {
                        ...
                        public EntryList get(SliceQuery query) {
                            return QueryProfiler.profile(profiler, query,
                                    q -> graph.edgeQuery(v.longId(), q, txHandle));
                        }
                    });
    ...
    public final QueryExecutor<GraphCentricQuery, HugeGraphElement, JointIndexQuery> elementProcessorImpl =
            new QueryExecutor<GraphCentricQuery, HugeGraphElement, JointIndexQuery>() {

                private PredicateCondition<PropertyKey, HugeGraphElement> getEqualityCondition(
                    ...
                    if (condition instanceof PredicateCondition) {
                        ...
                        if (pc.getPredicate() == Cmp.EQUAL && TypeUtil.hasSimpleInternalVertexKeyIndex(pc.getKey()))
                            return pc;
                ...
                public Iterator<HugeGraphElement> getNew(final GraphCentricQuery query) {
                    // If the query is unconstrained then we don't need to add new elements, so will be picked up by
                    // getVertices()/getEdges() below
                    if (query.numSubQueries() == 1 && query.getSubQuery(0).getBackendQuery().isEmpty())
                        return Collections.emptyIterator();
                    Preconditions.checkArgument(query.getCondition().hasChildren(),
                            "If the query is non-empty it needs to have a condition");
                    ...
                    if (query.getResultType() == ElementCategory.VERTEX && hasModifications()) {
                        ...
                        PredicateCondition<PropertyKey, HugeGraphElement> standardIndexKey =
                                getEqualityCondition(query.getCondition());
                        ...
                        if (standardIndexKey == null) {
                            ...
                            ConditionUtil.traversal(query.getCondition(), new Predicate<Condition<HugeGraphElement>>() {
                                ...
                                public boolean apply(@Nullable Condition<HugeGraphElement> cond) {
                                    Preconditions.checkArgument(cond.getType() != Condition.Type.LITERAL
                                            || cond instanceof PredicateCondition);
                                    if (cond instanceof PredicateCondition)
                                        keys.add(((PredicateCondition<PropertyKey, HugeGraphElement>) cond).getKey());
                                    return true;
                                }
                            });
                            Preconditions.checkArgument(!keys.isEmpty(), "Invalid query condition: %s",
                                    query.getCondition());
                            ...
                            vertices = com.google.common.collect.Iterators.transform(newVertexIndexEntries
                                    .get(standardIndexKey.getValue(), standardIndexKey.getKey()).iterator(),
                                    new Function<HugeGraphVertexProperty, HugeGraphVertex>() {
                                        @Nullable
                                        @Override
                        ...
                        return (Iterator) com.google.common.collect.Iterators.filter(vertices,
                                new Predicate<HugeGraphVertex>() {
                                    @Override
                            ...
                            || query.getResultType() == ElementCategory.PROPERTY) && !addedRelations.isEmpty()) {
                        return (Iterator) addedRelations.getView(new Predicate<InternalRelation>() {
                            @Override
                            public boolean apply(@Nullable InternalRelation relation) {
                                return query.getResultType().isInstance(relation) && !relation.isInvisible()
                ...
                public boolean isDeleted(GraphCentricQuery query, HugeGraphElement result) {
                    if (result == null || result.isRemoved())
                        ...
                        if (v.hasAddedRelations() || v.hasRemovedRelations()) {
                            ...
                            || query.getResultType() == ElementCategory.PROPERTY) {
                        Preconditions.checkArgument(result.isLoaded() || result.isNew());
                        // Loaded relations are immutable so we don't need to check those
                        // New relations could be modified in this transaction to now longer match the query, hence we
                        // need to
                ...
                @Override
                public Iterator<HugeGraphElement> execute(final GraphCentricQuery query,
                        final JointIndexQuery indexQuery, final Object exeInfo, final QueryProfiler profiler) {
                    ...
                    if (!indexQuery.isEmpty()) {
                                    ...
                                    try {
                                        return indexCache.get(adjustedQuery, new Callable<List<Object>>() {
                                            ...
                                            public List<Object> call() throws Exception {
                                                return QueryProfiler.profile(subquery.getProfiler(), adjustedQuery,
                                                        q -> indexSerializer.query(q, txHandle));
                                            }
                                        });
                                    } catch (Exception e) {
                        ...
                        List<Object> resultSet =
                                QueryUtil.processIntersectingRetrievals(retrievals, indexQuery.getLimit());
                        iter = com.google.common.collect.Iterators.transform(resultSet.iterator(),
                                getConversionFunction(query.getResultType()));
                    } else {
                        if (config.hasForceIndexUsage())
                            throw new HugeGraphException(
                                    "Could not find a suitable index to answer graph query and graph scans are disabled: "
                                            + query);
                        log.warn("Query requires iterating over all vertices [{}]. For better performance, use indexes",
                                query.getCondition());
                        ...
                        switch (query.getResultType()) {
                            ...
                            case PROPERTY:
                                return new VertexCentricEdgeIterable(getInternalVertices(), RelationCategory.PROPERTY)
                                        .iterator();
    ...
    private final Function<Object, HugeGraphVertexProperty> propertyIDConversionFct =
            new Function<Object, HugeGraphVertexProperty>() {
                @Override
