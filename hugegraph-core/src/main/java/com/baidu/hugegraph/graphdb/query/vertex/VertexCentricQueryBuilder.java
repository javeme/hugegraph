...
import com.baidu.hugegraph.graphdb.query.QueryProcessor;
import com.baidu.hugegraph.graphdb.query.profile.QueryProfiler;

import java.util.List;

/**
 * Implementation of {@link HugeGraphVertexQuery} that extends {@link BasicVertexCentricQueryBuilder} for all the query
 * building and optimization and adds only the execution logic in
 * {@link #constructQuery(com.baidu.hugegraph.graphdb.internal.RelationCategory)}. However, there is one important
 * special case: If the constructed query is simple (i.e.
 * {@link com.baidu.hugegraph.graphdb.query.vertex.VertexCentricQuery#isSimple()=true} then we use the
 * {@link SimpleVertexQueryProcessor} to execute the query instead of the generic {@link QueryProcessor} for performance
 * reasons and we compute the result sets differently to make things faster and more memory efficient.
 * </p>
 * The simplified vertex processing only applies to loaded (i.e. non-mutated) vertices. The query can be configured to
 * only included loaded relations in the result set (which is needed, for instance, when computing index deltas in
 * {@link com.baidu.hugegraph.graphdb.database.IndexSerializer}) via {@link #queryOnlyLoaded()}.
 * </p>
 * All other methods just prepare or transform that result set to fit the particular method semantics.
 *
 * @author Matthias Broecheler (me@matthiasb.com)
 */
public class VertexCentricQueryBuilder extends BasicVertexCentricQueryBuilder<VertexCentricQueryBuilder>
        implements HugeGraphVertexQuery<VertexCentricQueryBuilder> {
    ...
    protected <Q> Q execute(RelationCategory returnType, ResultConstructor<Q> resultConstructor) {
        ...
        if (isPartitionedVertex(vertex) && !hasQueryOnlyGivenVertex()) { // If it's a preloaded vertex we shouldn't
    ...
    public Iterable<HugeGraphVertexProperty> properties() {
        return (Iterable) (isImplicitKeyQuery(RelationCategory.PROPERTY) ? executeImplicitKeyQuery(vertex)
                : execute(RelationCategory.PROPERTY, new RelationConstructor()));
    }

    @Override
    public Iterable<HugeGraphRelation> relations() {
        return (Iterable) (isImplicitKeyQuery(RelationCategory.RELATION) ? executeImplicitKeyQuery(vertex)
