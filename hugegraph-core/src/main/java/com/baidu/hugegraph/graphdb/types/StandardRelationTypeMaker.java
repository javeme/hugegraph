...
import com.baidu.hugegraph.core.schema.RelationTypeMaker;
...
public abstract class StandardRelationTypeMaker implements RelationTypeMaker {
    ...
    public StandardRelationTypeMaker(final StandardHugeGraphTx tx, String name, final IndexSerializer indexSerializer,
    ...
    abstract HugeGraphSchemaCategory getSchemaCategory();
    ...
    private static long[] checkSignature(List<PropertyKey> sig) {
        Preconditions.checkArgument(sig.size() == (Sets.newHashSet(sig)).size(),
                "Signature and sort key cannot contain duplicate types");
     ...
     * <p/>
     * Specifying the sort key of a type allows relations of this type to be efficiently retrieved in the order of the
     * sort key. <br />
     * For instance, if the edge label <i>friend</i> has the sort key (<i>since</i>), which is a property key with a
     * timestamp data type, then one can efficiently retrieve all edges with label <i>friend</i> in a specified time
     * interval using
     * {@link com.baidu.hugegraph.core.HugeGraphVertexQuery#interval(com.baidu.hugegraph.core.PropertyKey, Comparable, Comparable)}.
     * <br />
     * In other words, relations are stored on disk in the order of the configured sort key. The sort key is empty by
     * default. <br />
     * If multiple types are specified as sort key, then those are considered as a <i>composite</i> sort key, i.e. taken
     * jointly in the given order.
     * <p/>
     * {@link com.baidu.hugegraph.core.RelationType}s used in the sort key must be either property out-unique keys or
     * out-unique unidirected edge lables.
     *
     * @param keys HugeGraphTypes composing the sort key. The order is relevant.
    ...
    /**
     * Defines in which order to sort the relations for efficient retrieval, i.e. either increasing
     * ({@link com.baidu.hugegraph.graphdb.internal.Order#ASC}) or decreasing
     * ({@link com.baidu.hugegraph.graphdb.internal.Order#DESC}).
     *
