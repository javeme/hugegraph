...
public class GraphDatabaseConfiguration {
    ...
    public static ConfigNamespace ROOT_NS = new ConfigNamespace(null,"root","Root Configuration Namespace for the HugeGraph Graph Database");
...
//            ConfigOption.Type.MASKABLE, KryoInstanceCacheImpl.class, KryoInstanceCacheImpl.THREAD_LOCAL);
    ...
    public static final ConfigOption<String> INITIAL_hugegraph_VERSION = new ConfigOption<String>(GRAPH_NS,"hugegraph-version",
    ...
    public static ConfigNamespace JOB_NS = new ConfigNamespace(null,"job","Root Configuration Namespace for HugeGraph OLAP jobs");
    ...
    public static final ConfigNamespace CACHE_NS = new ConfigNamespace(ROOT_NS,"cache","Configuration options that modify HugeGraph's caching behavior");
     ...
     * percentage of the total heap space available to the JVM this HugeGraph instance is running in.
...
//    public static final long DB_CACHE_CLEAN_WAIT_DEFAULT = 50;
     ...
     * HugeGraph will check against storage backend for a newer query answer.
     * Setting this value to 0 will cache elements forever (unless they get evicted due to space constraints). This only
     * makes sense when this is the only HugeGraph instance interacting with a storage backend.
     */
...
//    public static final String STORAGE_CONF_FILE_KEY = "conffile";
    ...
    public static final ConfigOption<String> STORAGE_BACKEND = new ConfigOption<String>(STORAGE_NS,"backend",
            ...
            "HugeGraph's built-in shorthand names for its standard storage backends " +
...
//    public static final int BUFFER_SIZE_DEFAULT = 1024;
     ...
     * temporary such as network failures. For temporary failures, HugeGraph will re-attempt to persist the
...
//    public static final int WRITE_ATTEMPTS_DEFAULT = 5;
     ...
     * temporary such as network failures. For temporary failures, HugeGraph will re-attempt to read the
...
//    public static final int READ_ATTEMPTS_DEFAULT = 3;
    ...
    /**
     * If enabled, HugeGraph attempts to parallelize storage operations against the storage backend using a fixed thread pool shared
     * across the entire HugeGraph graph database instance. Parallelization is only applicable to certain storage operations and
    ...
    public static final ConfigOption<Boolean> PARALLEL_BACKEND_OPS = new ConfigOption<Boolean>(STORAGE_NS,"parallel-backend-ops",
            "Whether HugeGraph should attempt to parallelize storage operations",
            ConfigOption.Type.MASKABLE, true);
...
//    public static final String CONNECTION_TIMEOUT_KEY = "connection-timeout";
     ...
     * become available when HugeGraph is run in server mode. Should the backend manager
...
//    public static final boolean IDS_FLUSH_DEFAULT = true;
    ...
    /**
     * The number of milliseconds that the HugeGraph id pool manager will wait before giving up on allocating a new block
...
//    public static final long IDAUTHORITY_WAIT_MS_DEFAULT = 300;
     ...
     * contention in ID block alloction between HugeGraph instances concurrently
    ...
    /**
     * When HugeGraph allocates IDs with {@link com.baidu.hugegraph.diskstorage.idmanagement.ConflictAvoidanceMode#GLOBAL_AUTO}
     ...
     * HugeGraph will generate a new random unique ID and try again. This controls
     * the maximum number of attempts before HugeGraph assumes the entire partition
...
//    public static final int IDAUTHORITY_RETRY_COUNT_DEFAULT = 20;
    ...
    /**
     * Configures the number of bits of HugeGraph assigned ids that are reserved for a unique id marker that
     * allows the id allocation to be scaled over multiple sub-clusters and to reduce race-conditions
     * when a lot of HugeGraph instances attempt to allocate ids at the same time (e.g. during parallel bulk loading)
     *
     * IMPORTANT: This should never ever, ever be modified from its initial value and ALL HugeGraph instances must use the
    ...
    /**
     * Unique id marker to be used by this HugeGraph instance when allocating ids. The unique id marker
     * must be non-negative and fit within the number of unique id bits configured.
     * By assigning different unique id markers to individual HugeGraph instances it can be assured
    ...
    public static final ConfigOption<String> INDEX_BACKEND = new ConfigOption<String>(INDEX_NS,"backend",
            ...
            "HugeGraph's built-in shorthand names for its standard index backends " +
...
//    public static final String INDEX_BACKEND_DEFAULT = "lucene";
    ...
    public static final ConfigOption<String> INDEX_NAME = new ConfigOption<String>(INDEX_NS,"index-name",
            ...
            ConfigOption.Type.GLOBAL_OFFLINE, "hugegraph");
    ...
    public static final ConfigNamespace LOG_NS = new ConfigNamespace(GraphDatabaseConfiguration.ROOT_NS,"log","Configuration options for HugeGraph's logging system",true);
...
//    public static final boolean BASIC_METRICS_DEFAULT = false;
    ...
    /**
     * The default name prefix for Metrics reported by HugeGraph. All metric names
    ...
    public static final ConfigOption<String> METRICS_PREFIX = new ConfigOption<String>(METRICS_NS,"prefix",
            "The default name prefix for Metrics reported by HugeGraph.",
...
//    public static final String METRICS_PREFIX_KEY = "prefix";
     ...
     * measures the sum of HugeGraph's backend activity without distinguishing
...
//    public static final String GRAPHITE_PREFIX_DEFAULT = null;
    ...
    public static final ConfigOption<String> GREMLIN_GRAPH = new ConfigOption<String>(GREMLIN_NS, "graph",
            "The implementation of graph factory that will be used by gremlin server", ConfigOption.Type.LOCAL, "com.baidu.hugegraph.core.HugeGraphFactory");
...
//        KeyColumnValueStoreManager storeManager=null;
        ...
        try {
            ...
            if (!globalWrite.isFrozen()) {
                ...
                Preconditions.checkArgument(!globalWrite.has(INITIAL_hugegraph_VERSION),"Database has already been initialized but not frozen");
                globalWrite.set(INITIAL_hugegraph_VERSION,HugeGraphConstants.VERSION);
                ...
                String version = globalWrite.get(INITIAL_hugegraph_VERSION);
                Preconditions.checkArgument(version!=null,"HugeGraph version has not been initialized");
                if (!HugeGraphConstants.VERSION.equals(version) && !HugeGraphConstants.COMPATIBLE_VERSIONS.contains(version)) {
                    throw new HugeGraphException("StorageBackend version is incompatible with current HugeGraph version: storage=" + version + " vs. runtime=" + HugeGraphConstants.VERSION);
                }
                ...
                if (0 < optionsWithDiscrepancies.size() && !managedOverridesAllowed) {
                    ...
                    throw new HugeGraphConfigurationException(String.format(template, Joiner.on(", ").join(optionsWithDiscrepancies), ManagementSystem.class.getSimpleName()));
                }
            }
