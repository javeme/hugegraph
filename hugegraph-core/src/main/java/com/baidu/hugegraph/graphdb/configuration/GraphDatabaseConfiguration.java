...
import com.baidu.hugegraph.diskstorage.StandardIndexProvider;
import com.baidu.hugegraph.diskstorage.StandardStoreManager;
import com.baidu.hugegraph.diskstorage.keycolumnvalue.ttl.TTLKCVSManager;
import com.baidu.hugegraph.graphdb.tinkerpop.HugeGraphDefaultSchemaMaker;
import com.baidu.hugegraph.graphdb.tinkerpop.Tp3DefaultSchemaMaker;
import com.baidu.hugegraph.graphdb.database.management.ManagementSystem;
import com.baidu.hugegraph.graphdb.types.typemaker.DisableDefaultSchemaMaker;
import com.baidu.hugegraph.util.stats.NumberUtil;
import com.baidu.hugegraph.diskstorage.util.time.*;
...
import com.baidu.hugegraph.diskstorage.idmanagement.ConflictAvoidanceMode;
import com.baidu.hugegraph.diskstorage.idmanagement.ConsistentKeyIDAuthority;
...
import com.baidu.hugegraph.diskstorage.log.kcvs.KCVSLogManager;
import com.baidu.hugegraph.graphdb.database.cache.MetricInstrumentedSchemaCache;
import com.baidu.hugegraph.graphdb.database.cache.StandardSchemaCache;
import com.baidu.hugegraph.graphdb.database.cache.SchemaCache;
import com.baidu.hugegraph.graphdb.database.serialize.StandardSerializer;
import com.baidu.hugegraph.util.encoding.LongEncoding;
import com.baidu.hugegraph.util.system.ConfigurationUtil;
...
import com.baidu.hugegraph.graphdb.database.idassigner.VertexIDAssigner;
...
/**
 * Provides functionality to configure a {@link com.baidu.hugegraph.core.HugeGraph} INSTANCE.
 * <p/>
 * <p/>
...
public class GraphDatabaseConfiguration {
    ...
    public static ConfigNamespace ROOT_NS =
            new ConfigNamespace(null, "root", "Root Configuration Namespace for the HugeGraph Graph Database");

    // ########## Graph-level Config Options ##########
    // ################################################

    public static final ConfigNamespace GRAPH_NS =
            new ConfigNamespace(ROOT_NS, "graph", "General configuration options");

    public static final ConfigOption<Boolean> ALLOW_SETTING_VERTEX_ID =
            new ConfigOption<Boolean>(GRAPH_NS, "set-vertex-id",
                    "Whether user provided vertex ids should be enabled and HugeGraph's automatic id allocation be disabled. "
                            + "Useful when operating HugeGraph in concert with another storage system that assigns long ids but disables some "
                            + "of HugeGraph's advanced features which can lead to inconsistent data. EXPERT FEATURE - USE WITH GREAT CARE.",
                    ConfigOption.Type.FIXED, false);

    public static final ConfigOption<TimestampProviders> TIMESTAMP_PROVIDER =
            new ConfigOption<TimestampProviders>(GRAPH_NS, "timestamps",
                    "The timestamp resolution to use when writing to storage and indices. Sets the time granularity for the "
                            ...
                            + "those of the backend systems. Some HugeGraph storage backends declare a preferred timestamp resolution that "
    ...
    // public static final ConfigOption<KryoInstanceCacheImpl> KRYO_INSTANCE_CACHE = new
    // ConfigOption<KryoInstanceCacheImpl>(GRAPH_NS, "kryo-instance-cache",
    // "Controls how Kryo instances are created and cached. Kryo instances are not " +
    // "safe for concurrent access. HugeGraph is responsible guaranteeing that concurrent threads use separate " +
    // "Kryo instances. HugeGraph defaults to a Kryo caching approach based on ThreadLocal, as recommended by the " +
    // "Kryo documentation (https://github.com/EsotericSoftware/kryo#threading). " +
    // "However, these ThreadLocals are not necessarily removed when HugeGraph shuts down. When HugeGraph runs on an " +
    ...
    // "CONCURRENT_HASH_MAP. The CHM implementation releases all references when HugeGraph is shutdown, but it " +
    ...
    public static final ConfigOption<String> UNIQUE_INSTANCE_ID =
            new ConfigOption<String>(GRAPH_NS, "unique-instance-id",
                    "Unique identifier for this HugeGraph instance.  This must be unique among all instances "
    ...
    public static final ConfigOption<Short> UNIQUE_INSTANCE_ID_SUFFIX =
            new ConfigOption<Short>(GRAPH_NS, "unique-instance-id-suffix",
                    "When this is set and " + UNIQUE_INSTANCE_ID.getName() + " is not, this HugeGraph "
    ...
    public static final ConfigOption<String> INITIAL_JANUSGRAPH_VERSION =
            new ConfigOption<String>(GRAPH_NS, "hugegraph-version",
                    "The version of HugeGraph with which this database was created. Automatically set on first start. Don't manually set this property.",
                    ConfigOption.Type.FIXED, String.class).hide();

    public static final ConfigOption<Boolean> ALLOW_STALE_CONFIG = new ConfigOption<Boolean>(GRAPH_NS,
            "allow-stale-config",
            "Whether to allow the local and storage-backend-hosted copies of the configuration to contain conflicting values for "
                    ...
                    + "HugeGraph will log these option conflicts, but continue normal operation using the storage-backend-hosted value "
                    + "for each conflicted option.  When this option is false, HugeGraph will log these option conflicts, but then it "
    ...
    public static final ConfigOption<Instant> REGISTRATION_TIME = new ConfigOption<>(REGISTRATION_NS, "startup-time",
            "Timestamp when this instance was started.  Automatically set.", ConfigOption.Type.GLOBAL, Instant.class)
                    .hide();

    // ########## OLAP Style Processing ##########
    // ################################################

    public static ConfigNamespace JOB_NS =
            new ConfigNamespace(null, "job", "Root Configuration Namespace for HugeGraph OLAP jobs");
    ...
    public static final ConfigNamespace COMPUTER_NS =
            new ConfigNamespace(ROOT_NS, "computer", "GraphComputer related configuration");
    ...
    public static final ConfigOption<String> COMPUTER_RESULT_MODE = new ConfigOption<String>(COMPUTER_NS, "result-mode",
            "How the graph computer should return the computed results. 'persist' for writing them into the graph, "
                    + "'localtx' for writing them into the local transaction, or 'none' (default)",
    ...
    public static final ConfigNamespace TRANSACTION_NS =
            new ConfigNamespace(ROOT_NS, "tx", "Configuration options for transaction handling");

    public static final ConfigOption<Boolean> SYSTEM_LOG_TRANSACTIONS =
            new ConfigOption<Boolean>(TRANSACTION_NS, "log-tx",
                    "Whether transaction mutations should be logged to HugeGraph's write-ahead transaction log which can be used for recovery of partially failed transactions",
    ...
    public static final ConfigNamespace TRANSACTION_RECOVERY_NS =
            new ConfigNamespace(TRANSACTION_NS, "recovery", "Configuration options for transaction recovery processes");

    public static final ConfigOption<Boolean> VERBOSE_TX_RECOVERY =
            new ConfigOption<Boolean>(TRANSACTION_RECOVERY_NS, "verbose",
    ...
    public static final ConfigNamespace QUERY_NS =
            new ConfigNamespace(ROOT_NS, "query", "Configuration options for query processing");

    public static final ConfigOption<Boolean> IGNORE_UNKNOWN_INDEX_FIELD = new ConfigOption<Boolean>(QUERY_NS,
            "ignore-unknown-index-key", "Whether to ignore undefined types encountered in user-provided index queries",
            ConfigOption.Type.MASKABLE, false);
    ...
    public static final ConfigOption<Boolean> FORCE_INDEX_USAGE = new ConfigOption<Boolean>(QUERY_NS, "force-index",
            "Whether HugeGraph should throw an exception if a graph query cannot be answered using an index. Doing so"
                    + "limits the functionality of HugeGraph's graph queries but ensures that slow graph queries are avoided "
                    + "on large graphs. Recommended for production use of HugeGraph.",
            ConfigOption.Type.MASKABLE, false);

    public static final ConfigOption<Boolean> PROPERTY_PREFETCHING =
            new ConfigOption<Boolean>(QUERY_NS, "fast-property",
    ...
    public static final ConfigNamespace SCHEMA_NS =
            new ConfigNamespace(ROOT_NS, "schema", "Schema related configuration options");
    ...
    private static final Map<String, DefaultSchemaMaker> preregisteredAutoType =
            new HashMap<String, DefaultSchemaMaker>() {
                {
                    put("none", DisableDefaultSchemaMaker.INSTANCE);
                    put("default", HugeGraphDefaultSchemaMaker.INSTANCE);
    ...
    public static final ConfigNamespace CACHE_NS =
            new ConfigNamespace(ROOT_NS, "cache", "Configuration options that modify HugeGraph's caching behavior");

    public static final ConfigOption<Boolean> DB_CACHE = new ConfigOption<Boolean>(CACHE_NS, "db-cache",
            "Whether to enable HugeGraph's database-level cache, which is shared across all transactions. "
     ...
     * then it is interpreted as a percentage of the total heap space available to the JVM this HugeGraph instance is
     * running in. If this value is bigger than 1.0 it is interpreted as an absolute size in bytes.
     */
    ...
    public static final ConfigOption<Double> DB_CACHE_SIZE = new ConfigOption<Double>(CACHE_NS, "db-cache-size",
            "Size of HugeGraph's database level cache.  Values between 0 and 1 are interpreted as a percentage "
                    + "of VM heap, while larger values are interpreted as an absolute size in bytes.",
            ConfigOption.Type.MASKABLE, 0.3);

    /**
     * How long the database level cache will keep keys expired while the mutations that triggered the expiration are
     * being persisted. This value should be larger than the time it takes for persisted mutations to become visible.
    ...
    public static final ConfigOption<Integer> DB_CACHE_CLEAN_WAIT = new ConfigOption<Integer>(CACHE_NS,
            "db-cache-clean-wait",
     ...
     * HugeGraph will check against storage backend for a newer query answer. Setting this value to 0 will cache
     * elements forever (unless they get evicted due to space constraints). This only makes sense when this is the only
     * HugeGraph instance interacting with a storage backend.
     */
     ...
     * less memory a transaction can consume at maximum. For many concurrent, long running transactions in memory
     * constraint environments, reducing the cache size can avoid OutOfMemory and GC limit exceeded exceptions. Note,
     * however, that all modifications in a transaction must always be kept in memory and hence this setting does not
     * have much impact on write intense transactions. Those must be split into smaller transactions in the case of
     * memory errors.
     * <p/>
    ...
    public static final ConfigOption<Integer> TX_CACHE_SIZE = new ConfigOption<Integer>(CACHE_NS, "tx-cache-size",
            "Maximum size of the transaction-level cache of recently-used vertices.", ConfigOption.Type.MASKABLE,
            20000);

    /**
     * Configures the initial size of the dirty (modified) vertex map used by a transaction. All vertices created or
     * updated by a transaction are held in that transaction's dirty vertex map until the transaction commits. This
     * option sets the initial size of the dirty map. Unlike {@link #TX_CACHE_SIZE}, this is not a maximum. The
     * transaction will transparently allocate more space to store dirty vertices if this initial size hint is exceeded.
     * Transactions that know how many vertices they are likely to modify a priori can avoid resize costs associated
     * with growing the dirty vertex data structure by setting this option.
     */
    ...
    /**
     * The default value of {@link #TX_DIRTY_SIZE} when batch loading is disabled. This value is only considered if the
     * user does not specify a value for {@code #TX_DIRTY_CACHE_SIZE} explictly in either the graph or transaction
     * config.
     */
    private static final int TX_DIRTY_SIZE_DEFAULT_WITHOUT_BATCH = 32;

    /**
     * The default value of {@link #TX_DIRTY_SIZE} when batch loading is enabled. This value is only considered if the
     * user does not specify a value for {@code #TX_DIRTY_CACHE_SIZE} explictly in either the graph or transaction
     * config.
     */
    ...
    public static final ConfigNamespace STORAGE_NS = new ConfigNamespace(ROOT_NS, "storage",
    ...
    public static final ConfigOption<String> STORAGE_DIRECTORY = new ConfigOption<String>(STORAGE_NS, "directory",
            "Storage directory for those storage backends that require local storage", ConfigOption.Type.LOCAL,
            String.class);
    ...
    public static final ConfigOption<String> STORAGE_BACKEND = new ConfigOption<String>(STORAGE_NS, "backend",
            "The primary persistence provider used by HugeGraph.  This is required.  It should be set one of "
                    + "HugeGraph's built-in shorthand names for its standard storage backends " + "(shorthands: "
                    + Joiner.on(", ").join(StandardStoreManager.getAllShorthands()) + ") "
    ...
    public static final ConfigOption<Boolean> STORAGE_READONLY =
            new ConfigOption<Boolean>(STORAGE_NS, "read-only", "Read-only database", ConfigOption.Type.LOCAL, false);
    // public static final String STORAGE_READONLY_KEY = "read-only";
    // public static final boolean STORAGE_READONLY_DEFAULT = false;

    /**
     * Enables batch loading which improves write performance but assumes that only one thread is interacting with the
     * graph
     */
    public static final ConfigOption<Boolean> STORAGE_BATCH = new ConfigOption<Boolean>(STORAGE_NS, "batch-loading",
            "Whether to enable batch loading into the storage backend", ConfigOption.Type.LOCAL, false);
    ...
    public static final ConfigOption<Boolean> STORAGE_TRANSACTIONAL =
            new ConfigOption<Boolean>(STORAGE_NS, "transactions",
                    "Enables transactions on storage backends that support them", ConfigOption.Type.MASKABLE, true);
     ...
     * Set to 0 to disable buffering. Buffering is disabled automatically if the storage backend does not support
     * buffered mutations.
     */
    public static final ConfigOption<Integer> BUFFER_SIZE =
            new ConfigOption<Integer>(STORAGE_NS, "buffer-size", "Size of the batch in which mutations are persisted",
                    ConfigOption.Type.MASKABLE, 1024, ConfigOption.positiveInt());
    // public static final String BUFFER_SIZE_KEY = "buffer-size";
    // public static final int BUFFER_SIZE_DEFAULT = 1024;

    /**
     * Number of times the database attempts to persist the transactional state to the storage layer. Persisting the
     * state of a committed transaction might fail for various reasons, some of which are temporary such as network
     * failures. For temporary failures, HugeGraph will re-attempt to persist the state up to the number of times
     * specified.
     */
    ...
    /**
     * Number of times the database attempts to execute a read operation against the storage layer in the current
     * transaction. A read operation might fail for various reasons, some of which are temporary such as network
     * failures. For temporary failures, HugeGraph will re-attempt to read the state up to the number of times specified
    ...
    public static final ConfigOption<Duration> STORAGE_WRITE_WAITTIME = new ConfigOption<>(STORAGE_NS, "write-time",
            "Maximum time (in ms) to wait for a backend write operation to complete successfully. If a backend write operation"
                    + "fails temporarily, HugeGraph will backoff exponentially and retry the operation until the wait time has been exhausted. ",
    ...
    /**
     * If enabled, HugeGraph attempts to parallelize storage operations against the storage backend using a fixed thread
     * pool shared across the entire HugeGraph graph database instance. Parallelization is only applicable to certain
     * storage operations and can be beneficial when the operation is I/O bound.
     */
    public static final ConfigOption<Boolean> PARALLEL_BACKEND_OPS = new ConfigOption<Boolean>(STORAGE_NS,
            "parallel-backend-ops", "Whether HugeGraph should attempt to parallelize storage operations",
            ConfigOption.Type.MASKABLE, true);
    ...
    /**
     * A unique identifier for the machine running the HugeGraph instance. It must be ensured that no other machine
     * accessing the storage backend can have the same identifier.
     */
    // public static final ConfigOption<String> INSTANCE_RID_RAW = new ConfigOption<String>(STORAGE_NS,"machine-id",
    // "A unique identifier for the machine running the HugeGraph instance",
    // ConfigOption.Type.LOCAL, String.class);
    ...
    public static final ConfigOption<Integer> STORAGE_PORT = new ConfigOption<Integer>(STORAGE_NS, "port",
            "The port on which to connect to storage backend servers.", ConfigOption.Type.LOCAL, Integer.class);

    /**
     * Username and password keys to be used to specify an access credential that may be needed to connect with a
     * secured storage backend.
     */
    public static final ConfigOption<String> AUTH_USERNAME = new ConfigOption<String>(STORAGE_NS, "username",
            "Username to authenticate against backend", ConfigOption.Type.LOCAL, String.class);
    public static final ConfigOption<String> AUTH_PASSWORD = new ConfigOption<String>(STORAGE_NS, "password",
            "Password to authenticate against backend", ConfigOption.Type.LOCAL, String.class);
    ...
    /**
     * Time in milliseconds for backend manager to wait for the storage backends to become available when HugeGraph is
     * run in server mode. Should the backend manager experience exceptions when attempting to access the storage
     * backend it will retry until this timeout is exceeded.
     * <p/>
    ...
    public static final ConfigOption<Duration> SETUP_WAITTIME = new ConfigOption<>(STORAGE_NS, "setup-wait",
            "Time in milliseconds for backend manager to wait for the storage backends to become available when HugeGraph is run in server mode",
    ...
    /**
     * Default number of results to pull over the wire when iterating over a distributed storage backend. This is batch
     * size of results to pull when iterating a result set.
     */
    public static final ConfigOption<Integer> PAGE_SIZE = new ConfigOption<Integer>(STORAGE_NS, "page-size",
            "HugeGraph break requests that may return many results from distributed storage backends "
     ...
     * are considered expired after this time and released. This value should be larger than the maximum time a
     * transaction can take in order to guarantee that no correctly held applications are expired pre-maturely and as
     * small as possible to avoid dead lock.
     */
    ...
    /**
     * Whether to attempt to delete expired locks from the storage backend. True will attempt to delete expired locks in
     * a background daemon thread. False will never attempt to delete expired locks. This option is only meaningful for
     * the default lock backend.
     *
     * @see #LOCK_BACKEND
     */
    public static final ConfigOption<Boolean> LOCK_CLEAN_EXPIRED = new ConfigOption<Boolean>(LOCK_NS, "clean-expired",
            "Whether to delete expired locks from the storage backend", ConfigOption.Type.MASKABLE, false);

    /**
     * Locker type to use. The supported types are in {@link com.baidu.hugegraph.diskstorage.Backend}.
     */
    public static final ConfigOption<String> LOCK_BACKEND = new ConfigOption<String>(LOCK_NS, "backend",
            "Locker type to use", ConfigOption.Type.GLOBAL_OFFLINE, "consistentkey");
     ...
     */
    public static final ConfigOption<String> LOCK_LOCAL_MEDIATOR_GROUP = new ConfigOption<String>(LOCK_NS,
            "local-mediator-group",
            "This option determines the LocalLockMediator instance used for early detection of lock contention "
                    + "between concurrent HugeGraph graph instances within the same process which are connected to the same "
                    + "storage backend.  HugeGraph instances that have the same value for this variable will attempt to discover "
                    + "lock contention among themselves in memory before proceeding with the general-case distributed locking "
                    + "code.  HugeGraph generates an appropriate default value for this option at startup.  Overridding "
                    + "the default is generally only useful in testing.",
            ConfigOption.Type.LOCAL, String.class);

    // ################ STORAGE - META #######################

    public static final ConfigNamespace STORE_META_NS =
            new ConfigNamespace(STORAGE_NS, "meta", "Meta data to include in storage backend retrievals", true);

    public static final ConfigOption<Boolean> STORE_META_TIMESTAMPS =
            new ConfigOption<Boolean>(STORE_META_NS, "timestamps",
    ...
    public static final ConfigOption<Boolean> STORE_META_VISIBILITY =
            new ConfigOption<Boolean>(STORE_META_NS, "visibility",
    ...
    public static final ConfigNamespace CLUSTER_NS =
            new ConfigNamespace(ROOT_NS, "cluster", "Configuration options for multi-machine deployments");

    /**
     * Whether the id space should be partitioned for equal distribution of keys. If the keyspace is ordered, this needs
     * to be enabled to ensure an even distribution of data. If the keyspace is random/hashed, then enabling this only
     * has the benefit of de-congesting a single id pool in the database.
     */
    ...
    // " +
    // "Unless explicitly set, this defaults false for stores that hash keys and defaults true for stores that preserve
    // key order " +
    // "(such as HBase and Cassandra with ByteOrderedPartitioner).",
    // ConfigOption.Type.FIXED, false);

    public static final ConfigOption<Integer> CLUSTER_MAX_PARTITIONS =
            new ConfigOption<Integer>(CLUSTER_NS, "max-partitions",
                    "The number of virtual partition blocks created in the partitioned graph. This should be larger than the maximum expected number of nodes"
                            + "in the HugeGraph graph cluster. Must be bigger than 1 and a power of 2.",
    ...
    public static final ConfigNamespace IDS_NS =
            new ConfigNamespace(ROOT_NS, "ids", "General configuration options for graph element IDs");
     ...
     * fraction of the id pool occupied and potentially lost. For write heavy applications, larger block sizes should be
     * chosen.
     */
    ...
    /**
     * The number of milliseconds that the HugeGraph id pool manager will wait before giving up on allocating a new
     * block of ids. Note, that failure to allocate a new id block will cause the entire database to fail, hence this
     * value should be set conservatively. Choose a high value if there is a lot of contention around id allocation.
     */
    public static final ConfigOption<Duration> IDS_RENEW_TIMEOUT = new ConfigOption<>(IDS_NS, "renew-timeout",
            "The number of milliseconds that the HugeGraph id pool manager will wait before giving up on allocating a new block of ids",
    ...
    /**
     * Configures when the id pool manager will attempt to allocate a new id block. When all but the configured
     * percentage of the current block is consumed, a new block will be allocated. Larger values should be used if a lot
     * of ids are allocated in a short amount of time. Value must be in (0,1].
     */
    public static final ConfigOption<Double> IDS_RENEW_BUFFER_PERCENTAGE =
            new ConfigOption<Double>(IDS_NS, "renew-percentage",
                    "When the most-recently-reserved ID block has only this percentage of its total IDs remaining "
                            + "(expressed as a value between 0 and 1), HugeGraph asynchronously begins reserving another block. "
    ...
    // public static final String STORAGE_NAMESPACE = "storage";
    public static final ConfigNamespace IDAUTHORITY_NS = new ConfigNamespace(IDS_NS, "authority",
     ...
     * backend. Also, the time waited after the application before verifying that the application was successful.
     */
    ...
    /**
     * Sets the strategy used by {@link ConsistentKeyIDAuthority} to avoid contention in ID block alloction between
     * HugeGraph instances concurrently sharing a single distributed storage backend.
     */
    // This is set to GLOBAL_OFFLINE as opposed to MASKABLE or GLOBAL to prevent mixing both global-randomized and
    // local-manual modes within the same cluster
    public static final ConfigOption<ConflictAvoidanceMode> IDAUTHORITY_CONFLICT_AVOIDANCE =
            new ConfigOption<ConflictAvoidanceMode>(IDAUTHORITY_NS, "conflict-avoidance-mode",
                    "This setting helps separate HugeGraph instances sharing a single graph storage backend avoid contention when reserving ID blocks, "
                            + "increasing overall throughput.",
                    ConfigOption.Type.GLOBAL_OFFLINE, ConflictAvoidanceMode.class, ConflictAvoidanceMode.NONE);

    /**
     * When HugeGraph allocates IDs with
     * {@link com.baidu.hugegraph.diskstorage.idmanagement.ConflictAvoidanceMode#GLOBAL_AUTO} configured, it picks a
     * random unique ID marker and attempts to allocate IDs from a partition using the marker. The ID markers function
     * as subpartitions with each ID partition. If the attempt fails because that partition + uniqueid combination is
     * already completely allocated, then HugeGraph will generate a new random unique ID and try again. This controls
     * the maximum number of attempts before HugeGraph assumes the entire partition is allocated and fails the request.
     * It must be set to at least 1 and should generally be set to 3 or more.
     * <p/>
     * This setting has no effect when {@link #IDAUTHORITY_CONFLICT_AVOIDANCE} is not configured to
     * {@link com.baidu.hugegraph.diskstorage.idmanagement.ConflictAvoidanceMode#GLOBAL_AUTO}.
     */
    public static final ConfigOption<Integer> IDAUTHORITY_CAV_RETRIES =
            new ConfigOption<Integer>(IDAUTHORITY_NS, "randomized-conflict-avoidance-retries",
    ...
    /**
     * Configures the number of bits of HugeGraph assigned ids that are reserved for a unique id marker that allows the
     * id allocation to be scaled over multiple sub-clusters and to reduce race-conditions when a lot of HugeGraph
     * instances attempt to allocate ids at the same time (e.g. during parallel bulk loading)
     *
     * IMPORTANT: This should never ever, ever be modified from its initial value and ALL HugeGraph instances must use
     * the same value. Otherwise, data corruption will occur.
     *
     * This setting has no effect when {@link #IDAUTHORITY_CONFLICT_AVOIDANCE} is configured to
     * {@link com.baidu.hugegraph.diskstorage.idmanagement.ConflictAvoidanceMode#NONE}. However, note that while the
     * conflict avoidance mode can be changed, this setting cannot ever be changed and must therefore be considered a
     * priori.
     */
    public static final ConfigOption<Integer> IDAUTHORITY_CAV_BITS =
            new ConfigOption<Integer>(IDAUTHORITY_NS, "conflict-avoidance-tag-bits",
                    "Configures the number of bits of HugeGraph-assigned element IDs that are reserved for the conflict avoidance tag",
    ...
    /**
     * Unique id marker to be used by this HugeGraph instance when allocating ids. The unique id marker must be
     * non-negative and fit within the number of unique id bits configured. By assigning different unique id markers to
     * individual HugeGraph instances it can be assured that those instances don't conflict with one another when
     * attempting to allocate new id blocks.
     *
     * IMPORTANT: The configured unique id marker must fit within the configured unique id bit width.
     *
     * This setting has no effect when {@link #IDAUTHORITY_CONFLICT_AVOIDANCE} is configured to
     * {@link com.baidu.hugegraph.diskstorage.idmanagement.ConflictAvoidanceMode#NONE}.
     */
    public static final ConfigOption<Integer> IDAUTHORITY_CAV_TAG =
            new ConfigOption<Integer>(IDAUTHORITY_NS, "conflict-avoidance-tag",
                    "Conflict avoidance tag to be used by this HugeGraph instance when allocating IDs",
    ...
    public static final ConfigNamespace INDEX_NS =
            new ConfigNamespace(ROOT_NS, "index", "Configuration options for the individual indexing backends", true);
    ...
    public static final ConfigOption<String> INDEX_BACKEND = new ConfigOption<String>(INDEX_NS, "backend",
            "The indexing backend used to extend and optimize HugeGraph's query functionality. "
                    + "This setting is optional.  HugeGraph can use multiple heterogeneous index backends.  "
                    ...
                    + "HugeGraph's built-in shorthand names for its standard index backends " + "(shorthands: "
                    + Joiner.on(", ").join(StandardIndexProvider.getAllShorthands()) + ") "
    ...
    public static final ConfigOption<String> INDEX_DIRECTORY = new ConfigOption<String>(INDEX_NS, "directory",
            "Directory to store index data locally", ConfigOption.Type.MASKABLE, String.class);

    public static final ConfigOption<String> INDEX_NAME = new ConfigOption<String>(INDEX_NS, "index-name",
            "Name of the index if required by the indexing backend", ConfigOption.Type.GLOBAL_OFFLINE, "hugegraph");
    ...
    public static final ConfigOption<Integer> INDEX_PORT = new ConfigOption<Integer>(INDEX_NS, "port",
            "The port on which to connect to index backend servers", ConfigOption.Type.MASKABLE, Integer.class);
    ...
    public static final ConfigOption<Integer> INDEX_MAX_RESULT_SET_SIZE =
            new ConfigOption<Integer>(INDEX_NS, "max-result-set-size",
    ...
    public static final ConfigNamespace LOG_NS = new ConfigNamespace(GraphDatabaseConfiguration.ROOT_NS, "log",
            "Configuration options for HugeGraph's logging system", true);

    public static final String MANAGEMENT_LOG = "hugegraph";
    ...
    public static final ConfigOption<String> LOG_BACKEND = new ConfigOption<String>(LOG_NS, "backend",
            "Define the log backed to use", ConfigOption.Type.GLOBAL_OFFLINE, "default");

    public static final ConfigOption<Integer> LOG_NUM_BUCKETS = new ConfigOption<Integer>(LOG_NS, "num-buckets",
            "The number of buckets to split log entries into for load balancing", ConfigOption.Type.GLOBAL_OFFLINE, 1,
    ...
    public static final ConfigOption<Integer> LOG_READ_THREADS = new ConfigOption<Integer>(LOG_NS, "read-threads",
            "Number of threads to be used in reading and processing log messages", ConfigOption.Type.MASKABLE, 1,
            ConfigOption.positiveInt());
    ...
    public static final ConfigNamespace ATTRIBUTE_NS =
            new ConfigNamespace(ROOT_NS, "attributes", "Configuration options for attribute handling");

    public static final ConfigNamespace CUSTOM_ATTRIBUTE_NS =
            new ConfigNamespace(ATTRIBUTE_NS, "custom", "Custom attribute serialization and handling", true);

    public static final String ATTRIBUTE_PREFIX = "attribute";

    public static final ConfigOption<String> CUSTOM_ATTRIBUTE_CLASS =
            new ConfigOption<String>(CUSTOM_ATTRIBUTE_NS, "attribute-class",
                    "Class of the custom attribute to be registered", ConfigOption.Type.GLOBAL_OFFLINE, String.class);
    public static final ConfigOption<String> CUSTOM_SERIALIZER_CLASS = new ConfigOption<String>(CUSTOM_ATTRIBUTE_NS,
            "serializer-class", "Class of the custom attribute serializer to be registered",
    ...
    public static final ConfigNamespace METRICS_NS =
            new ConfigNamespace(ROOT_NS, "metrics", "Configuration options for metrics reporting");

    /**
     * Whether to enable basic timing and operation count monitoring on backend methods using the
     * {@code com.codahale.metrics} package.
     */
    public static final ConfigOption<Boolean> BASIC_METRICS = new ConfigOption<Boolean>(METRICS_NS, "enabled",
            "Whether to enable basic timing and operation count monitoring on backend", ConfigOption.Type.MASKABLE,
            false);
    // public static final String BASIC_METRICS = "enable-basic-metrics";
    // public static final boolean BASIC_METRICS_DEFAULT = false;

    /**
     * This is the prefix used outside of a graph database configuration, or for operations where a system-internal
     * transaction is necessary as an implementation detail. It currently can't be modified, though there is no
     * substantial technical obstacle preventing it from being configured -- some kind of configuration object is in
     * scope everywhere it is used, and it could theoretically be stored in and read from that object.
     */
    public static final String METRICS_PREFIX_DEFAULT = "com.baidu.hugegraph";
    ...
    /**
     * The default name prefix for Metrics reported by HugeGraph. All metric names will begin with this string and a
     * period. This value can be overridden on a transaction-specific basis through
    ...
    public static final ConfigOption<String> METRICS_PREFIX =
            new ConfigOption<String>(METRICS_NS, "prefix", "The default name prefix for Metrics reported by HugeGraph.",
     ...
     * <p/>
     * If true, then metrics for each of these backends will use the same metric name ("stores"). All of their
     * measurements will be combined. This setting measures the sum of HugeGraph's backend activity without
     * distinguishing between contributions of its various internal stores.
     * <p/>
     * If false, then metrics for each of these backends will use a unique metric name ("idStore", "edgeStore",
     * "vertexIndex", and "edgeIndex"). This setting exposes the activity associated with each backend component, but it
     * also multiplies the number of measurements involved by four.
     * <p/>
     * This option has no effect when {@link #BASIC_METRICS} is false.
     */
    public static final ConfigOption<Boolean> METRICS_MERGE_STORES =
            new ConfigOption<Boolean>(METRICS_NS, "merge-stores",
                    "Whether to aggregate measurements for the edge store, vertex index, edge index, and ID store",
                    ConfigOption.Type.MASKABLE, true);
    ...
    public static final ConfigNamespace METRICS_CONSOLE_NS =
            new ConfigNamespace(METRICS_NS, "console", "Configuration options for metrics reporting to console");

    /**
     * Metrics console reporter interval in milliseconds. Leaving this configuration key absent or null disables the
     * console reporter.
     */
    public static final ConfigOption<Duration> METRICS_CONSOLE_INTERVAL = new ConfigOption<>(METRICS_CONSOLE_NS,
            "interval", "Time between Metrics reports printing to the console, in milliseconds",
    ...
    public static final ConfigNamespace METRICS_CSV_NS =
            new ConfigNamespace(METRICS_NS, "csv", "Configuration options for metrics reporting to CSV file");

    /**
     * Metrics CSV reporter interval in milliseconds. Leaving this configuration key absent or null disables the CSV
     * reporter.
     */
    public static final ConfigOption<Duration> METRICS_CSV_INTERVAL = new ConfigOption<>(METRICS_CSV_NS, "interval",
            "Time between dumps of CSV files containing Metrics data, in milliseconds", ConfigOption.Type.MASKABLE,
            Duration.class);
    ...
    /**
     * Metrics CSV output directory. It will be created if it doesn't already exist. This option must be non-null if
     * {@link #METRICS_CSV_INTERVAL} is non-null. This option has no effect if {@code #METRICS_CSV_INTERVAL} is null.
     */
    public static final ConfigOption<String> METRICS_CSV_DIR = new ConfigOption<String>(METRICS_CSV_NS, "directory",
            "Metrics CSV output directory", ConfigOption.Type.MASKABLE, String.class);
    ...
    public static final ConfigNamespace METRICS_JMX_NS =
            new ConfigNamespace(METRICS_NS, "jmx", "Configuration options for metrics reporting through JMX");
    ...
    public static final ConfigOption<Boolean> METRICS_JMX_ENABLED = new ConfigOption<Boolean>(METRICS_JMX_NS, "enabled",
            "Whether to report Metrics through a JMX MBean", ConfigOption.Type.MASKABLE, false);
    ...
    public static final ConfigOption<String> METRICS_JMX_DOMAIN = new ConfigOption<String>(METRICS_JMX_NS, "domain",
            "The JMX domain in which to report Metrics", ConfigOption.Type.MASKABLE, String.class);
    // public static final String METRICS_JMX_DOMAIN_KEY = "jmx.domain";
    // public static final String METRICS_JMX_DOMAIN_DEFAULT = null;

    /**
     * The JMX agentId through which to report Metrics. Calling {@link MBeanServerFactory#findMBeanServer(String)} on
     * this value must return exactly one {@code MBeanServer} at runtime. If null, then Metrics applies its default
     * value.
     */
    public static final ConfigOption<String> METRICS_JMX_AGENTID = new ConfigOption<String>(METRICS_JMX_NS, "agentid",
            "The JMX agentId used by Metrics", ConfigOption.Type.MASKABLE, String.class);
    ...
    public static final ConfigNamespace METRICS_SLF4J_NS =
            new ConfigNamespace(METRICS_NS, "slf4j", "Configuration options for metrics reporting through slf4j");

    /**
     * Metrics Slf4j reporter interval in milliseconds. Leaving this configuration key absent or null disables the Slf4j
     * reporter.
     */
    public static final ConfigOption<Duration> METRICS_SLF4J_INTERVAL = new ConfigOption<>(METRICS_SLF4J_NS, "interval",
            "Time between slf4j logging reports of Metrics data, in milliseconds", ConfigOption.Type.MASKABLE,
            Duration.class);
    ...
    /**
     * The complete name of the Logger through which Metrics will report via Slf4j. If non-null, then Metrics will be
     * dumped on {@link LoggerFactory#getLogger(String)} with the configured value as the argument. If null, then
     * Metrics will use its default Slf4j logger.
     */
    public static final ConfigOption<String> METRICS_SLF4J_LOGGER = new ConfigOption<String>(METRICS_SLF4J_NS, "logger",
            "The complete name of the Logger through which Metrics will report via Slf4j", ConfigOption.Type.MASKABLE,
            String.class);
    ...
    public static final ConfigNamespace METRICS_GANGLIA_NS =
            new ConfigNamespace(METRICS_NS, "ganglia", "Configuration options for metrics reporting through Ganglia");

    /**
     * The unicast host or multicast group name to which Metrics will send Ganglia data. Setting this config key has no
     * effect unless {@link #GANGLIA_INTERVAL} is also set.
     */
    public static final ConfigOption<String> GANGLIA_HOST_OR_GROUP = new ConfigOption<String>(METRICS_GANGLIA_NS,
            "hostname", "The unicast host or multicast group name to which Metrics will send Ganglia data",
            ConfigOption.Type.MASKABLE, String.class);
    //
    // public static final String GANGLIA_HOST_OR_GROUP_KEY = "hostname";

    /**
     * The number of milliseconds to wait between sending Metrics data to the host or group specified by
     * {@link #GANGLIA_HOST_OR_GROUP}. This has no effect unless {@link #GANGLIA_HOST_OR_GROUP} is also set.
     */
    public static final ConfigOption<Duration> GANGLIA_INTERVAL = new ConfigOption<>(METRICS_GANGLIA_NS, "interval",
            "The number of milliseconds to wait between sending Metrics data to Ganglia", ConfigOption.Type.MASKABLE,
            Duration.class);
    ...
    public static final ConfigOption<Integer> GANGLIA_PORT = new ConfigOption<Integer>(METRICS_GANGLIA_NS, "port",
            "The port to which Ganglia data are sent", ConfigOption.Type.MASKABLE, 8649);
    // public static final String GANGLIA_PORT = "port";
    // public static final int GANGLIA_PORT_DEFAULT = 8649;

    /**
     * Whether to interpret {@link #GANGLIA_HOST_OR_GROUP} as a unicast or multicast address. If present, it must be
     * either the string "multicast" or the string "unicast".
     * <p/>
     */
    public static final ConfigOption<String> GANGLIA_ADDRESSING_MODE = new ConfigOption<String>(METRICS_GANGLIA_NS,
            "addressing-mode", "Whether to communicate to Ganglia via uni- or multicast", ConfigOption.Type.MASKABLE,
            "unicast", new Predicate<String>() {
                @Override
                public boolean apply(@Nullable String s) {
    ...
    /**
     * The multicast TTL to set on outgoing Ganglia datagrams. This has no effect when {@link #GANGLIA_ADDRESSING_MODE}
     * is set to "multicast".
     * <p/>
     * This is a TTL in the multicast protocol sense (number of routed hops), not a timestamp sense.
     */
    public static final ConfigOption<Integer> GANGLIA_TTL = new ConfigOption<Integer>(METRICS_GANGLIA_NS, "ttl",
            "The multicast TTL to set on outgoing Ganglia datagrams", ConfigOption.Type.MASKABLE, 1);
    ...
    public static final ConfigOption<Boolean> GANGLIA_USE_PROTOCOL_31 =
            new ConfigOption<Boolean>(METRICS_GANGLIA_NS, "protocol-31",
                    "Whether to send data to Ganglia in the 3.1 protocol format", ConfigOption.Type.MASKABLE, true);
    ...
    /**
     * The host UUID to set on outgoing Ganglia datagrams. If null, no UUID is set on outgoing data.
     * <p/>
    ...
    /**
     * If non-null, it must be a valid Gmetric spoof string formatted as an IP:hostname pair. If null, Ganglia will
     * automatically determine the IP and hostname to set on outgoing datagrams.
     * <p/>
    ...
    /**
     * The configuration namespace within {@link #METRICS_NS} for Graphite.
     */
    public static final ConfigNamespace METRICS_GRAPHITE_NS =
            new ConfigNamespace(METRICS_NS, "graphite", "Configuration options for metrics reporting through Graphite");

    // public static final String GRAPHITE_NAMESPACE = "graphite";

    /**
     * The hostname to receive Graphite plaintext protocol metric data. Setting this config key has no effect unless
     * {@link #GRAPHITE_INTERVAL} is also set.
     */
    public static final ConfigOption<String> GRAPHITE_HOST = new ConfigOption<String>(METRICS_GRAPHITE_NS, "hostname",
            "The hostname to receive Graphite plaintext protocol metric data", ConfigOption.Type.MASKABLE,
            String.class);
    // public static final String GRAPHITE_HOST_KEY = "hostname";

    /**
     * The number of milliseconds to wait between sending Metrics data to the host specified {@link #GRAPHITE_HOST}.
     * This has no effect unless {@link #GRAPHITE_HOST} is also set.
     */
    public static final ConfigOption<Duration> GRAPHITE_INTERVAL = new ConfigOption<>(METRICS_GRAPHITE_NS, "interval",
            "The number of milliseconds to wait between sending Metrics data", ConfigOption.Type.MASKABLE,
            Duration.class);
    ...
    public static final ConfigOption<Integer> GRAPHITE_PORT = new ConfigOption<Integer>(METRICS_GRAPHITE_NS, "port",
            "The port to which Graphite data are sent", ConfigOption.Type.MASKABLE, 2003);
    // public static final String GRAPHITE_PORT_KEY = "port";
    // public static final int GRAPHITE_PORT_DEFAULT = 2003;

    /**
     * A Graphite-specific prefix for reported metrics. If non-null, Metrics prepends this and a "." to all metric names
     * before reporting them to Graphite.
     * <p/>
     */
    public static final ConfigOption<String> GRAPHITE_PREFIX = new ConfigOption<String>(METRICS_GRAPHITE_NS, "prefix",
            "A Graphite-specific prefix for reported metrics", ConfigOption.Type.MASKABLE, String.class);
    ...
    public static final ConfigNamespace GREMLIN_NS =
            new ConfigNamespace(ROOT_NS, "gremlin", "Gremlin configuration options");

    public static final ConfigOption<String> GREMLIN_GRAPH = new ConfigOption<String>(GREMLIN_NS, "graph",
            "The implementation of graph factory that will be used by gremlin server", ConfigOption.Type.LOCAL,
            "com.baidu.hugegraph.core.HugeGraphFactory");
    ...
    public GraphDatabaseConfiguration(ReadConfiguration localConfig) {
        ...
        ModifiableConfiguration overwrite =
                new ModifiableConfiguration(ROOT_NS, new CommonsConfiguration(), BasicConfiguration.Restriction.NONE);
        ...
        localConfiguration =
                new ModifiableConfiguration(ROOT_NS, new CommonsConfiguration(), BasicConfiguration.Restriction.LOCAL);
        ...
        try {
            ...
            ModifiableConfiguration globalWrite =
                    new ModifiableConfiguration(ROOT_NS, kcvsConfig, BasicConfiguration.Restriction.GLOBAL);
            if (!globalWrite.isFrozen()) {
                // Copy over global configurations
                globalWrite.setAll(getGlobalSubset(localbc.getAll()));

                // Write HugeGraph version
                ...
                globalWrite.set(INITIAL_JANUSGRAPH_VERSION, HugeGraphConstants.VERSION);

                /*
                 * If the configuration does not explicitly set a timestamp provider and the storage backend both
                 * supports timestamps and has a preference for a specific timestamp provider, then apply the backend's
                 * preference.
                 */
                ...
                Preconditions.checkArgument(version != null, "HugeGraph version has not been initialized");
                if (!HugeGraphConstants.VERSION.equals(version)
                        && !HugeGraphConstants.COMPATIBLE_VERSIONS.contains(version)) {
                    throw new HugeGraphException(
                            "StorageBackend version is incompatible with current HugeGraph version: storage=" + version
                                    + " vs. runtime=" + HugeGraphConstants.VERSION);
                }
                ...
                // The point of this check is to find edits to the local config which have no effect (and therefore
                // likely indicate misconfiguration)
                ...
                for (Map.Entry<ConfigElement.PathIdentifier, Object> ent : getManagedSubset(localbc.getAll())
                    ...
                    if (!match) {
                        ...
                        String template =
                                "Local setting {}={} (Type: {}) is overridden by globally managed value ({}).  Use the {} interface instead of the local configuration to control this setting.";
                        Object replacements[] = new Object[] { fullOptionName, localValue, opt.getType(), storeValue,
                ...
                if (0 < optionsWithDiscrepancies.size() && !managedOverridesAllowed) {
                    String template =
                            "Local settings present for one or more globally managed options: [%s].  These options are controlled through the %s interface; local settings have no effect.";
                    throw new HugeGraphConfigurationException(String.format(template,
                            Joiner.on(", ").join(optionsWithDiscrepancies), ManagementSystem.class.getSimpleName()));
                }
            }
    ...
    private static String computeUniqueInstanceId(Configuration config) {
        ...
        try {
            ...
            throw new HugeGraphConfigurationException("Cannot determine local host", e);
        }
    ...
    public static ModifiableConfiguration getGlobalSystemConfig(Backend backend) {
        return new ModifiableConfiguration(ROOT_NS, backend.getGlobalSystemConfig(),
    ...
    private void preLoadConfiguration() {
        ...
        if (batchLoading)
            defaultSchemaMaker = DisableDefaultSchemaMaker.INSTANCE;
    ...
    private void configureMetricsCsvReporter() {
        if (configuration.has(METRICS_CSV_DIR)) {
            MetricManager.INSTANCE.addCsvReporter(configuration.get(METRICS_CSV_INTERVAL),
                    configuration.get(METRICS_CSV_DIR));
        }
    }

    private void configureMetricsJmxReporter() {
        if (configuration.get(METRICS_JMX_ENABLED)) {
            MetricManager.INSTANCE.addJmxReporter(configuration.get(METRICS_JMX_DOMAIN),
                    configuration.get(METRICS_JMX_AGENTID));
        }
    }
    ...
    private void configureMetricsGangliaReporter() {
        if (configuration.has(GANGLIA_HOST_OR_GROUP)) {
            ...
            String spoof = null;
            if (configuration.has(GANGLIA_SPOOF))
                spoof = configuration.get(GANGLIA_SPOOF);
    ...
    private void configureMetricsGraphiteReporter() {
        if (configuration.has(GRAPHITE_HOST)) {
            MetricManager.INSTANCE.addGraphiteReporter(configuration.get(GRAPHITE_HOST),
                    configuration.get(GRAPHITE_PORT), configuration.get(GRAPHITE_PREFIX),
    ...
    public static List<RegisteredAttributeClass<?>> getRegisteredAttributeClasses(Configuration configuration) {
        ...
        for (String attributeId : configuration.getContainedNamespaces(CUSTOM_ATTRIBUTE_NS)) {
            Preconditions.checkArgument(attributeId.startsWith(ATTRIBUTE_PREFIX), "Invalid attribute definition: %s",
                    attributeId);
            int position;
            try {
                position = Integer.parseInt(attributeId.substring(ATTRIBUTE_PREFIX.length()));
            } catch (NumberFormatException e) {
                throw new IllegalArgumentException("Expected entry of the form [" + ATTRIBUTE_PREFIX
                        + "X] where X is a number but given" + attributeId);
            }
            Class<?> clazz = null;
            ...
            for (int i = 0; i < all.size(); i++) {
                if (all.get(i).equals(reg)) {
                    throw new IllegalArgumentException(
                            "Duplicate attribute registration: " + all.get(i) + " and " + reg);
                }
            }
            all.add(reg);
    ...
    public StoreFeatures getStoreFeatures() {
        Preconditions.checkArgument(storeFeatures != null,
                "Cannot retrieve store features before the storage backend has been initialized");
    ...
    public org.apache.commons.configuration.Configuration getLocalConfiguration() {
        org.apache.commons.configuration.Configuration config =
                ((CommonsConfiguration) localConfiguration.getConfiguration()).getCommonConfiguration();
        config.setProperty(Graph.GRAPH, HugeGraphFactory.class.getName());
        return config;
    }
