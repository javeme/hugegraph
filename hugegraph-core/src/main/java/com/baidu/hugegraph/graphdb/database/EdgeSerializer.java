...
import static com.baidu.hugegraph.graphdb.database.idhandling.IDHandler.DirectionID;
import static com.baidu.hugegraph.graphdb.database.idhandling.IDHandler.RelationTypeParse;
...
public class EdgeSerializer implements RelationReader {
    ...
    public StaticArrayEntry writeRelation(InternalRelation relation, InternalRelationType type, int position,
        ...
        if (relation.isEdge()) {
            ...
            Object value = ((HugeGraphVertexProperty) relation).value();
        ...
        StaticArrayEntry entry = new StaticArrayEntry(type.getSortOrder() == Order.DESC
                ? out.getStaticBufferFlipBytes(keyStartPos, keyEndPos) : out.getStaticBuffer(), valuePosition);
        return entry;
    }
        ...
        if (dir == Direction.BOTH) {
            ...
            for (int i = 0; i < sortKey.length && sortKey[i] != null; i++) {
                ...
                if (i >= sortKeyIDs.length) {
                    ...
                    assert (skey instanceof ImplicitKey)
                            && (skey == ImplicitKey.JANUSGRAPHID || skey == ImplicitKey.ADJACENT_ID);
                    assert skey != ImplicitKey.ADJACENT_ID || (i == sortKeyIDs.length);
                    assert skey != ImplicitKey.JANUSGRAPHID
                            || (!type.multiplicity().isConstrained() && (i == sortKeyIDs.length && skey.isPropertyKey()
                                    || i == sortKeyIDs.length + 1 && skey.isEdgeLabel()));
                    assert colStart.getPosition() == colEnd.getPosition();
