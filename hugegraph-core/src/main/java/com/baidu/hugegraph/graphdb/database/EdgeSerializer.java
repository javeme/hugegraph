        ...
        if (dir == Direction.BOTH) {
            ...
            for (int i = 0; i < sortKey.length && sortKey[i] != null; i++) {
                ...
                if (i>=sortKeyIDs.length) {
                    ...
                    assert (skey instanceof ImplicitKey) && (skey==ImplicitKey.hugegraphID || skey==ImplicitKey.ADJACENT_ID);
                    assert skey!=ImplicitKey.ADJACENT_ID || (i==sortKeyIDs.length);
                    assert skey!=ImplicitKey.hugegraphID || (!type.multiplicity().isConstrained() &&
                ...
                if (interval.isPoints()) {
                    if (skey==ImplicitKey.hugegraphID || skey==ImplicitKey.ADJACENT_ID) {
                        assert !type.multiplicity().isUnique(dir);
