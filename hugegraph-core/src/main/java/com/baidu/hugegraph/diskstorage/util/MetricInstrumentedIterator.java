...
import com.baidu.hugegraph.diskstorage.keycolumnvalue.KeyIterator;
import org.apache.commons.lang.StringUtils;

/**
 * This class is used by {@code MetricInstrumentedStore} to measure wallclock time, method invocation counts, and
 * exceptions thrown by the methods on {@link RecordIterator} instances returned from
...
public class MetricInstrumentedIterator implements KeyIterator {
    ...
    /**
     * If the iterator argument is non-null, then return a new {@code MetricInstrumentedIterator} wrapping it. Metrics
     * for method calls on the wrapped instance will be prefixed with the string {@code prefix} which must be non-null.
     * If the iterator argument is null, then return null.
     * 
     * @param keyIterator the iterator to wrap with Metrics measurements
     * @param prefix the Metrics name prefix string
     * 
     * @return a wrapper around {@code keyIterator} or null if {@code keyIterator} is null
     */
