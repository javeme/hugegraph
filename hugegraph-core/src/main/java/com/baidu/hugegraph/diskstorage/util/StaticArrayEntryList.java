...
public class StaticArrayEntryList extends AbstractList<Entry> implements EntryList {
    ...
    /**
     * Stores the offset and value position information for each entry. The length of this array equals the number of
     * entries in this list. Hence, limitAndValuePos[i] contains the limit and value position information of entry i.
     * The first 32 bits of that long value are the offset (as int) and the second 32 bits are the value position (as
     * int).
     */
    ...
    private StaticArrayEntryList(final byte[] data, final long[] limitAndValuePos,
            final EntryMetaData[] metaDataSchema) {
    ...
    public Entry get(int index) {
        ...
        return new StaticEntry(index, offset, getLimit(limitAndValuePos[index]), getValuePos(limitAndValuePos[index]),
                metadata);
    }
    ...
    public static final <E> EntryList ofByteBuffer(Iterable<E> elements,
            StaticArrayEntry.GetColVal<E, ByteBuffer> getter) {
        return of(elements, getter, StaticArrayEntry.ByteBufferHandler.INSTANCE);
    }

    public static final <E> EntryList ofStaticBuffer(Iterable<E> elements,
            StaticArrayEntry.GetColVal<E, StaticBuffer> getter) {
        return of(elements, getter, StaticArrayEntry.StaticBufferHandler.INSTANCE);
    }

    public static final <E> EntryList ofByteBuffer(Iterator<E> elements,
            StaticArrayEntry.GetColVal<E, ByteBuffer> getter) {
        return of(elements, getter, StaticArrayEntry.ByteBufferHandler.INSTANCE);
    }

    public static final <E> EntryList ofStaticBuffer(Iterator<E> elements,
            StaticArrayEntry.GetColVal<E, StaticBuffer> getter) {
        return of(elements, getter, StaticArrayEntry.StaticBufferHandler.INSTANCE);
    }

    private static final <E, D> EntryList of(Iterable<E> elements, StaticArrayEntry.GetColVal<E, D> getter,
    ...
    private static final <E, D> EntryList of(Iterator<E> elements, StaticArrayEntry.GetColVal<E, D> getter,
            StaticArrayEntry.DataHandler<D> datahandler) {
        Preconditions.checkArgument(elements != null && getter != null && datahandler != null);
        if (!elements.hasNext())
            return EMPTY_LIST;
    ...
    private static <D, K> int getMetaDataSize(EntryMetaData[] schema, D entry,
            StaticArrayEntry.GetColVal<D, K> metaGetter) {
        int datasize = 0;
        if (schema.length > 0) {
    ...
    private static <D, K> int writeMetaData(byte[] data, int startPos, EntryMetaData[] schema, D entry,
            StaticArrayEntry.GetColVal<D, K> metaGetter) {
        if (schema.length == 0)
            return startPos;
        for (EntryMetaData meta : schema) {
