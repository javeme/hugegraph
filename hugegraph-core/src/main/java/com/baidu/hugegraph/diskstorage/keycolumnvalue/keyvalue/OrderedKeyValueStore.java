...
public interface OrderedKeyValueStore extends KeyValueStore {
    ...
    /**
     * Inserts the given key-value pair into the store. If the key already exists, its value is overwritten by the given
     * one.
     *
     * @param key
     * @param value
     ...
     * That means, the key lies between the query's start and end buffers, satisfied the filter condition (if any) and
     * the position of the result in the result list iterator is less than the given limit.
     *
    ...
    /**
     * Like {@link #getSlice(KVQuery, com.baidu.hugegraph.diskstorage.keycolumnvalue.StoreTransaction)} but executes all
     * of the given queries at once and returns a map of all the result sets of each query.
     *
     * Only supported when the given store implementation supports multi-query, i.e.
     * {@link com.baidu.hugegraph.diskstorage.keycolumnvalue.StoreFeatures#hasMultiQuery()} return true. Otherwise this
     * method may throw a {@link UnsupportedOperationException}.
     *
     * @param queries
     * @param txh
     * @return
