...
package com.baidu.hugegraph.diskstorage.idmanagement;
...
import com.baidu.hugegraph.graphdb.database.idassigner.IDPoolExhaustedException;
import com.baidu.hugegraph.graphdb.database.idhandling.VariableLong;

/**
 * {@link com.baidu.hugegraph.diskstorage.IDAuthority} implementation assuming that the backing store supports
 * consistent key operations.
 * <p/>
 * ID blocks are allocated by first applying for an id block, waiting for a specified period of time and then checking
 * that the application was the first received for that particular id block. If so, the application is considered
 * successful. If not, some other process won the application and a new application is tried.
 * <p/>
...
public class ConsistentKeyIDAuthority extends AbstractIDAuthority implements BackendOperation.TransactionalProvider {
     ...
     *
     * ----------------------------------------------------------- | 8 bytes counter | 8 bytes timestamp | var bytes
     * rid/uid | -----------------------------------------------------------
     *
     * The argument for the following two slice bounds mirrors the argument for choosing bounds in ConsistentKeyLocker.
     */
    ...
    public ConsistentKeyIDAuthority(KeyColumnValueStore idStore, StoreManager manager, Configuration config)
        ...
        storeTxConfigBuilder =
                new StandardBaseTransactionConfig.Builder().groupName(metricsPrefix).timestampProvider(times);
        ...
        if (conflictAvoidanceMode.equals(ConflictAvoidanceMode.GLOBAL_AUTO)) {
            Preconditions.checkArgument(!config.has(IDAUTHORITY_CAV_TAG),
                    "Conflicting configuration: a unique id and randomization have been set");
            randomizeUniqueId = true;
            randomUniqueIDLimit = config.get(IDAUTHORITY_CAV_RETRIES);
            Preconditions.checkArgument(randomUniqueIDLimit < uniqueIDUpperBound,
                    "Cannot have more uid retries [%d] than available values [%d]", randomUniqueIDLimit,
                    uniqueIDUpperBound);
            uniqueId = -1;
            storeTxConfigBuilder.customOptions(manager.getFeatures().getKeyConsistentTxConfig());
        } else {
            randomizeUniqueId = false;
            Preconditions.checkArgument(!config.has(IDAUTHORITY_CAV_RETRIES), "Retry count is only meaningful when "
                    + IDAUTHORITY_CONFLICT_AVOIDANCE + " is set to " + ConflictAvoidanceMode.GLOBAL_AUTO);
            randomUniqueIDLimit = 0;
            if (conflictAvoidanceMode.equals(ConflictAvoidanceMode.LOCAL_MANUAL)) {
                Preconditions.checkArgument(config.has(IDAUTHORITY_CAV_TAG),
                        "Need to configure a unique id in order to use local consistency");
            ...
            Preconditions.checkArgument(uniqueId < uniqueIDUpperBound, "Unique id is too large for bit width [%s]: %s",
                    uniqueIdBitWidth, uniqueId);
        }
    ...
    @Override
    public synchronized IDBlock getIDBlock(final int partition, final int idNamespace, Duration timeout)
            throws BackendException {
        Preconditions.checkArgument(partition >= 0 && partition < (1 << partitionBitWdith),
                "Invalid partition id [%s] for bit width [%s]", partition, partitionBitWdith);
        ...
        Preconditions.checkArgument(maxAvailableBits > 0,
                "Unique id bit width [%s] is too wide for id-namespace [%s] id bound [%s]", uniqueIdBitWidth,
                idNamespace, idUpperBound);
        ...
        Preconditions.checkArgument(idBlockUpperBound > blockSize,
                "Block size [%s] is larger than upper bound [%s] for bit width [%s]", blockSize, idBlockUpperBound,
                uniqueIdBitWidth);
        ...
        while (methodTime.elapsed().compareTo(timeout) < 0) {
            ...
            try {
                ...
                if (idBlockUpperBound - blockSize <= nextStart) {
                    log.info(
                            "ID overflow detected on partition({})-namespace({}) with uniqueid {}. Current id {}, block size {}, and upper bound {} for bit width {}.",
                            partition, idNamespace, uniquePID, nextStart, blockSize, idBlockUpperBound,
                            uniqueIdBitWidth);
                    if (randomizeUniqueId) {
                        exhaustedUniquePIDs.add(uniquePID);
                        if (exhaustedUniquePIDs.size() == randomUniqueIDLimit)
                            throw new IDPoolExhaustedException(
                                    String.format("Exhausted %d uniqueid(s) on partition(%d)-namespace(%d): %s",
                                            exhaustedUniquePIDs.size(), partition, idNamespace,
                                            Joiner.on(",").join(exhaustedUniquePIDs)));
                        else
                            throw new UniqueIDExhaustedException(String.format(
                                    "Exhausted ID partition(%d)-namespace(%d) with uniqueid %d (uniqueid attempt %d/%d)",
                                    partition, idNamespace, uniquePID, exhaustedUniquePIDs.size(),
                                    randomUniqueIDLimit));
                    }
                    throw new IDPoolExhaustedException("Exhausted id block for partition(" + partition + ")-namespace("
                            + idNamespace + ") with upper bound: " + idBlockUpperBound);
                }
                ...
                try {
                    ...
                    BackendOperation.execute(new BackendOperation.Transactional<Boolean>() {
                        ...
                        public Boolean call(StoreTransaction txh) throws BackendException {
                            idStore.mutate(partitionKey, Arrays.asList(StaticArrayEntry.of(finalTarget)),
                    ...
                    if (idApplicationWaitMS.compareTo(writeElapsed) < 0) {
                        throw new TemporaryBackendException("Wrote claim for id block [" + nextStart + ", " + nextEnd
                                + ") in " + (writeElapsed) + " => too slow, threshold is: " + idApplicationWaitMS);
                    } else {
                        ...
                        /*
                         * At this point we've written our claim on [nextStart, nextEnd), but we haven't yet guaranteed
                         * the absence of a contending claim on the same id block from another machine
                         */
                        ...
                        List<Entry> blocks =
                                BackendOperation.execute(new BackendOperation.Transactional<List<Entry>>() {
                                    @Override
                                    public List<Entry> call(StoreTransaction txh) throws BackendException {
                                        return idStore.getSlice(new KeySliceQuery(partitionKey, slice[0], slice[1]),
                                                txh);
                                    }
                                }, this, times);
                        if (blocks == null)
                        ...
                        if (blocks.isEmpty())
                            throw new PermanentBackendException(
                                    "It seems there is a race-condition in the block application. "
                                            + "If you have multiple HugeGraph instances running on one physical machine, ensure that they have unique machine idAuthorities");

                        /*
                         * If our claim is the lexicographically first one, then our claim is the most senior one and we
                         * own this id block
                         */
                        if (target.equals(blocks.get(0).getColumnAs(StaticBuffer.STATIC_FACTORY))) {

                            ConsistentKeyIDBlock idblock =
                                    new ConsistentKeyIDBlock(nextStart, blockSize, uniqueIdBitWidth, uniquePID);
                            ...
                            log.debug("Failed to acquire ID block [{},{}) (another host claimed it first)", nextStart,
                                    nextEnd);
                        }
                    }
                } finally {
                        ...
                        for (int attempt = 0; attempt < rollbackAttempts; attempt++) {
                            try {
                                ...
                                BackendOperation.execute(new BackendOperation.Transactional<Boolean>() {
                                    @Override
                                    public Boolean call(StoreTransaction txh) throws BackendException {
                                        idStore.mutate(partitionKey, KeyColumnValueStore.NO_ADDITIONS,
                                                Arrays.asList(finalTarget), txh);
                                        return true;
                                    }
                                }, new BackendOperation.TransactionalProvider() { // Use normal consistency level for
                                                                                  // these non-critical delete
                                                                                  // operations
                                    @Override
                                    public StoreTransaction openTx() throws BackendException {
                                        return manager.beginTransaction(storeTxConfigBuilder.build());
                                ...
                                log.warn("Storage exception while deleting old block application - retrying in {}",
                                        rollbackWaitTime, e);
        ...
        throw new TemporaryLockingException(String.format(
                "Reached timeout %d (%s elapsed) when attempting to allocate id block on partition(%d)-namespace(%d)",
    ...
    private final StaticBuffer getBlockApplication(long blockValue, Instant timestamp) {
        WriteByteBuffer bb = new WriteByteBuffer(8 // counter long
                + 8 // time in ms
                + uidBytes.length);
    ...
    private static class ConsistentKeyIDBlock implements IDBlock {
        ...
        public String toString() {
            ...
            if (uniqueIDBitWidth > 0)
                interval += "/" + uniqueID + ":" + uniqueIDBitWidth;
            return interval;
        }
    }
}
