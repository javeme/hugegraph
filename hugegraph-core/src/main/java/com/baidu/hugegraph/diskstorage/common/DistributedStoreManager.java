...
public abstract class DistributedStoreManager extends AbstractStoreManager {
    ...
    public DistributedStoreManager(Configuration storageConfig, int portDefault) {
        ...
        if (storageConfig.has(STORAGE_PORT))
            this.port = storageConfig.get(STORAGE_PORT);
        else
    ...
    /**
     * Returns the default configured page size for this storage backend. The page size is used to determine the number
     * of records to request at a time when streaming result data.
     * 
     * @return
     */
    ...
    /**
     * This method attempts to generate Rid in the following three ways, in order, returning the value produced by the
     * first successful attempt in the sequence.
     * <p/>
     * <ol>
     * <li>If {@code config} contains {@see GraphDatabaseConfiguration#INSTANCE_RID_RAW_KEY}, then read it as a String
     * value. Convert the String returned into a char[] and call
     * {@code org.apache.commons.codec.binary.Hex#decodeHex on the char[]}. The byte[] returned by {@code decodeHex} is
     * then returned as Rid.</li>
     * <li>If {@code config} contains {@see GraphDatabaseConfiguration#INSTANCE_RID_SHORT_KEY}, then read it as a short
     * value. Call {@see java.net.InetAddress#getLocalHost()}, and on its return value call
     * {@see java.net.InetAddress#getAddress()} to retrieve the machine's IP address in byte[] form. The returned Rid is
     * a byte[] containing the localhost address bytes in its lower indices and the short value in its penultimate and
     * final indices.</li>
     * <li>If both of the previous failed, then call {@see java.lang.management.RuntimeMXBean#getName()} and then call
     * {@code String#getBytes()} on the returned value. Return a Rid as described in the previous point, replacing the
     * short value with the byte[] representing the JVM name.</li>
     * </ol>
     *
    ...
    // throw new HugeGraphConfigurationException("Could not decode hex value", e);
    // }
    //
    ...
    // throw new HugeGraphConfigurationException("Unknown host specified", e);
    // }
    //
    ...
    /**
     * Helper class to create the deletion and addition timestamps for a particular transaction. It needs to be ensured
     * that the deletion time is prior to the addition time since some storage backends use the time to resolve
     * conflicts.
     */
