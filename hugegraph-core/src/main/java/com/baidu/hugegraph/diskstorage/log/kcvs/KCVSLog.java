...
import com.baidu.hugegraph.diskstorage.log.util.FutureMessage;
import com.baidu.hugegraph.diskstorage.log.util.ProcessMessageJob;
...
import com.baidu.hugegraph.graphdb.configuration.PreInitializeConfigOptions;
...
/**
 * Implementation of {@link Log} wrapped around a {@link KeyColumnValueStore}. Each message is written as a column-value
 * pair ({@link Entry}) into a timeslice slot. A timeslice slot is uniquely identified by:
 * <ul>
 * <li>The partition id: On storage backends that are key-ordered, a partition bit width can be configured which
 * configures the number of first bits that comprise the partition id. On unordered storage backends, this is always
 * 0</li>
 * <li>A bucket id: The number of parallel buckets that should be maintained is configured by
 * {@link com.baidu.hugegraph.graphdb.configuration.GraphDatabaseConfiguration#LOG_NUM_BUCKETS}. Messages are written to
 * the buckets in round-robin fashion and each bucket is identified by a bucket id. Having multiple buckets per
 * timeslice allows for load balancing across multiple keys in the storage backend.</li>
 * <li>The start time of the timeslice: Each time slice is {@link #TIMESLICE_INTERVAL} microseconds long. And all
 * messages that are added between start-time and start-time+{@link #TIMESLICE_INTERVAL} end up in the same timeslice.
 * For high throughput logs that might be more messages than the underlying storage backend can handle per key. In that
 * case, ensure that (2^(partition-bit-width) x (num-bucekts) is large enough to distribute the load.</li>
 * </ul>
 *
 * Each message is uniquely identified by its timestamp, sender id (which uniquely identifies a particular instance of
 * {@link KCVSLogManager}), and the message id (which is auto-incrementing). These three data points comprise the column
 * of a log message. The actual content of the message is written into the value.
 * </p>
 * When {@link MessageReader} are registered, one reader thread per partition id and bucket is created which
 * periodically (as configured) checks for new messages in the storage backend and invokes the reader. </br>
 * Read-markers are maintained (for each partition-id & bucket id combination) under a dedicated key in the same
 * {@link KeyColumnValueStoreManager} as the log messages. The read markers are updated to the current position before
 * each new iteration of reading messages from the log. If the system fails while reading a batch of messages, a
 * subsequently restarted log reader may therefore read messages twice. Hence, {@link MessageReader} implementations
 * should exhibit correct behavior for the (rare) circumstance that messages are read twice.
 *
 * Note: All time values in this class are in mircoseconds. Hence, there are many cases where milliseconds are converted
 * to microseconds.
 *
...
public class KCVSLog implements Log, BackendOperation.TransactionalProvider {
    ...
    /**
     * The time period that is stored under one key in the underlying KCVS. This value should NEVER be changed since
     * this will cause backwards incompatibility. This setting is not configurable. If too many messages end up under
     * one key, please configure either 1) the number of buckets or 2) introduce partitioning.
     */
    public static final long TIMESLICE_INTERVAL = 100L * 1000 * 1000; // 100 seconds

    /**
     * For batch sending to make sense against a KCVS, the maximum send/delivery delay must be at least this number. If
     * the delivery delay is configured to be smaller than this time interval, messages will be send immediately since
     * batching will likely be ineffective.
     */
    private final static Duration MIN_DELIVERY_DELAY = Duration.ofMillis(10L);

    /**
     * Multiplier for the maximum number of messages to hold in the outgoing message queue before producing back
     * pressure. Multiplied with the message sending batch size. If back pressure is a regular occurrence, decrease the
     * sending interval or increase the sending batch size
     */
    ...
    /**
     * All system settings use this value for the partition id (first 4 byte of the key). This is all 1s in binary
     * representation which would be an illegal value for a partition id. Hence, we avoid conflict.
     */
    private final static int SYSTEM_PARTITION_ID = 0xFFFFFFFF;

    /**
     * The first byte of any system column is used to indicate the type of column. The next two variables define the
     * prefixes for message counter columns (i.e. keeping of the log message numbers) and for the marker columns (i.e.
     * keeping track of the timestamps to which it has been read)
     */
     ...
     */
    private final static StaticBuffer MESSAGE_COUNTER_COLUMN =
            new WriteByteBuffer(1).putByte(MESSAGE_COUNTER).getStaticBuffer();
    ...
    /**
     * Used for batch addition of messages to the log. Newly added entries are buffered in this queue before being
     * written in batch
     */
    private final ArrayBlockingQueue<MessageEnvelope> outgoingMsg;
    /**
     * Background thread which periodically writes out the queued up messages. TODO: consider batching messages across
     * ALL logs
     */
    ...
    public KCVSLog(String name, KCVSLogManager manager, KeyColumnValueStore store, Configuration config) {
        ...
        if (MIN_DELIVERY_DELAY.compareTo(maxSendDelay) <= 0) { // No need to locally queue messages since they will be
                                                               // sent immediately
    ...
    public synchronized void close() throws BackendException {
        ...
        if (readExecutor != null) {
            ...
            if (!readExecutor.isTerminated()) {
                ...
                log.error("Reader thread pool for KCVSLog " + name
                        + " did not shut down in time - could not clean up or set read markers");
            } else {
    ...
    public StoreTransaction openTx() throws BackendException {
        ...
        if (keyConsistentOperations) {
            config = StandardBaseTransactionConfig.of(times,
                    manager.storeManager.getFeatures().getKeyConsistentTxConfig());
        } else {
            config = StandardBaseTransactionConfig.of(times);
        }
    ...
    public Future<Message> add(StaticBuffer content, StaticBuffer key, ExternalPersistor persistor) {
        ...
        if (manager.partitionBitWidth == 0)
            partitionId = 0;
        else
    ...
    /**
     * Adds the given message (content) to the timeslice for the partition identified by the provided partitionId. If a
     * persistor is specified, this persistor is used to add the message otherwise the internal delivery systems are
     * used.
     *
     * @param content
     * @param partitionId
    ...
    private Future<Message> add(StaticBuffer content, int partitionId, ExternalPersistor persistor) {
        ...
        Preconditions.checkArgument(partitionId >= 0 && partitionId < (1 << manager.partitionBitWidth),
                "Invalid partition id: %s", partitionId);
        ...
        StaticBuffer key = getLogKey(partitionId, (int) (numBucketCounter.incrementAndGet() % numBuckets),
                getTimeSlice(timestamp));
        ...
        if (persistor != null) {
            ...
            try {
                ...
                throw new HugeGraphException("Got interrupted waiting to send message", e);
            }
        }
        return fmsg;
    }
    ...
    private static class MessageEnvelope {
        ...
        public String toString() {
            return "MessageEnvelope[message=" + message + ",key=" + key + ",entry=" + entry + "]";
        }
    }
        ...
        try {
            boolean success = BackendOperation.execute(new BackendOperation.Transactional<Boolean>() {
                ...
                public Boolean call(StoreTransaction txh) throws BackendException {
                    ...
                    Map<StaticBuffer, KCVMutation> muts =
                            new HashMap<StaticBuffer, KCVMutation>(mutations.keySet().size());
                    ...
                    log.debug("Wrote {} total envelopes with operation timestamp {}", msgEnvelopes.size(),
                            txh.getConfiguration().getCommitTime());
                    return Boolean.TRUE;
                }
    ...
    /**
     * This background thread only gets started when messages are locally queued for up to a maximum number of
     * microseconds or until the maximum number of local messages is reached. This thread waits for either event and
     * then triggers {@link #sendMessages(java.util.List)} call to persist the messages.
     */
    private class SendThread extends BackgroundThread {
        ...
        protected void action() {
            MessageEnvelope msg;
            // Opportunistically drain the queue for up to the batch-send-size number of messages before evaluating
            // condition
            while (toSend.size() < sendBatchSize && (msg = outgoingMsg.poll()) != null) {
                toSend.add(msg);
            }
            // Evaluate send condition: 1) Is the oldest message waiting longer than the delay? or 2) Do we have enough
            // messages to send?
            if (!toSend.isEmpty()
                    && (maxSendDelay.compareTo(timeSinceFirstMsg()) <= 0 || toSend.size() >= sendBatchSize)) {
                try {
                    sendMessages(toSend);
                } finally {
                    toSend.clear();
        ...
        if (firstRegistration && !this.readers.isEmpty()) {
            ...
            for (int partitionId : manager.readPartitionIds) {
                for (int bucketId = 0; bucketId < numBuckets; bucketId++) {
                    ...
                    log.debug("Creating log read executor: initialDelay={} delay={} unit={}",
                            INITIAL_READER_DELAY.toNanos(), readPollingInterval.toNanos(), TimeUnit.NANOSECONDS);
                    readExecutor.scheduleWithFixedDelay(msgPullers[pos], INITIAL_READER_DELAY.toNanos(),
                            readPollingInterval.toNanos(), TimeUnit.NANOSECONDS);
                    pos++;
                }
            }
        }
    ...
    /**
     * Thread which runs to read all messages from a particular partition id and bucket up to the next timeslice or
     * current timestamp minus the configured read lag time {@link #LOG_READ_LAG_TIME}. The read marker is used to
     * initialize the start time to read from. If a read marker is configured, then the read marker time is looked up
     * for initialization.
     */
    private class MessagePuller implements Runnable {
        ...
        public void run() {
            try {
                ...
                Instant messageTimeEnd = 0 > maxSafeMessageTime.compareTo(timesliceEnd)
                        /* maxSafeMessageTime < timesliceEnd */ ? maxSafeMessageTime : timesliceEnd;
                ...
                if (0 > messageTimeStart.compareTo(messageTimeEnd)) {
                    ...
                    /*
                     * nextTimepoint is equal to or later than timeWindowEnd. We can't run a column slice using these
                     * timestamps, since the start would be greater than the end.
                     *
                     * This could happen during a brief window right after startup with ReadMarker.fromNow(). However,
                     * if nextTimestamp is much later than timeWindowEnd, then something is probably misconfigured.
                     */
                    final Duration delta = Duration.between(messageTimeEnd, messageTimeStart);

                    if (delta.toNanos() / 3 > readLagTime.toNanos()) {
                        log.warn(
                                "MessagePuller configured with ReadMarker timestamp in the improbably distant future: {} (current time is {})",
                                messageTimeStart, currentTime);
                    } else {
                        log.debug(
                                "MessagePuller configured with ReadMarker timestamp slightly ahead of read lag time; waiting for the clock to catch up");
                    }

                    return;
                }
                Preconditions.checkState(messageTimeStart.compareTo(messageTimeEnd) < 0);
                Preconditions.checkState(messageTimeEnd.compareTo(currentTime) <= 0,
                        "Attempting to read messages from the future: messageTimeEnd=% vs currentTime=%s",
                        messageTimeEnd, currentTime);

                StaticBuffer logKey = getLogKey(partitionId, bucketId, timeslice);
                KeySliceQuery query =
                        new KeySliceQuery(logKey, BufferUtil.getLongBuffer(times.getTime(messageTimeStart)),
                                BufferUtil.getLongBuffer(times.getTime(messageTimeEnd)));
                ...
                if (entries.size() >= maxReadMsg) {
                    ...
                    // Adding 2 microseconds (=> very few extra messages), not adding one to avoid that the slice is
                    // possibly empty
                    messageTimeEnd = messageTimeEnd.plus(TWO_MICROSECONDS);
                    log.debug("Extended time window to {}", messageTimeEnd);
                    // Retrieve all messages up to this adjusted timepoint (no limit this time => get all entries to
                    // that point)
                    query = new KeySliceQuery(logKey, BufferUtil.nextBiggerBuffer(lastEntry.getColumn()),
                            BufferUtil.getLongBuffer(times.getTime(messageTimeEnd)));
                    log.debug("Converted extended MessagePuller time window to {}", query);
                    List<Entry> extraEntries =
                            BackendOperation.execute(getOperation(query), KCVSLog.this, times, maxReadTime);
                    prepareMessageProcessing(extraEntries);
                }
                messageTimeStart = messageTimeEnd;
            } catch (Throwable e) {
                log.warn("Could not read messages for timestamp [" + messageTimeStart + "] (this read will be retried)",
                        e);
            }
        }
        ...
        private void initializeTimepoint() {
            ...
            if (!readMarker.hasIdentifier()) {
                ...
                long savedTimestamp = readSetting(readMarker.getIdentifier(), getMarkerColumn(partitionId, bucketId),
                        times.getTime(readMarker.getStartTime(times)));
        ...
        private void setReadMarker() {
            if (readMarker.hasIdentifier()) {
                try {
                    ...
                    writeSetting(readMarker.getIdentifier(), getMarkerColumn(partitionId, bucketId),
                    ...
                    log.error("Could not persist read marker [" + readMarker.getIdentifier() + "] on bucket ["
                            + bucketId + "] + partition [" + partitionId + "]", e);
                }
            }
        }
