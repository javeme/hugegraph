...
import com.baidu.hugegraph.util.system.Threads;
 ...
 */
class StandardScannerExecutor extends AbstractFuture<ScanMetrics>
        implements HugeGraphManagement.IndexJobFuture, Runnable {
    ...
    StandardScannerExecutor(final ScanJob job, final Consumer<ScanMetrics> finishJob, final KeyColumnValueStore store,
            final StoreTransaction storeTx, final StoreFeatures storeFeatures, final int numProcessors,
            final int workBlockSize, final Configuration jobConfiguration, final Configuration graphConfiguration)
        ...
        try {
            ...
            while (!interrupted) {
                for (int i = 0; i < numQueries; i++) {
                    ...
                    if (qr == null) {
                        if (pullThreads[i].isFinished())
                            continue; // No more data to be expected
                ...
                if (conditionQuery == null)
                    break; // Termination condition - primary query has no more data
    ...
    private static class DataPuller extends Thread {
        ...
        private DataPuller(SliceQuery query, BlockingQueue<SliceResult> queue, KeyIterator keyIter,
