...
public class ConfigOption<O> extends ConfigElement {
    ...
    public enum Type {
        /**
         * Once the database has been opened, these configuration options cannot be changed for the entire life of the
         * database
         */
        FIXED,
        /**
    ...
    /**
     * This is a subset of types accepted by StandardSerializer. Its subset-ness is enforced in the static initializer
    ...
    static {
        ACCEPTED_DATATYPES = ImmutableSet.of(ConflictAvoidanceMode.class, Duration.class, TimestampProviders.class,
                Instant.class, Boolean.class, Short.class, Integer.class, Byte.class, Long.class, Float.class,
        ...
        for (Class<?> c : ACCEPTED_DATATYPES) {
            if (!ss.validDataType(c)) {
                String msg = String.format("%s datatype %s is not accepted by %s", ConfigOption.class.getSimpleName(),
                        c, StandardSerializer.class.getSimpleName());
                log.error(msg);
    ...
    public ConfigOption(ConfigNamespace parent, String name, String description, Type type, Class<O> datatype,
            O defaultValue, Predicate<O> verificationFct) {
        this(parent, name, description, type, datatype, defaultValue, verificationFct, null);
    }

    public ConfigOption(ConfigNamespace parent, String name, String description, Type type, Class<O> datatype,
            O defaultValue, Predicate<O> verificationFct, ConfigOption<?> supersededBy) {
    ...
    /**
     * Returns true on config options whose values are not local or maskable, that is, cluster-wide options that are
     * either fixed or which can be changed only by using the
     * {@link com.baidu.hugegraph.graphdb.database.management.ManagementSystem} (and not by editing the local config).
     *
    ...
    public O get(Object input) {
        ...
        if (input == null) {
            Preconditions.checkState(verificationFct.apply((O) input), "Need to set configuration value: %s",
                    this.toString());
            return null;
        } else {
            return verify(input);
    ...
    public O verify(Object input) {
        ...
        Preconditions.checkArgument(datatype.isInstance(input),
                ...
                input.getClass());
        O result = (O) input;
        Preconditions.checkArgument(verificationFct.apply(result), "Invalid configuration value for [%s]: %s",
                this.toString(), input);
        return result;
    }
        ...
        return new Predicate<O>() {
            ...
            public boolean apply(@Nullable O o) {
                ...
                if (o instanceof Collection
                        && (((Collection) o).isEmpty() || ((Collection) o).iterator().next() == null))
                    return false;
                return true;
            }
        };
