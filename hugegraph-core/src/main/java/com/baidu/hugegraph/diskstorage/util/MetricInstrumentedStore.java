...
/**
 * This class instruments an arbitrary KeyColumnValueStore backend with Metrics. The cumulative runtime of, number of
 * invocations of, and number of exceptions thrown by each interface method are instrumented with Metrics (using Timer,
 * Counter, and Counter again, respectively). The Metric names are generated by calling
 * {@link MetricRegistry#name(backendClass, methodName, identifier)}, where methodName is the exact name of the method
 * including capitalization, and identifier is "time", "calls", or "exceptions".
 * <p/>
 * In addition to the three standard metrics, {@code getSlice} and {@code getKeys} have some additional metrics related
 * to their return values. {@code getSlice} carries metrics with the identifiers "entries-returned" and
 * "entries-histogram". The first is a counter of total Entry objects returned. The second is a histogram of the size of
 * Entry lists returned. {@code getKeys} returns a {@link RecordIterator} that manages metrics for its methods.
 * <p/>
 * This implementation does not catch any exceptions. Exceptions emitted by the backend store implementation are
 * guaranteed to pass through this implementation's methods.
 * <p/>
 * The implementation includes repeated {@code try...catch} boilerplate that could be reduced by using reflection to
 * determine the method name and by delegating Metrics object handling to a common helper that takes a Callable closure,
 * but I'm not sure that the extra complexity and potential performance hit is worth it.
 *
...
public class MetricInstrumentedStore implements KeyColumnValueStore {
    ...
    public EntryList getSlice(final KeySliceQuery query, final StoreTransaction txh) throws BackendException {
        return runWithMetrics(txh, metricsStoreName, M_GET_SLICE, new StorageCallable<EntryList>() {
    ...
    public Map<StaticBuffer, EntryList> getSlice(final List<StaticBuffer> keys, final SliceQuery query,
        ...
        return runWithMetrics(txh, metricsStoreName, M_GET_SLICE, new StorageCallable<Map<StaticBuffer, EntryList>>() {
    ...
    @Override
    public void mutate(final StaticBuffer key, final List<Entry> additions, final List<StaticBuffer> deletions,
            final StoreTransaction txh) throws BackendException {
        runWithMetrics(txh, metricsStoreName, M_MUTATE, new StorageCallable<Void>() {
    ...
    @Override
    public void acquireLock(final StaticBuffer key, final StaticBuffer column, final StaticBuffer expectedValue,
            final StoreTransaction txh) throws BackendException {
        runWithMetrics(txh, metricsStoreName, M_ACQUIRE_LOCK, new StorageCallable<Void>() {
            public Void call() throws BackendException {
    ...
    public KeyIterator getKeys(final KeyRangeQuery query, final StoreTransaction txh) throws BackendException {
        return runWithMetrics(txh, metricsStoreName, M_GET_KEYS, new StorageCallable<KeyIterator>() {
            public KeyIterator call() throws BackendException {
                ...
                if (txh.getConfiguration().hasGroupName()) {
                    return MetricInstrumentedIterator.of(ki, txh.getConfiguration().getGroupName(), metricsStoreName,
                            M_GET_KEYS, M_ITERATOR);
                } else {
                    return ki;
                }
            }
    ...
    static <T> T runWithMetrics(StoreTransaction txh, String storeName, String name, StorageCallable<T> impl)
