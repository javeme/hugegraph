 ...
 * concurrent HugeGraph instances using a shared storage backend. These strategies
...
public enum ConflictAvoidanceMode {
     ...
     * compromise correctness. However, in a clustered HugeGraph deployment, this
     ...
     * HugeGraph instance a unique conflict avoidance tag in its local graph
     * configuration. The tag becomes part of globally-visible graph element IDs
     * allocated by that HugeGraph instance. Each HugeGraph instance assumes it has
     ...
     * <p>
     * This is useful for reducing write latency in HugeGraph deployments atop a
     * multi-datacenter distributed store.
     * <p>
     * <b>When this mode is set, the user is responsible for setting a unique
     * tag in each HugeGraph graph configuration. Setting the same tag on two
     * different HugeGraph configurations can lead to silent graph corruption in
    ...
    /**
     * The user assigns a tag to each HugeGraph instance. The tags should be
     ...
     * is safe in this mode. HugeGraph uses global-quorum-level or greater on
     * storage backends that have a notion of eventual consistency, so HugeGraph
    ...
    /**
     * HugeGraph randomly selects a tag from the space of all possible tags when
