...
import com.baidu.hugegraph.diskstorage.keycolumnvalue.KCVSProxy;
...
/**
 * A {@link KeyColumnValueStore} wrapper intended for nontransactional stores that forwards all <b>but</b> these two
 * methods to an encapsulated store instance:
 * <p/>
 * <ul>
 ...
 * <p/>
 * This class, along with its collaborator class {@link ExpectedValueCheckingTransaction}, track all
 * {@code expectedValue} arguments passed to {@code acquireLock} for each {@code StoreTransaction}. When the transaction
 * first {@code mutate(...)}s, the these classes cooperate to check that all previously provided expected values match
 * actual values, throwing an exception and preventing mutation if a mismatch is detected.
 * <p/>
 * This relies on a {@code Locker} instance supplied during construction for locking.
 */
public class ExpectedValueCheckingStore extends KCVSProxy {
     ...
     * This implementation supports locking when {@code lockStore} is non-null.
     * <p/>
     * Consider the following scenario. This method is called twice with identical key, column, and txh arguments, but
     * with different expectedValue arguments in each call. In testing, it seems HugeGraph's graphdb requires that
     * implementations discard the second expectedValue and, when checking expectedValues vs actual values just prior to
     * mutate, only the initial expectedValue argument should be considered.
     */
    @Override
