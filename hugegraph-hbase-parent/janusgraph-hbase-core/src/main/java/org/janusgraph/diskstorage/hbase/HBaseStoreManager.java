...
public class HBaseStoreManager extends DistributedStoreManager implements KeyColumnValueStoreManager {
    ...
    /**
     * The total number of HBase regions to create with JanusGraph's table. This setting only effects table creation;
     * this normally happens just once when JanusGraph connects to an HBase backend for the first time.
     */
    public static final ConfigOption<Integer> REGION_COUNT = new ConfigOption<Integer>(HBASE_NS, "region-count",
            "The number of initial regions set when creating JanusGraph's HBase table", ConfigOption.Type.MASKABLE,
     ...
     * <p/>
     * If JanusGraph's HBase table does not exist, then it will be created with total region count = (number of servers
     ...
     * <p>
     * When this <b>is not</b> set, JanusGraph attempts to automatically detect the HBase runtime version by calling
     * {@link VersionInfo#getVersion()}. JanusGraph then checks the returned version string against a hard-coded list of
     * supported version prefixes and instantiates the associated compat layer if a match is found.
     * <p>
     * When this <b>is</b> set, JanusGraph will not call {@code VersionInfo.getVersion()} or read its hard-coded list of
     * supported version prefixes. JanusGraph will instead attempt to instantiate the class specified (via the no-arg
     * constructor which must exist) and then attempt to cast it to HBaseCompat and use it as such. JanusGraph will
     ...
     * JanusGraph doesn't yet officially support.
     *
     */
    ...
    public HBaseStoreManager(org.janusgraph.diskstorage.configuration.Configuration config) throws BackendException {
        ...
        if (config.has(REGIONS_PER_SERVER) && config.has(REGION_COUNT)) {
            logger.warn(
                    "Both {} and {} are set in JanusGraph's configuration, but "
     ...
     * {@link KeyRange} expressing the region's start and end key bounds using JanusGraph-partitioning-friendly
